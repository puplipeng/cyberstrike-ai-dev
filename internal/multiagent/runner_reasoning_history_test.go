...
func TestHistoryToMessagesDropsStaleToolSearchCatalog(t *testing.T) {
	catalog := "<available-deferred-tools>\nold_tool\n</available-deferred-tools>"
	history := []agent.ChatMessage{
		{Role: "user", Content: "retain this user request", ModelFacingTrace: true},
		{Role: "user", Content: catalog, ModelFacingTrace: true},
		{Role: "assistant", Content: "prior result", ModelFacingTrace: true},
	}
	msgs := historyToMessages(history, nil, nil)
	if len(msgs) != 2 || msgs[0].Content != history[0].Content || msgs[1].Content != history[2].Content {
		t.Fatalf("stale SDK catalog restored: %#v", msgs)
	}
	// Ordinary message history is not a versioned SDK trace; do not silently
	// remove a user's explicitly supplied text from that source.
	msgs = historyToMessages([]agent.ChatMessage{{Role: "user", Content: catalog}}, nil, nil)
	if len(msgs) != 1 || msgs[0].Content != catalog {
		t.Fatal("ordinary user history was removed")
	}
}

func TestHistoryToMessagesRestoresOnlyVersionedConstraintLedger(t *testing.T) {
	ledger := wrapUserIntentLedger("- [U001] Preserve the authorized scope and do not modify data.")
	for _, versioned := range []bool{false, true} {
		history := []agent.ChatMessage{
			{Role: "system", Content: "old system prompt\n" + ledger + "\nold tool catalog", ModelFacingTrace: versioned},
			{Role: "user", Content: "continue"},
		}
		msgs := historyToMessages(history, &config.Config{}, nil)
		if !versioned {
			if len(msgs) != 1 || msgs[0].Role != "user" {
				t.Fatalf("legacy system prompt restored: %+v", msgs)
			}
			continue
		}
		if len(msgs) != 2 || msgs[0].Role != "system" || msgs[0].Content != ledger {
			t.Fatalf("compacted user constraints not restored independently of old prompt: %+v", msgs)
		}
	}
}
