...
func splitToolsForToolSearchByNames(all []tool.BaseTool, names []string, fallbackAlwaysVisible int) (static []tool.BaseTool, dynamic []tool.BaseTool, ok bool) {
	...
	if len(dynamic) == 0 {
		return static, nil, false
	}
	if len(static) == 0 {
		// Fall back to the positional limit only when no named tool is present.
		return splitToolsForToolSearch(all, fallbackAlwaysVisible)
	}
	return static, dynamic, true
}

// DefaultAlwaysVisibleToolNames is the small shared tool set kept in every
// default-role model request. Other registered tools stay discoverable through
// tool_search; being a backend builtin does not make its schema always visible.
// Return a fresh slice so the public configuration view cannot mutate defaults.
func DefaultAlwaysVisibleToolNames() []string {
	return []string{
...
func mergeAlwaysVisibleToolNames(configured []string) []string {
	defaults := DefaultAlwaysVisibleToolNames()
	merged := make([]string, 0, len(configured)+len(defaults))
