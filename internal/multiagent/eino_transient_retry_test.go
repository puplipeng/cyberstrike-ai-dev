...
func TestIsEinoTransientRunError(t *testing.T) {
	...
	cases := []struct {
		...
		{"task budget", modelbudget.ErrExceeded, false},
		{"wrapped task budget with stream marker", fmt.Errorf("failed to receive stream chunk: %w", modelbudget.ErrExceeded), false},
		{"serialized task budget with retry marker", errors.New("unexpected EOF: [task_token_budget_exhausted]"), false},
	}
