...
func newEinoSummarizationMiddleware(
	...
	guardedSummaryModel := newNonEmptySummaryChatModel(summaryModel)
	buildSummaryInput := func(ctx context.Context, sysInstruction, userInstruction adk.Message, originalMsgs []adk.Message, aggressive bool) ([]adk.Message, error) {
...
// Eino's Retry reuses the original model input. Context overflow needs its
// failover hook instead, which can regenerate a smaller request from the same
// pre-compaction history. Keep this one-attempt policy shared by both formats.
func newEinoSummaryOverflowFailover[M adk.MessageType](
	summaryModel model.BaseModel[M],
	buildInput func(context.Context, M, M, []M, bool) ([]M, error),
	logger *zap.Logger,
) *summarization.TypedFailoverConfig[M] {
	maxRetries := 1
	return &summarization.TypedFailoverConfig[M]{
		MaxRetries: &maxRetries,
		ShouldFailover: func(ctx context.Context, _ M, err error) bool {
			return ctx.Err() == nil && isEinoContextOverflowError(err)
		},
		GetFailoverModel: func(ctx context.Context, retry *summarization.TypedFailoverContext[M]) (model.BaseModel[M], []M, error) {
			input, err := buildInput(ctx, retry.SystemInstruction, retry.UserInstruction, retry.OriginalMessages, true)
			if err != nil {
				return nil, nil, err
			}
			if logger != nil {
				logger.Warn("eino summarization context overflow, retrying with rebuilt compacted input", zap.Error(retry.LastErr))
			}
			return summaryModel, input, nil
		},
	}
}

func summaryOverflowInputBudget(maxTokens int) int {
	budget := maxTokens * 70 / 100
...
func newEinoSummarizationModelOptions(outputReserve int, modelName, kind string, oa *config.OpenAIConfig, logger *zap.Logger) []model.Option {
	...
	return []model.Option{
		...
		llm.WithCodexReasoningEffort("low"),
...
func einoSummarizationTokenCounter(openAIModel string) summarization.TokenCounterFunc {
	...
	return func(ctx context.Context, input *summarization.TokenCounterInput) (int, error) {
		...
		for _, tl := range input.Tools {
			...
			cp := *tl
			cp.Extra = nil
			// ToolInfo.MarshalJSON has a pointer receiver; a value silently drops
			// the parameter schema held in ParamsOneOf's unexported fields.
