...
func RunDeepAgent(
	...
	agenticLoc, agenticSkillMW, agenticFSTools, agenticSkillsRoot, einoErr := prepareEinoAgenticSkills(ctx, appCfg.EffectiveSkillsDir(), ma, logger)
	if einoErr != nil {
		return nil, einoErr
	}
	...
	return runEinoADKAgentLoop(ctx, &einoADKRunLoopArgs{
		...
		MaxTotalTokens:          appCfg.OpenAI.MaxTotalTokens,
		MaxTaskTokens:           appCfg.Agent.MaxTaskTokensEffective(),
		ToolMaxBytes:            toolMaxBytesFromMW(&ma.EinoMiddleware),
		ModelName:               appCfg.OpenAI.Model,
		MiddlewareConfig:        &ma.EinoMiddleware,
	...
	for _, h := range history {
		...
		case "system":
			if h.ModelFacingTrace {
				if ledger := RestoreOriginalUserIntentLedger(h.Content, appCfg); ledger != "" {
					raw = append(raw, schema.SystemMessage(ledger))
				}
			}
		case "user":
			if h.ModelFacingTrace && IsToolSearchReminder(h.Content, nil) {
				continue
			}
