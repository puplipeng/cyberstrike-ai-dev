...
func TestEinoTokenEstimatesIncludeToolParameterSchema(t *testing.T) {
	ctx := context.Background()
	makeTool := func(description string) *schema.ToolInfo {
		return &schema.ToolInfo{
			Name: "inspect_asset", Desc: "Read one asset",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
				"asset": {Type: schema.String, Desc: description, Required: true},
			}),
		}
	}
	small := []*schema.ToolInfo{makeTool("identifier")}
	large := []*schema.ToolInfo{makeTool(strings.Repeat("unique parameter description ", 1000))}
	for name, count := range map[string]func([]*schema.ToolInfo) int{
		"summarization": func(tools []*schema.ToolInfo) int {
			n, err := einoSummarizationTokenCounter("gpt-4o")(ctx, &summarization.TokenCounterInput{Tools: tools})
			if err != nil {
				t.Fatal(err)
			}
			return n
		},
		"telemetry": func(tools []*schema.ToolInfo) int {
			return estimateTokensForMessagesAndTools(ctx, "gpt-4o", nil, tools)
		},
	} {
		t.Run(name, func(t *testing.T) {
			before, after := count(small), count(large)
			if after <= before+1000 {
				t.Fatalf("tool parameter schema was not counted: short=%d long=%d", before, after)
			}
		})
	}
}

type overflowOnceSummaryModel[M adk.MessageType] struct {
	inputs [][]M
	output M
}

func (m *overflowOnceSummaryModel[M]) Generate(_ context.Context, input []M, _ ...model.Option) (M, error) {
	m.inputs = append(m.inputs, input)
	if len(m.inputs) == 1 {
		var zero M
		return zero, errors.New("maximum context length exceeded")
	}
	return m.output, nil
}

func (m *overflowOnceSummaryModel[M]) Stream(ctx context.Context, input []M, opts ...model.Option) (*schema.StreamReader[M], error) {
	msg, err := m.Generate(ctx, input, opts...)
	if err != nil {
		return nil, err
	}
...
func summaryOverflowTestHistory() []adk.Message {
	msgs := []adk.Message{schema.SystemMessage("root instruction"), schema.UserMessage("Retain the authorized scope example.com.")}
	for i := 0; i < 40; i++ {
		msgs = append(msgs, schema.AssistantMessage(strings.Repeat("raw scan evidence ", 200), nil))
	}
	return msgs
}

func TestEinoSummarizationOverflowRetryRebuildsSmallerInput(t *testing.T) {
	ctx := context.Background()
	emit := false
	...
	fake := &overflowOnceSummaryModel[*schema.Message]{output: schema.AssistantMessage("<summary>Retain the authorized scope example.com.</summary>", nil)}
	mw, err := newEinoSummarizationMiddleware(ctx, fake, appCfg, mwCfg, "", nil, "", nil)
	if err != nil {
		t.Fatal(err)
	}
	_, after, err := mw.BeforeModelRewriteState(ctx, &adk.ChatModelAgentState{Messages: summaryOverflowTestHistory()}, nil)
	...
	beforeTokens, _ := countMessagesTokens(ctx, fake.inputs[0], counter, nil)
	afterTokens, _ := countMessagesTokens(ctx, fake.inputs[1], counter, nil)
