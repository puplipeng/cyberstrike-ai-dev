...
func injectToolNamesOnlyInstruction(ctx context.Context, instruction string, tools []tool.BaseTool, toolSearchMiddlewareActive bool) string {
	...
	if hasToolSearch {
		...
		sb.WriteString("3) 调用顺序：先 tool_search（必填参数 query：用关键词如 nuclei 搜索，或用 select:exact_tool_name 精确选择；可选 max_results 限制关键词结果数，默认 5）→ 在后续轮次确认目标工具已出现在 tools 列表且已阅读其 schema → 再发起对该工具的真实调用。不要把 query 当作正则表达式。\n")
		sb.WriteString("4) tool_search 返回的 matches 是匹配工具名列表；schema 在解锁后的下一轮才会下发。只选择当前步骤需要的工具，禁止在 schema 未出现时编造 JSON 参数。\n")
