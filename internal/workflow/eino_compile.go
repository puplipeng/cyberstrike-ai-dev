...
func extractAwaitingHITL(ctx context.Context, err error, art *compiledArtifact, runID string, args RunArgs, state *WorkflowLocalState) error {
	...
	if args.DB != nil {
		pending := map[string]any{
			...
			"tokenBudget":   modelbudget.FromContext(ctx).Snapshot(),
		}
...
func ResumeWorkflowRun(ctx context.Context, args RunArgs, runID string, approved bool, comment string) (*RunResult, error) {
	...
	// Paused results live in pending_hitl_json; output_json is written only
	// when the workflow finishes. Approval updates preserve this metadata.
	ctx = workflowTaskBudgetContext(ctx, args, run.PendingHITLJSON)
