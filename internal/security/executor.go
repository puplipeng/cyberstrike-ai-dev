...
func (e *Executor) buildToolIndex() {
	...
	for i := range e.config.Tools {
		tool := &e.config.Tools[i]
		if !tool.Enabled {
			continue
		}
		if tool.Name == "exec" || strings.HasPrefix(tool.Command, "internal:") {
			e.toolIndex[tool.Name] = tool
			continue
		}
		resolved, ok := resolveToolCommand(tool.Command)
		if !ok {
			e.logger.Warn("跳过不可用的安全工具",
				zap.String("tool", tool.Name),
				zap.String("command", tool.Command),
				zap.String("reason", "executable not found"),
			)
			continue
		}
		dependenciesAvailable := true
		for _, dependency := range tool.RequiredCommands {
			if _, dependencyOK := resolveToolCommand(dependency); !dependencyOK {
				e.logger.Warn("跳过缺少运行依赖的安全工具",
					zap.String("tool", tool.Name),
		...
		for _, module := range tool.RequiredPythonModules {
			if !pythonModuleAvailable(resolved, module) {
				e.logger.Warn("跳过缺少 Python 运行依赖的安全工具",
					zap.String("tool", tool.Name),
...
func (e *Executor) ExecuteTool(ctx context.Context, toolName string, args map[string]interface{}) (*mcp.ToolResult, error) {
	...
	// 构建命令。Windows 对命令行长度有限制，Python 脚本类工具会先落到临时文件。
	cmdArgs := e.buildCommandArgs(toolName, toolConfig, args)
	cleanup := func() {}
	if toolName == "execute-python-script" {
		var prepErr error
		cmdArgs, cleanup, prepErr = prepareUserPythonScript(args)
		if prepErr != nil {
			return &mcp.ToolResult{Content: []mcp.Content{{Type: "text", Text: prepErr.Error()}}, IsError: true}, nil
		}
	}
	defer cleanup()
	var inlineCleanup func()
	var prepErr error
	cmdArgs, inlineCleanup, prepErr = prepareLongInlinePythonScript(toolConfig.Command, cmdArgs)
	if prepErr != nil {
		return &mcp.ToolResult{Content: []mcp.Content{{Type: "text", Text: prepErr.Error()}}, IsError: true}, nil
	}
	defer inlineCleanup()
...
func (e *Executor) RegisterTools(mcpServer *mcp.Server) {
	...
	for i, toolConfig := range e.config.Tools {
		if _, available := e.toolIndex[toolConfig.Name]; !available {
			e.logger.Debug("跳过未启用或当前系统不可用的工具",
...
func (e *Executor) buildCommandArgs(toolName string, toolConfig *config.ToolConfig, args map[string]interface{}) []string {
	...
	if len(toolConfig.Parameters) > 0 {
		...
		for _, param := range positionalParams {
			if shouldDeferCommandParam(param) {
				continue
			}
		...
		for _, param := range flagParams {
			// 跳过特殊参数，它们会在后面单独处理。
			// 保留 position=0 的 action 子命令，兼容 steghide/cloudmapper/libc-database 这类封装。
			if shouldDeferCommandParam(param) {
				continue
			}
		...
		for i := 0; i <= maxPosition; i++ {
			...
			for _, param := range positionalParams {
				// 跳过特殊参数，它们会在后面单独处理。
				// 只有非位置 action 仍保留旧行为，避免把内部保留字段透传成 CLI flag。
				if shouldDeferCommandParam(param) {
					continue
				}
...
func shouldDeferCommandParam(param config.ParameterConfig) bool {
	switch param.Name {
	case "additional_args", "scan_type":
		return true
	case "action":
		// 保留旧工具把 action 当作内部保留 flag 的行为，但允许
		// position=0 的 action 作为子命令透传给 Windows 兼容封装。
		return param.Position == nil
	default:
		return false
	}
}
...
func (e *Executor) executeSystemCommand(ctx context.Context, args map[string]interface{}) (*mcp.ToolResult, error) {
	...
	shellArgs, preparedCommand, posixShell := prepareShellInvocation(shell, command)
	...
	isBackground := posixShell && IsBackgroundShellCommand(command)
	...
	if cb, ok := ctx.Value(ToolOutputCallbackCtxKey).(ToolOutputCallback); (ok && cb != nil) || mcp.MCPExecutionIDFromContext(ctx) != "" {
		...
		if err != nil && shouldRetryWithPTY(output) {
			...
			cmd2 := exec.CommandContext(ctx, shell, shellArgs...)
			if workDir != "" {
				cmd2.Dir = workDir
			}
...
func streamCommandOutput(ctx context.Context, cmd *exec.Cmd, cb ToolOutputCallback, noOutputSec int, maxBytes int, spill tooloutput.SpillOpts) (string, error) {
	...
	readFn := func(r io.Reader) {
		...
		for {
			...
			if n > 0 {
				chunks <- validUTF8String(string(buf[:n]))
			}
			if readErr != nil {
				return
			}
...
func isPythonCommand(command string) bool {
	base := strings.ToLower(strings.TrimSuffix(filepath.Base(strings.TrimSpace(command)), filepath.Ext(command)))
	return base == "python" || base == "python3" || strings.HasPrefix(base, "python3.")
}

// prepareLongInlinePythonScript avoids Windows CreateProcess' command-line limit by
// replacing `python -c <large script>` with `python <temporary file>`.
func prepareLongInlinePythonScript(command string, args []string) ([]string, func(), error) {
	cleanup := func() {}
	if runtime.GOOS != "windows" || !isPythonCommand(command) {
	...
	if idx < 0 || len(args[idx+1]) < 7000 {
		return args, cleanup, nil
	}
	f, err := os.CreateTemp("", "cyberstrike-tool-*.py")
	if err != nil {
		return nil, cleanup, fmt.Errorf("创建 Python 临时脚本失败: %w", err)
	}
	path := f.Name()
	if _, err = f.WriteString(args[idx+1]); err != nil {
		_ = f.Close()
		_ = os.Remove(path)
		return nil, cleanup, fmt.Errorf("写入 Python 临时脚本失败: %w", err)
	}
	if err = f.Close(); err != nil {
		_ = os.Remove(path)
		return nil, cleanup, fmt.Errorf("关闭 Python 临时脚本失败: %w", err)
	}
...
func prepareUserPythonScript(args map[string]interface{}) ([]string, func(), error) {
	script, ok := args["script"].(string)
	if !ok || strings.TrimSpace(script) == "" {
		return nil, func() {}, fmt.Errorf("错误: script 参数不能为空")
	}
	f, err := os.CreateTemp("", "cyberstrike-user-script-*.py")
	if err != nil {
		return nil, func() {}, fmt.Errorf("创建 Python 临时脚本失败: %w", err)
	}
	path := f.Name()
	if _, err = f.WriteString(script); err != nil {
		_ = f.Close()
		_ = os.Remove(path)
		return nil, func() {}, fmt.Errorf("写入 Python 临时脚本失败: %w", err)
	}
	if err = f.Close(); err != nil {
		_ = os.Remove(path)
		return nil, func() {}, fmt.Errorf("关闭 Python 临时脚本失败: %w", err)
	}
	cmdArgs := make([]string, 0, 4)
	if raw, ok := args["additional_args"].(string); ok && strings.TrimSpace(raw) != "" {
		cmdArgs = append(cmdArgs, strings.Fields(raw)...)
	}
	cmdArgs = append(cmdArgs, path)
	return cmdArgs, func() { _ = os.Remove(path) }, nil
}
...
func applyDefaultTerminalEnv(cmd *exec.Cmd) {
	...
	cmd.Env = appendToolRuntimePath(cmd.Env)
	cmd.Env = applyManagedToolStateEnv(cmd.Env)
	...
	if runtime.GOOS == "windows" {
		cmd.Env = setEnvValue(cmd.Env, "LANG", "C")
		cmd.Env = setEnvValue(cmd.Env, "LC_ALL", "C")
		cmd.Env = setEnvValue(cmd.Env, "LC_CTYPE", "C")
	}
}
