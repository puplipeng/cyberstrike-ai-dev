...
// In particular, POSIX `export`/`exec </dev/null` must never be sent to PowerShell.
func prepareShellInvocation(shell, shellCommand string) (args []string, prepared string, posix bool) {
	name := strings.ToLower(filepath.Base(strings.TrimSpace(shell)))
	name = strings.TrimSuffix(name, filepath.Ext(name))
	switch name {
	case "powershell", "pwsh":
		prepared = "$utf8 = [System.Text.UTF8Encoding]::new($false); " +
			"[Console]::OutputEncoding = $utf8; $OutputEncoding = $utf8; " + shellCommand
		return []string{"-NoLogo", "-NoProfile", "-NonInteractive", "-Command", prepared}, prepared, false
	case "cmd":
		prepared = "@chcp 65001 >NUL\r\n" + shellCommand
		return []string{"/D", "/S", "/C", prepared}, prepared, false
	case "sh", "bash", "dash", "zsh", "ksh":
		prepared = PrepareShellCommandForExecute(shellCommand)
		return []string{"-c", prepared}, prepared, true
	default:
		return []string{"-c", shellCommand}, shellCommand, false
	}
}
