...
func AuthMiddleware(manager *AuthManager) gin.HandlerFunc {
	return func(c *gin.Context) {
		if c.Query("token") != "" {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "URL session tokens are no longer supported"})
			return
		}
		token := extractTokenFromRequest(c)
		if token == "" && c.Request.Method == http.MethodGet && c.Request.URL.Path == TerminalSocketPath && strings.EqualFold(c.GetHeader("Upgrade"), "websocket") {
			for _, protocol := range strings.Split(c.GetHeader("Sec-WebSocket-Protocol"), ",") {
				if value, ok := strings.CutPrefix(strings.TrimSpace(protocol), TerminalTicketProtocol); ok {
					token = manager.consumeTerminalTicket(value)
					break
				}
			}
		}
