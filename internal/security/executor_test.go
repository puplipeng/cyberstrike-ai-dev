...
func TestExecutorSkipsToolWithMissingRuntimeDependency(t *testing.T) {
	logger := zap.NewNop()
	mcpServer := mcp.NewServer(logger)
	cfg := &config.SecurityConfig{Tools: []config.ToolConfig{{
		...
		Command:          "python3",
		Enabled:          true,
		RequiredCommands: []string{"cyberstrike-command-that-does-not-exist"},
	}}}

	executor := NewExecutor(cfg, mcpServer, logger)
	if _, ok := executor.toolIndex["wrapped-tool"]; ok {
		t.Fatal("tool with a missing secondary command dependency was registered")
	}
}
...
func TestBoundedOutputCollectorRepairsInvalidUTF8(t *testing.T) {
	collector := newBoundedOutputCollector(1024, nil)
	_, _ = collector.Write([]byte{0xcb, 0xf9, 'o', 'k'})
	if !utf8.ValidString(collector.String()) {
		t.Fatalf("collector kept invalid UTF-8: %q", collector.String())
	}
	if !strings.Contains(collector.String(), "ok") {
		t.Fatalf("collector lost valid suffix: %q", collector.String())
	}
}

func TestPrepareLongInlinePythonScriptOnWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows command-line limit regression")
	}
	script := strings.Repeat("print('ok')\n", 1000)
	args, cleanup, err := prepareLongInlinePythonScript("python.exe", []string{"-c", script, "--demo"})
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
	if len(args) != 2 || args[1] != "--demo" || args[0] == "-c" {
	...
	if string(data) != script {
		t.Fatal("temporary Python script content changed")
	}
}
...
func TestBuildCommandArgs_PreservesPositionalActionSubcommand(t *testing.T) {
	pos0 := 0
	pos1 := 1
	executor, _ := setupTestExecutor(t)
	toolConfig := &config.ToolConfig{
		...
		Command: "steghide",
		Parameters: []config.ParameterConfig{
			{Name: "action", Type: "string", Required: true, Position: &pos0, Format: "positional"},
			{Name: "cover_file", Type: "string", Flag: "-cf", Format: "flag"},
			{Name: "stego_file", Type: "string", Flag: "-sf", Format: "flag"},
			{Name: "target", Type: "string", Required: true, Position: &pos1, Format: "positional"},
		},
	}
	...
	args := map[string]interface{}{
		...
		"stego_file": "payload.jpg",
		"target":     "unused-tail",
	}

	cmdArgs := executor.buildCommandArgs("steghide", toolConfig, args)
	if len(cmdArgs) == 0 || cmdArgs[0] != "extract" {
		t.Fatalf("expected positional action subcommand first, got: %v", cmdArgs)
	}
	if indexOf(cmdArgs, "-cf") < 0 || indexOf(cmdArgs, "-sf") < 0 {
		t.Fatalf("expected regular flags to remain present, got: %v", cmdArgs)
	}
}

func TestBuildCommandArgs_SkipsLegacyFlagActionField(t *testing.T) {
	pos1 := 1
	executor, _ := setupTestExecutor(t)
	toolConfig := &config.ToolConfig{
		Name:    "http-framework-test",
		Command: "http-framework-test",
		Parameters: []config.ParameterConfig{
			{Name: "url", Type: "string", Required: true, Position: &pos1, Format: "positional"},
			{Name: "action", Type: "string", Flag: "--action", Format: "flag", Default: "request"},
		},
	}
	...
	cmdArgs := executor.buildCommandArgs("http-framework-test", toolConfig, args)
	joined := strings.Join(cmdArgs, " ")
	if strings.Contains(joined, "--action") || strings.Contains(joined, "request") {
		t.Fatalf("expected legacy internal action flag to stay hidden, got: %v", cmdArgs)
	}
}
