...
func permissionForRequest(method, fullPath string) string {
	...
	case strings.HasPrefix(path, "/github-leaks"):
		if method == http.MethodGet || method == http.MethodHead {
			return "vulnerability:read"
		}
		if path == "/github-leaks/run" {
			return "config:write"
		}
		return "vulnerability:write"
	case strings.HasPrefix(path, "/vulnerabilities"):
		return crudPermission(method, "vulnerability")
	case strings.HasPrefix(path, "/vulnerability-intel"):
		if strings.HasPrefix(path, "/vulnerability-intel/subscriptions") || strings.HasPrefix(path, "/vulnerability-intel/notifications") {
			return "vulnerability:read"
		}
		if strings.HasSuffix(path, "/analysis") && method == http.MethodPost {
			return "agent:execute"
		}
		if method == http.MethodGet || method == http.MethodHead {
			return "vulnerability:read"
	...
	case path == "/assets/batch-delete", path == "/assets/merge":
		return "asset:delete"
	case strings.HasPrefix(path, "/asset-monitors"):
	...
	case strings.HasPrefix(path, "/ssh/"):
		if path == "/ssh/connections" && (method == http.MethodGet || method == http.MethodHead) {
			return "webshell:read"
		}
		if method == http.MethodDelete {
			return "webshell:delete"
		}
		// Even GET terminal/file operations use stored remote credentials.
		return "webshell:write"
	...
	case strings.HasPrefix(path, "/skill-library"):
		if method == http.MethodGet || method == http.MethodHead {
			return "skills:read"
		}
		return "skills:write"
...
func isProcessGlobalMutationPath(path string) bool {
	if strings.HasPrefix(path, "/github-leaks") {
		return true
	}
	if strings.HasPrefix(path, "/vulnerability-intel") {
		return !strings.HasPrefix(path, "/vulnerability-intel/subscriptions") && !strings.HasPrefix(path, "/vulnerability-intel/notifications") && !strings.HasSuffix(path, "/analysis")
	}
	if strings.HasPrefix(path, "/roles") || strings.HasPrefix(path, "/skills") ||
		strings.HasPrefix(path, "/skill-library") ||
