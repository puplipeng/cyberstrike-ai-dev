...
func TestFormatCommandFailureFromErr(t *testing.T) {
	if os.Getenv("CSAI_EXIT_TEST_CHILD") == "1" {
		os.Exit(42)
	}
	cmd := exec.Command(os.Args[0], "-test.run=^TestFormatCommandFailureFromErr$")
	cmd.Env = append(os.Environ(), "CSAI_EXIT_TEST_CHILD=1")
	err := cmd.Run()
