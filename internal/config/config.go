...
type Config struct {
	...
	GitHubLeakMonitor GitHubLeakMonitorConfig `yaml:"github_leak_monitor,omitempty" json:"github_leak_monitor,omitempty"`
	...
	SkillLibrary      SkillLibraryConfig      `yaml:"skill_library,omitempty" json:"skill_library,omitempty"`
...
// Only loopback embedding services are accepted; indexing never sends package files to a cloud model.
type SkillLibraryConfig struct {
	...
	PoCsDir       string `yaml:"pocs_dir" json:"pocs_dir"`
	// Deprecated: general reference material is indexed by Knowledge Management.
	// Retained so existing configuration can be loaded without losing the value.
	ReferencesDir string `yaml:"references_dir" json:"references_dir"`
}
...
type MultiAgentPublic struct {
	...
	ToolSearchEnable                           bool     `json:"tool_search_enable"`
	ReductionEnable                            bool     `json:"reduction_enable"`
}
...
type MultiAgentAPIUpdate struct {
	...
	ToolSearchAlwaysVisibleTools *[]string `json:"tool_search_always_visible_tools,omitempty"`
	ToolSearchEnable             *bool     `json:"tool_search_enable,omitempty"`
	ReductionEnable              *bool     `json:"reduction_enable,omitempty"`
}
...
type RobotWechatConfig struct {
	Enabled       bool                     `yaml:"enabled" json:"enabled"`
	BotToken      string                   `yaml:"bot_token,omitempty" json:"bot_token,omitempty" secret:"true"`
...
type RobotWecomConfig struct {
	Enabled        bool                     `yaml:"enabled" json:"enabled"`
	Token          string                   `yaml:"token" json:"token" secret:"true"`                       // 回调 URL 校验 Token
	EncodingAESKey string                   `yaml:"encoding_aes_key" json:"encoding_aes_key" secret:"true"` // EncodingAESKey
	CorpID         string                   `yaml:"corp_id" json:"corp_id"`                                 // 企业 ID
	Secret         string                   `yaml:"secret" json:"secret" secret:"true"`                     // 应用 Secret
...
type RobotDingtalkConfig struct {
	...
	ClientSecret                string                   `yaml:"client_secret" json:"client_secret" secret:"true"`                     // 应用 Secret
...
type RobotLarkConfig struct {
	...
	AppSecret           string                   `yaml:"app_secret" json:"app_secret" secret:"true"`           // 应用 App Secret
	VerifyToken         string                   `yaml:"verify_token" json:"verify_token" secret:"true"`       // 事件订阅 Verification Token（可选）
...
type RobotTelegramConfig struct {
	Enabled            bool                     `yaml:"enabled" json:"enabled"`
	BotToken           string                   `yaml:"bot_token" json:"bot_token" secret:"true"`
...
type RobotSlackConfig struct {
	Enabled  bool                     `yaml:"enabled" json:"enabled"`
	BotToken string                   `yaml:"bot_token" json:"bot_token" secret:"true"` // xoxb-
	AppToken string                   `yaml:"app_token" json:"app_token" secret:"true"` // xapp-（connections:write）
	Auth     RobotAuthorizationConfig `yaml:"auth,omitempty" json:"auth,omitempty"`
}
...
type RobotQQConfig struct {
	...
	ClientSecret string                   `yaml:"client_secret" json:"client_secret" secret:"true"`
...
type ServerConfig struct {
	...
	// TrustedProxies lists reverse proxy IPs/CIDRs. Empty means direct connections;
	// client-supplied forwarding headers are ignored. Changes require a restart.
...
type MCPConfig struct {
	...
	AuthHeaderValue   string `yaml:"auth_header_value,omitempty" secret:"true"` // 全局服务凭证，仅 allow_global_access=true 时接受
...
func NormalizeOpenAIProviderProfile(oa *OpenAIConfig) {
	...
	if strings.EqualFold(strings.TrimSpace(oa.Provider), "codex_account") {
		oa.APIKey = ""
		oa.BaseURL = ""
		return
	}
...
func (c *Config) NormalizeAIProviderProfiles() {
	...
	if c.AI.Channels != nil {
		for id, ch := range c.AI.Channels {
			...
			if strings.EqualFold(strings.TrimSpace(ch.Provider), "codex_account") {
				ch.APIKey = ""
				ch.BaseURL = ""
			}
			c.AI.Channels[id] = ch
...
const (
	MinGitHubLeakIntervalSeconds     = 31
	MaxGitHubLeakIntervalSeconds     = 86400
	...
	MaxGitHubLeakQueryLiteralBytes   = 256
)

type GitHubLeakRuleConfig struct {
	Enabled  bool     `yaml:"enabled" json:"enabled"`
	Name     string   `yaml:"name" json:"name"`
	Keywords []string `yaml:"keywords" json:"keywords"`
}

// GitHubLeakMonitorConfig configures the defensive GitHub code-search monitor.
// The monitor stores only redacted evidence and never validates discovered credentials.
type GitHubLeakMonitorConfig struct {
	Enabled               bool                   `yaml:"enabled" json:"enabled"`
	Token                 string                 `yaml:"token,omitempty" json:"token,omitempty" secret:"true"`
	FingerprintKey        string                 `yaml:"fingerprint_key,omitempty" json:"fingerprint_key,omitempty" secret:"true"`
	Keywords              []string               `yaml:"keywords,omitempty" json:"keywords,omitempty"`
	Rules                 []GitHubLeakRuleConfig `yaml:"rules,omitempty" json:"rules,omitempty"`
...
func (c GitHubLeakMonitorConfig) IntervalSecondsEffective() int {
	if c.IntervalSeconds < MinGitHubLeakIntervalSeconds || c.IntervalSeconds > MaxGitHubLeakIntervalSeconds {
		return DefaultGitHubLeakIntervalSeconds
	}
	return c.IntervalSeconds
}

func (c GitHubLeakMonitorConfig) RequestTimeoutSecondsEffective() int {
	if c.RequestTimeoutSeconds <= 0 {
		return DefaultGitHubLeakTimeoutSeconds
	}
...
func (c GitHubLeakMonitorConfig) PerPageEffective() int {
	if c.PerPage <= 0 {
		return DefaultGitHubLeakPerPage
	}
	if c.PerPage > 100 {
		return 100
	}
...
func (c GitHubLeakMonitorConfig) ValidateFingerprintKey() error {
	key := strings.TrimSpace(c.FingerprintKey)
	if key == "" {
		return nil
	}
	if len(key) >= 4 && strings.EqualFold(key[:4], "hex:") {
		encoded := key[4:]
		if len(encoded) != 64 {
			return fmt.Errorf("github_leak_monitor.fingerprint_key 的 hex: 值必须包含 64 个十六进制字符")
		}
		if _, err := hex.DecodeString(encoded); err != nil {
			return fmt.Errorf("github_leak_monitor.fingerprint_key 的 hex: 值不是有效十六进制")
		}
		return nil
	}
	if len(key) < 16 {
		return fmt.Errorf("github_leak_monitor.fingerprint_key 至少需要 16 字节")
	}
	return nil
}
...
type DatabaseConfig struct {
	DSN           string `yaml:"dsn"`                      // PostgreSQL 连接串（必填）
	KnowledgeDSN  string `yaml:"knowledge_dsn,omitempty"`  // PostgreSQL 知识库连接串（可选，为空则复用 dsn）
	LocalTimezone string `yaml:"local_timezone,omitempty"` // 业务时区（IANA 名称，如 Asia/Shanghai），用于时间桶聚合；默认 UTC
}

type AgentConfig struct {
	MaxIterations int `yaml:"max_iterations" json:"max_iterations"`
	// MaxTaskTokens caps cumulative model tokens for one run. 0 uses the default;
	// -1 disables the limit. In-flight calls can exceed estimates; subsequent calls stop.
	MaxTaskTokens                      int `yaml:"max_task_tokens,omitempty" json:"max_task_tokens"`
...
func (c AgentConfig) MaxTaskTokensEffective() int {
	if c.MaxTaskTokens < 0 {
		return 0
	}
	if c.MaxTaskTokens == 0 {
		return DefaultMaxTaskTokens
	}
	return c.MaxTaskTokens
}
...
type ToolConfig struct {
	...
	RequiredCommands      []string          `yaml:"required_commands,omitempty"`       // 包装器运行时依赖的额外命令
	RequiredPythonModules []string          `yaml:"required_python_modules,omitempty"` // Python 包装器运行时依赖的模块
}
...
func Load(path string) (*Config, error) {
	...
	if cfg.Agent.MaxTaskTokens < -1 {
		return nil, fmt.Errorf("agent.max_task_tokens 必须为正数、0（默认）或 -1（不限）")
	}
...
func Default() *Config {
	...
	return &Config{
		...
		GitHubLeakMonitor: GitHubLeakMonitorConfig{
			Enabled:               false,
		...
		Agent: AgentConfig{
			...
			MaxTaskTokens:                      DefaultMaxTaskTokens,
...
type EmbeddingConfig struct {
	...
	APIKey   string `yaml:"api_key" json:"api_key" secret:"true"` // API Key（从OpenAI配置继承）
}
...
// MultiQueryConfig makes model-assisted query rewriting explicitly opt-in.
type MultiQueryConfig struct {
	Enabled bool `yaml:"enabled" json:"enabled"`
	// MaxQueries includes the original query; 1 skips rewriting, 0 defaults to 4.
