...
func SafeRelPath(root, rel string) (string, error) {
	...
	if !filepath.IsLocal(rel) || strings.Contains(rel, ":") || strings.HasPrefix(rel, "/") {
		return "", fmt.Errorf("invalid resource path %q", rel)
	}
	if rel == "" || rel == "." {
...
func ListPackageFiles(skillsRoot, skillID string) ([]PackageFileInfo, error) {
	root, err := OpenSkillRoot(skillsRoot, skillID)
	if err != nil {
		return nil, err
	}
	defer root.Close()
...
func WritePackageFile(skillsRoot, skillID, relPath string, content []byte) error {
	...
	return root.WriteFile(rel, content, 0644)
}
