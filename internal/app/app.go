...
import (
	...
	"cyberstrike-ai/internal/vulnintel"
...
func New(cfg *config.Config, log *logger.Logger, configPath string) (*App, error) {
	...
	router, routerErr := newHTTPRouter(cfg.Server.TrustedProxies)
	if routerErr != nil {
	...
	if dsn == "" {
		return nil, fmt.Errorf("database.dsn 必须配置 PostgreSQL 连接串")
	}
	localTZ := resolveDatabaseLocalTimezone(cfg.Database.LocalTimezone)
	db, err = database.NewPostgresDB(dsn, localTZ, log.Logger)
	if err != nil {
		return nil, fmt.Errorf("初始化数据库失败: %w", err)
	}

	intelStore, err := vulnintel.NewStore(context.Background(), db.DB)
	if err != nil {
		db.Close()
		return nil, fmt.Errorf("初始化漏洞情报存储失败: %w", err)
	}
	githubLeakStore, err := githubleak.NewStore(context.Background(), db.DB)
	if err != nil {
		db.Close()
		return nil, fmt.Errorf("初始化 GitHub 泄露监控存储失败: %w", err)
	}
	githubLeakMonitor, err := githubleak.NewService(githubLeakStore, githubLeakSettingsFromConfig(cfg.GitHubLeakMonitor), log.Logger)
	if err != nil {
		db.Close()
	...
	if cfg.Knowledge.Enabled {
		...
		// knowledge_dsn 为空则复用会话库（表名互不冲突，
		// knowledge_retrieval_logs 由会话库初始化流程统一创建）。
		knowledgeDSN := strings.TrimSpace(cfg.Database.KnowledgeDSN)
		if knowledgeDSN == "" {
			if err := db.EnsureKnowledgeTables(); err != nil {
				return nil, fmt.Errorf("初始化共享知识库表失败: %w", err)
			}
			knowledgeDB = db.DB
			log.Logger.Info("知识库数据复用会话数据库（可配置 knowledge_dsn 分离数据）")
		} else {
			var err error
			knowledgeDBConn, err = database.NewKnowledgePostgresDB(knowledgeDSN, resolveDatabaseLocalTimezone(cfg.Database.LocalTimezone), log.Logger)
			if err != nil {
				return nil, fmt.Errorf("初始化知识库数据库失败: %w", err)
			}
			knowledgeDB = knowledgeDBConn.DB
			log.Logger.Info("使用独立的 PostgreSQL 知识库数据库")
		}
	...
	terminalHandler := handler.NewTerminalHandler(log.Logger).WithAuthManager(authManager)
	if db != nil {
	...
	sshStore, sshErr := sshclient.NewStore(context.Background(), db.DB, filepath.Join(filepath.Dir(configPath), "data", "ssh-vault", "master.key"))
	if sshErr != nil {
		log.Logger.Error("SSH vault unavailable; SSH endpoints will stay disabled", zap.Error(sshErr))
	}
	app.sshHandler = handler.NewSSHHandler(sshStore, authManager, auditSvc)
	if cfg.SkillLibrary.Enabled {
		embedder, err := skilllibrary.NewLocalEmbedder(cfg.SkillLibrary.EmbeddingURL, cfg.SkillLibrary.Model, cfg.SkillLibrary.ModelRevision)
		if err != nil {
			return nil, fmt.Errorf("skill library embedding: %w", err)
		}
		store, err := skilllibrary.NewStore(context.Background(), db.DB)
		if err != nil {
			return nil, fmt.Errorf("skill library schema (pgvector required): %w", err)
		}
		...
		if strings.TrimSpace(cfg.SkillLibrary.ReferencesDir) != "" {
			log.Logger.Info("skill_library.references_dir 已停用；通用经验和指南由知识管理索引", zap.String("knowledge_base_path", cfg.Knowledge.BasePath))
		}
		// The capability catalog owns executable skills and PoC evidence. General
		// guides belong to Knowledge Management and must not be indexed twice.
		// Files inside a skill package remain attached references for that skill.
		app.skillLibrary = skilllibrary.NewService(store, embedder, []skilllibrary.Source{{Name: "skills", Path: resolve(skills), Kind: "reference"}, {Name: "pocs", Path: resolve(pocs), Kind: "poc"}}, log.Logger)
	}
...
func (a *App) RunWithContext(ctx context.Context) error {
	if a.skillLibrary != nil {
		a.skillLibrary.Start(ctx)
		defer a.skillLibrary.Close()
	}
	if a.intelligence != nil {
		a.intelligence.Start(ctx)
		defer a.intelligence.Close()
	}
...
func githubLeakSettingsFromConfig(cfg config.GitHubLeakMonitorConfig) githubleak.Settings {
	...
	if fingerprintKey == "" {
		fingerprintKey = strings.TrimSpace(os.Getenv("GITHUB_LEAK_FINGERPRINT_KEY"))
	}
	interval := cfg.IntervalSecondsEffective()
	rules := make([]githubleak.Rule, 0, len(cfg.Rules))
	for _, rule := range cfg.Rules {
		rules = append(rules, githubleak.Rule{
			Enabled: rule.Enabled, Name: rule.Name, Keywords: append([]string(nil), rule.Keywords...),
		})
	}
	return githubleak.Settings{
		Enabled:               cfg.Enabled,
		Token:                 token,
		FingerprintKey:        fingerprintKey,
		Keywords:              append([]string(nil), cfg.Keywords...),
		Rules:                 rules,
		IntervalSeconds:       githubleak.DefaultIntervalSeconds,
...
// UpdateGitHubLeakMonitorConfig applies saved monitor settings without exposing the token.
func (a *App) UpdateGitHubLeakMonitorConfig(cfg config.GitHubLeakMonitorConfig) {
	if a == nil || a.githubLeakMonitor == nil {
		return
	}
	if err := a.githubLeakMonitor.UpdateSettings(githubLeakSettingsFromConfig(cfg)); err != nil {
		a.logger.Logger.Warn("GitHub 泄露监控配置将在服务重启后生效", zap.Error(err))
	}
}
...
func setupRoutes(
	...
	if app.sshHandler != nil {
		api.GET("/ssh/connections/:id/terminal", app.sshHandler.AuthenticateSocket, security.AuthMiddleware(authManager), security.RBACMiddleware(app.db), app.sshHandler.Terminal)
	}
	protected := api.Group("")
	...
	}))
	{
		...
		protected.POST("/terminal/ws-ticket", terminalHandler.SocketTicket)
		...
		if app.intelligence != nil {
			intelHandler := handler.NewVulnerabilityIntelHandler(app.intelligence, app.logger.Logger).WithAnalysisConfig(app.config)
			protected.GET("/vulnerability-intel", intelHandler.List)
			protected.GET("/vulnerability-intel/stats", intelHandler.Stats)
			protected.GET("/vulnerability-intel/sources", intelHandler.Sources)
			protected.GET("/vulnerability-intel/subscriptions", intelHandler.Subscriptions)
			protected.POST("/vulnerability-intel/subscriptions", intelHandler.Subscribe)
			protected.DELETE("/vulnerability-intel/subscriptions/:subscriptionID", intelHandler.Unsubscribe)
			protected.GET("/vulnerability-intel/notifications", intelHandler.Notifications)
			protected.POST("/vulnerability-intel/notifications/read", intelHandler.ReadNotifications)
			protected.GET("/vulnerability-intel/:id/analysis", intelHandler.Analysis)
			protected.POST("/vulnerability-intel/:id/analysis", intelHandler.Analysis)
			protected.GET("/vulnerability-intel/:id", intelHandler.Detail)
			protected.POST("/vulnerability-intel/sync", intelHandler.Sync)
			protected.PUT("/vulnerability-intel/sources/:source", intelHandler.Enable)
		}
		if app.githubLeakMonitor != nil {
			githubLeakHandler := handler.NewGitHubLeakHandler(app.githubLeakMonitor, app.logger.Logger)
			githubLeakHandler.SetAudit(auditSvc)
			protected.GET("/github-leaks/findings", githubLeakHandler.List)
			protected.GET("/github-leaks/findings/:id", githubLeakHandler.Get)
			protected.GET("/github-leaks/stats", githubLeakHandler.Stats)
			protected.GET("/github-leaks/runtime", githubLeakHandler.Runtime)
			protected.POST("/github-leaks/run", githubLeakHandler.Run)
			protected.PATCH("/github-leaks/findings/:id/status", githubLeakHandler.UpdateStatus)
		}
		...
		if app.sshHandler != nil {
			protected.GET("/ssh/connections", app.sshHandler.List)
			protected.POST("/ssh/connections", app.sshHandler.Save)
			protected.PUT("/ssh/connections/:id", app.sshHandler.Save)
			protected.DELETE("/ssh/connections/:id", app.sshHandler.Delete)
			protected.POST("/ssh/connections/:id/probe", app.sshHandler.Probe)
			protected.POST("/ssh/connections/:id/trust", app.sshHandler.Trust)
			protected.POST("/ssh/connections/:id/test", app.sshHandler.Test)
			protected.POST("/ssh/connections/:id/terminal-ticket", app.sshHandler.Ticket)
			protected.GET("/ssh/connections/:id/files/:action", app.sshHandler.Files)
			protected.POST("/ssh/connections/:id/files/:action", app.sshHandler.Files)
		}
		...
		libraryHandler := handler.NewSkillLibraryHandler(app.skillLibrary, app.logger.Logger)
		protected.GET("/skill-library/search", libraryHandler.Search)
		protected.GET("/skill-library/status", libraryHandler.Status)
		protected.POST("/skill-library/index", libraryHandler.Index)
		protected.GET("/skill-library/documents/:id", libraryHandler.Detail)
		protected.PUT("/skill-library/documents/:id", libraryHandler.Edit)
		protected.POST("/skill-library/links", libraryHandler.Link)
		protected.DELETE("/skill-library/links", libraryHandler.Link)
...
func initializeKnowledge(
	...
	// PostgreSQL：knowledge_dsn 为空则复用会话库（表名互不冲突，
	// knowledge_retrieval_logs 由会话库初始化流程统一创建）。
	knowledgeDSN := strings.TrimSpace(cfg.Database.KnowledgeDSN)
	if knowledgeDSN == "" {
		if err := db.EnsureKnowledgeTables(); err != nil {
		...
		knowledgeDB = db.DB
		logger.Info("知识库数据复用会话数据库（可配置 knowledge_dsn 分离数据）")
	} else {
		var err error
		knowledgeDBConn, err = database.NewKnowledgePostgresDB(knowledgeDSN, resolveDatabaseLocalTimezone(cfg.Database.LocalTimezone), logger)
		if err != nil {
			return nil, fmt.Errorf("初始化知识库数据库失败: %w", err)
		}
		knowledgeDB = knowledgeDBConn.DB
		logger.Info("使用独立的 PostgreSQL 知识库数据库")
	}
...
// resolveDatabaseLocalTimezone 解析 PostgreSQL 方言使用的业务时区（IANA 名称）。
// 优先级：显式配置 > TZ 环境变量 > time.Local（仅当能取到 IANA 名）> UTC。
func resolveDatabaseLocalTimezone(configured string) string {
	...
	if name := time.Local.String(); name != "" && name != "Local" && strings.Contains(name, "/") {
		return name
	}
	return "UTC"
}
