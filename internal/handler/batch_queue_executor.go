...
func (h *AgentHandler) executeOneBatchSubTask(queueID string, queue *BatchTaskQueue, task *BatchTask) {
	...
	// 队列启动前已经确认审批策略；子会话创建后、任何工具执行前先持久化并激活。
	// 显式 off 同样写入，避免新会话回退到全局默认审批而在运行中途阻塞。
	batchHITL := normalizeBatchQueueHITL(queue.HITL)
	if batchHITL != nil {
		if h.hitlManager == nil {
			if batchHITL.Enabled {
				h.batchTaskManager.UpdateTaskStatus(queueID, task.ID, BatchTaskStatusFailed, "", "审批服务不可用，任务未启动")
				return
			}
		} else {
			if err := h.hitlManager.SaveConversationConfig(conversationID, batchHITL); err != nil {
				h.logger.Error("保存批量任务审批策略失败", zap.String("queueId", queueID), zap.String("taskId", task.ID), zap.Error(err))
				h.batchTaskManager.UpdateTaskStatus(queueID, task.ID, BatchTaskStatusFailed, "", "保存审批策略失败: "+err.Error())
				return
			}
			h.activateHITLForConversation(conversationID, batchHITL)
