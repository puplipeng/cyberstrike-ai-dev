...
func TestWebshellUpdatePreservesReplacesAndClearsSecret(t *testing.T) {
	db, user, conn, _ := setupWebshellRBACTest(t)
	conn.Password = "saved-canary"
	if err := db.UpdateWebshellConnection(conn); err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		{"missing", nil, "saved-canary"},
		{"replace", func() *string { s := "new-canary"; return &s }(), "new-canary"},
		{"clear", func() *string { s := ""; return &s }(), ""},
	} {
		body := map[string]interface{}{"url": conn.URL, "remark": tc.name}
		if tc.password != nil {
			body["password"] = *tc.password
		}
		w := performWebshellJSON(user, http.MethodPut, "/api/webshell/connections/"+conn.ID, body, func(c *gin.Context) { c.Params = gin.Params{{Key: "id", Value: conn.ID}}; h.UpdateConnection(c) })
		if w.Code != 200 {
			t.Fatalf("%s: HTTP %d", tc.name, w.Code)
		}
		stored, err := db.GetWebshellConnection(conn.ID)
		if err != nil {
			t.Fatal(err)
		}
		if stored.Password != tc.want {
			t.Fatalf("%s did not preserve expected password", tc.name)
		}
		if bytes.Contains(w.Body.Bytes(), []byte("canary")) {
			t.Fatal("write response leaked password")
		}
	}
}
