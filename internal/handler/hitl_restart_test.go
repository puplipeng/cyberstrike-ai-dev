...
func TestEnsureSchemaCancelsPendingInterruptsAfterRestart(t *testing.T) {
	...
	if err := db.QueryRow(`SELECT status, decision, decision_comment, decided_by, decided_at
		FROM hitl_interrupts WHERE id = $1`, "restart-pending").
	...
	if err := db.QueryRow(`SELECT content, updated_at FROM messages WHERE id = $1`, message.ID).
	...
	if err := db.QueryRow(`SELECT event_type, message FROM process_details WHERE message_id = $1`, message.ID).
...
func TestEnsureSchemaFinalizesOnlyHistoricalPlaceholdersWithTerminalEvidence(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO hitl_interrupts
		...
		VALUES ($1, $2, $3, 'approval', 'browser', 'timeout', 'reject', 'HITL timeout auto-reject for safety', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
	...
	if _, err := db.Exec(`INSERT INTO hitl_interrupts
		(id, conversation_id, message_id, mode, tool_name, status, decision, decision_comment, created_at, decided_at)
		VALUES ($1, $2, $3, 'approval', 'exec', 'decided', 'reject', 'user rejected', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
	...
	assertTerminal := func(messageID, wantContent, wantEvent string) {
		...
		if err := db.QueryRow(`SELECT content FROM messages WHERE id = $1`, messageID).Scan(&content); err != nil {
			t.Fatalf("query message %s: %v", messageID, err)
		}
		if content != wantContent {
			t.Fatalf("message %s content=%q, want %q", messageID, content, wantContent)
		}
		if err := db.QueryRow(`SELECT event_type FROM process_details WHERE message_id = $1
	...
	if err := db.QueryRow(`SELECT content FROM messages WHERE id = $1`, potentiallyActive.ID).Scan(&activeContent); err != nil {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM process_details WHERE message_id = $1
