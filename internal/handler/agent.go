...
func (h *AgentHandler) appendAssistantMessageNotice(messageID, notice string) error {
	...
	_, err := h.db.Exec(
		`UPDATE messages
		 SET content = CASE
			WHEN content IS NULL OR TRIM(content) = '' THEN $1
			WHEN POSITION($2 IN content) > 0 THEN content
			ELSE content || '\n\n' || $3
		 END,
		     updated_at = $4
		 WHERE id = $5`,
...
func (h *AgentHandler) mergeAssistantMessagePartialOnCancel(messageID, partial string) error {
	...
	_, err := h.db.Exec(
		`UPDATE messages
		 SET content = CASE
			WHEN content IS NULL OR TRIM(content) = '' OR TRIM(content) = '处理中...' THEN $1
			WHEN POSITION($2 IN content) > 0 THEN content
			ELSE content || '\n\n' || $3
		 END,
		     updated_at = $4
...
type BatchTaskRequest struct {
	...
	HITL         *HITLRequest `json:"hitl,omitempty"`           // 启动前固定的审批策略；显式 off 表示本队列不审批
}
...
func (h *AgentHandler) loadHistoryFromAgentTrace(conversationID string) ([]agent.ChatMessage, error) {
	...
	for _, msgMap := range messagesArray {
		...
		// 只恢复压缩后的原始用户约束账本；旧系统提示仍由本轮 Instruction 重建。
		if msg.Role == "system" {
			if modelFacingTrace {
				content, _ := msgMap["content"].(string)
				if ledger := multiagent.RestoreOriginalUserIntentLedger(content, h.config); ledger != "" {
					msg.Content = ledger
		...
		if msg.Role == "user" && modelFacingTrace {
			extra, _ := msgMap["extra"].(map[string]interface{})
			if multiagent.IsToolSearchReminder(msg.Content, extra) {
				continue
			}
		}
