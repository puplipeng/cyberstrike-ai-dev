...
WHERE id = $3 AND TRIM(content) IN ('处理中...', 'Processing...')`,
...
WHERE NOT EXISTS (
    ...
    WHERE message_id = $8 AND event_type IN ('cancelled', 'timeout', 'error')
...
func (m *HITLManager) PendingHITLInterruptMode(conversationID string) (string, bool) {
	...
	var mode string
	err := m.db.QueryRow(`SELECT mode FROM hitl_interrupts WHERE conversation_id = $1 AND status = 'pending' ORDER BY created_at DESC LIMIT 1`, conversationID).
		Scan(&mode)
	if err != nil {
...
func (m *HITLManager) LoadConversationConfig(conversationID string) (*HITLRequest, error) {
	...
	var timeout int
	err := m.db.QueryRow(`SELECT enabled, mode, COALESCE(reviewer,'human'), sensitive_tools, timeout_seconds FROM hitl_conversation_configs WHERE conversation_id = $1`, conversationID).
...
func (m *HITLManager) HasConversationConfig(conversationID string) (bool, error) {
	...
	var one int
	err := m.db.QueryRow(`SELECT 1 FROM hitl_conversation_configs WHERE conversation_id = $1 LIMIT 1`, conversationID).Scan(&one)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
	return err == nil, err
...
func (m *HITLManager) waitDecision(ctx context.Context, p *pendingInterrupt, timeout time.Duration) (hitlDecision, error) {
	...
	case d := <-p.decideCh:
		...
		_, _ = m.db.Exec(`UPDATE hitl_interrupts SET status='decided', decision=$1, decision_comment=$2, decided_at=$3, decided_by='human' WHERE id=$4`,
			d.Decision, d.Comment, time.Now(), p.InterruptID)
		return d, nil
	case <-timeoutCh:
		comment := "HITL timeout auto-reject for safety"
		_, _ = m.db.Exec(`UPDATE hitl_interrupts SET status='timeout', decision='reject', decision_comment=$1, decided_at=$2, decided_by='system' WHERE id=$3`,
			comment, time.Now(), p.InterruptID)
		return hitlDecision{Decision: "reject", Comment: comment}, nil
	case <-ctx.Done():
		_, _ = m.db.Exec(`UPDATE hitl_interrupts SET status='cancelled', decision='reject', decision_comment='task cancelled', decided_at=$1, decided_by='system' WHERE id=$2`,
...
func (h *AgentHandler) waitHITLApproval(runCtx context.Context, cancelRun context.CancelCauseFunc, conversationID, assistantMessageID, toolName, toolCallID string, payload map[string]interface{}, sendEventFunc func(eventType, message string, data interface{})) (*hitlDecision, error) {
	...
	if cfg.Reviewer == "audit_agent" {
		...
		now := time.Now()
		_, _ = h.db.Exec(`UPDATE hitl_interrupts SET status='decided', decision=$1, decision_comment=$2, decided_at=$3, decided_by='audit_agent' WHERE id=$4`,
...
func (h *AgentHandler) ListHITLPending(c *gin.Context) {
	...
	q += fmt.Sprintf(" ORDER BY created_at DESC LIMIT $%d::bigint OFFSET $%d::bigint", len(args)+1, len(args)+2)
...
func (h *AgentHandler) DismissHITLInterrupt(c *gin.Context) {
	...
	res, err := h.db.Exec(`UPDATE hitl_interrupts SET status='cancelled', decision='reject',
		...
		WHERE id=$1 AND status='pending'`, req.InterruptID)
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
