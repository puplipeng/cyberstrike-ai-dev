...
func (h *TerminalHandler) WithAuthManager(auth *security.AuthManager) *TerminalHandler {
	h.auth = auth
	return h
}

func (h *TerminalHandler) SocketTicket(c *gin.Context) {
	if h.auth == nil {
		c.Status(http.StatusServiceUnavailable)
		return
	}
	ticket, err := h.auth.IssueTerminalTicket(c.GetString(security.ContextAuthTokenKey))
	if err != nil {
