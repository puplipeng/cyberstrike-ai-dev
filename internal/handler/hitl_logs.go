...
func (h *AgentHandler) appendHitlListFilters(q string, args []interface{}, c *gin.Context) (string, []interface{}) {
	...
	if conversationID != "" {
		q += fmt.Sprintf(" AND conversation_id = $%d", len(args)+1)
		args = append(args, conversationID)
	}
	if toolName != "" {
		q += fmt.Sprintf(" AND tool_name LIKE $%d", len(args)+1)
		args = append(args, "%"+toolName+"%")
	}
	if decision != "" && decision != "all" {
		q += fmt.Sprintf(" AND decision = $%d", len(args)+1)
		args = append(args, decision)
	}
	if decidedBy != "" && decidedBy != "all" {
		q += fmt.Sprintf(" AND COALESCE(decided_by,'human') = $%d", len(args)+1)
	...
	if search != "" {
		like := "%" + search + "%"
		q += fmt.Sprintf(" AND (id LIKE $%d OR conversation_id LIKE $%d OR tool_name LIKE $%d OR payload LIKE $%d OR COALESCE(decision_comment,'') LIKE $%d)", len(args)+1, len(args)+2, len(args)+3, len(args)+4, len(args)+5)
...
func (h *AgentHandler) ListHITLLogs(c *gin.Context) {
	...
	q += fmt.Sprintf(" ORDER BY COALESCE(decided_at, created_at) DESC LIMIT $%d::bigint OFFSET $%d::bigint", len(args)+1, len(args)+2)
...
func (h *AgentHandler) GetHITLLog(c *gin.Context) {
	...
	q := `SELECT id, conversation_id, message_id, mode, tool_name, tool_call_id, payload, status, COALESCE(reviewer,'human'), decision, decision_comment, COALESCE(decided_by,'human'), created_at, decided_at FROM hitl_interrupts WHERE id = $1`
...
func (h *AgentHandler) filterAllowedHitlInterruptIDs(c *gin.Context, ids []string) ([]string, error) {
	...
	query := `SELECT id, conversation_id FROM hitl_interrupts WHERE id IN (` + buildPlaceholders(len(clean), 1) + `)`
...
func (h *AgentHandler) hitlInterruptAllowed(c *gin.Context, interruptID string) bool {
	...
	if err := h.db.QueryRow(`SELECT conversation_id FROM hitl_interrupts WHERE id = $1`, strings.TrimSpace(interruptID)).Scan(&conversationID); err != nil {
		return false
	}
