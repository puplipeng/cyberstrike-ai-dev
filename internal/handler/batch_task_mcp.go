...
【执行】默认创建后为 pending，不自动跑。execute_now=true 可创建后立即跑；否则之后调用 batch_task_start。Cron 自动下一轮需 schedule_enabled 为 true（可用 batch_task_schedule_enabled）。`,
		...
		if projectID == "" {
			// 继承当前对话的项目（用户在本项目对话中触发批量任务时，产物留在同一项目）
			if convID := mcp.MCPConversationIDFromContext(ctx); convID != "" && h.db != nil {
				if pid, err := h.db.GetConversationProjectID(convID); err == nil && pid != "" {
					projectID = pid
				}
			}
		}
		...
		queue, createErr := h.batchTaskManager.CreateBatchQueue(title, role, agentMode, scheduleMode, cronExpr, projectID, nextRunAt, concurrency, nil, tasks)
		if createErr != nil {
