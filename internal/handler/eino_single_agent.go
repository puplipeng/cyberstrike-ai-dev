...
// Keep the tracker on the outer task context, before deriving per-segment run
// contexts. Rebinding cancellation after automatic continuation or an interrupt
// then retains the same budget instead of granting each Runner a fresh limit.
func newEinoTaskBaseContext(parent context.Context, cfg config.AgentConfig) (context.Context, context.CancelCauseFunc) {
	return context.WithCancelCause(modelbudget.WithContext(parent, cfg.MaxTaskTokensEffective()))
}
...
func (h *AgentHandler) EinoSingleAgentLoopStream(c *gin.Context) {
	...
	baseCtx, cancelWithCause = newEinoTaskBaseContext(detachedAgentContext(c.Request.Context()), runCfg.Agent)
	...
	if _, err := h.tasks.StartTask(conversationID, req.Message, cancelWithCause); err != nil {
		...
		if assistantMessageID != "" {
			_, _ = h.db.Exec("UPDATE messages SET content = $1, updated_at = $2 WHERE id = $3", errorMsg, time.Now(), assistantMessageID)
		}
		sendEvent("done", "", map[string]interface{}{"conversationId": conversationID})
		timeoutCancel()
		return
	...
	for {
		...
		if errors.Is(runErr, context.DeadlineExceeded) || errors.Is(context.Cause(taskCtx), context.DeadlineExceeded) {
			...
			if assistantMessageID != "" {
				_, _ = h.db.Exec("UPDATE messages SET content = $1, updated_at = $2 WHERE id = $3", timeoutMsg, time.Now(), assistantMessageID)
...
func (h *AgentHandler) EinoSingleAgentLoop(c *gin.Context) {
	...
	baseCtx, cancelWithCause := newEinoTaskBaseContext(c.Request.Context(), runCfg.Agent)
