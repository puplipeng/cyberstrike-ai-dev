...
func (h *AgentHandler) recordHitlToolExecutionResult(conversationID, toolCallID, toolName string, success bool, result string) {
	...
	err := h.db.QueryRow(`SELECT payload FROM hitl_interrupts WHERE id = $1`, interruptID).Scan(&payloadJSON)
	if err != nil {
		return
	}
	...
	_, _ = h.db.Exec(`UPDATE hitl_interrupts SET payload = $1 WHERE id = $2`, merged, interruptID)
}
