...
func (h *ChatUploadsHandler) conversationArtifactVirtualPathAllowed(c *gin.Context, relativePath string) bool {
	rel, err := canonicalConversationArtifactPath(relativePath)
	if err != nil {
		return false
	}
	parts := strings.Split(filepath.ToSlash(rel), "/")
...
// Authorization and filesystem resolution must use exactly the same path.
// Reject parent segments instead of silently changing the authorized owner.
func canonicalConversationArtifactPath(relativePath string) (string, error) {
	raw := strings.TrimSpace(relativePath)
	if !strings.HasPrefix(raw, artifactVirtualPrefix) {
		return "", fmt.Errorf("invalid artifact path")
	}
	raw = strings.TrimPrefix(raw, artifactVirtualPrefix)
	if strings.ContainsAny(raw, "\\\x00") {
		return "", fmt.Errorf("invalid artifact path separator")
	}
