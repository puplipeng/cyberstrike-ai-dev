...
// GitHubLeakMonitorUpdater applies monitor configuration without restarting the app.
type GitHubLeakMonitorUpdater interface {
	UpdateGitHubLeakMonitorConfig(config.GitHubLeakMonitorConfig)
}
...
type ConfigHandler struct {
	...
	githubLeakMonitorUpdater   GitHubLeakMonitorUpdater
	audit                      *audit.Service
	db                         *database.DB
	logger                     *zap.Logger
	mu                         sync.RWMutex
...
func (h *ConfigHandler) SetGitHubLeakMonitorUpdater(updater GitHubLeakMonitorUpdater) {
	h.githubLeakMonitorUpdater = updater
}
...
type GetConfigResponse struct {
	...
	GitHubLeakMonitor GitHubLeakMonitorPublic  `json:"github_leak_monitor"`
...
type GitHubLeakMonitorPublic struct {
	Enabled                  bool                          `json:"enabled"`
	TokenConfigured          bool                          `json:"token_configured"`
	FingerprintKeyConfigured bool                          `json:"fingerprint_key_configured"`
	Keywords                 []string                      `json:"keywords"`
	Rules                    []config.GitHubLeakRuleConfig `json:"rules"`
...
func (h *ConfigHandler) GetConfig(c *gin.Context) {
	...
	multiPub := config.MultiAgentPublic{
		...
		ToolSearchEnable:                           h.config.MultiAgent.EinoMiddleware.ToolSearchEnable,
		ReductionEnable:                            h.config.MultiAgent.EinoMiddleware.ReductionEnable,
		ToolSearchAlwaysVisibleEffectiveTools: mergeToolNameLists(
			h.config.MultiAgent.EinoMiddleware.ToolSearchAlwaysVisibleTools,
			multiagent.DefaultAlwaysVisibleToolNames(),
		),
	}

	githubLeakKeywords := append([]string(nil), h.config.GitHubLeakMonitor.Keywords...)
	if len(h.config.GitHubLeakMonitor.Rules) > 0 {
		githubLeakKeywords = []string{}
	}
	public, err := config.RedactedCopy(GetConfigResponse{
		AI:      h.config.AI,
		...
		GitHubLeakMonitor: GitHubLeakMonitorPublic{
			Enabled:                  h.config.GitHubLeakMonitor.Enabled,
			TokenConfigured:          strings.TrimSpace(h.config.GitHubLeakMonitor.Token) != "" || strings.TrimSpace(os.Getenv("GITHUB_TOKEN")) != "",
			FingerprintKeyConfigured: strings.TrimSpace(h.config.GitHubLeakMonitor.FingerprintKey) != "" || strings.TrimSpace(os.Getenv("GITHUB_LEAK_FINGERPRINT_KEY")) != "",
			Keywords:                 githubLeakKeywords,
			Rules:                    cloneGitHubLeakRules(h.config.GitHubLeakMonitor.Rules),
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "cannot prepare public configuration"})
		return
	}
...
type UpdateConfigRequest struct {
	...
	GitHubLeakMonitor *config.GitHubLeakMonitorConfig `json:"github_leak_monitor,omitempty"`
...
func normalizeGitHubLeakMonitorConfig(in config.GitHubLeakMonitorConfig) (config.GitHubLeakMonitorConfig, error) {
	if in.IntervalSeconds == 0 {
		in.IntervalSeconds = config.DefaultGitHubLeakIntervalSeconds
	}
	if in.IntervalSeconds < config.MinGitHubLeakIntervalSeconds {
		return in, fmt.Errorf("github_leak_monitor.interval_seconds 必须大于 30 秒")
	}
	if in.IntervalSeconds > config.MaxGitHubLeakIntervalSeconds {
		return in, fmt.Errorf("github_leak_monitor.interval_seconds 不能超过 %d 秒", config.MaxGitHubLeakIntervalSeconds)
	}
	if in.RequestTimeoutSeconds == 0 {
		in.RequestTimeoutSeconds = config.DefaultGitHubLeakTimeoutSeconds
	}
	if in.RequestTimeoutSeconds < 30 || in.RequestTimeoutSeconds > 120 {
		return in, fmt.Errorf("github_leak_monitor.request_timeout_seconds 必须在 30 到 120 秒之间")
	}
	if in.PerPage == 0 {
		in.PerPage = config.DefaultGitHubLeakPerPage
	}
	...
	if len(in.Rules) == 0 {
		...
		if err != nil {
			return in, fmt.Errorf("github_leak_monitor.keywords: %w", err)
		}
		in.Keywords, legacyQuery = legacyKeywords, query
	} else {
		// Named rules replace the deprecated single-rule field. Persisting an
		// empty legacy field avoids an invisible extra query in future clients.
		in.Keywords = nil
	}
	...
	if totalRules > config.MaxGitHubLeakRules {
		return in, fmt.Errorf("github_leak_monitor 最多允许 %d 条规则（含 legacy）", config.MaxGitHubLeakRules)
	}
	seenNames := make(map[string]struct{}, totalRules)
	seenQueries := make(map[string]string, totalRules)
	if legacyQuery != "" {
		seenNames["legacy"] = struct{}{}
		seenQueries[strings.ToLower(legacyQuery)] = "legacy"
	}
	for i := range in.Rules {
		rule := in.Rules[i]
		name, nameErr := normalizeGitHubLeakRuleName(rule.Name)
		if nameErr != nil {
			return in, fmt.Errorf("github_leak_monitor.rules[%d].name: %w", i, nameErr)
		}
		foldedName := strings.ToLower(name)
		if _, exists := seenNames[foldedName]; exists {
			return in, fmt.Errorf("GitHub 监控规则名称 %q 重复", name)
		}
		seenNames[foldedName] = struct{}{}
		keywords, query, keywordErr := normalizeGitHubLeakKeywords(rule.Keywords)
		if keywordErr != nil {
			return in, fmt.Errorf("github_leak_monitor.rules[%d].keywords: %w", i, keywordErr)
		}
		if len(keywords) == 0 {
			return in, fmt.Errorf("github_leak_monitor.rules[%d].keywords 不能为空", i)
		}
		queryKey := strings.ToLower(query)
		if previous, exists := seenQueries[queryKey]; exists {
			return in, fmt.Errorf("GitHub 监控规则 %q 与 %q 的规范查询重复", previous, name)
		}
		seenQueries[queryKey] = name
		rule.Name, rule.Keywords = name, keywords
		in.Rules[i] = rule
	}
	if in.Enabled {
		if len(in.Rules) == 0 {
			if len(in.Keywords) == 0 {
				return in, errors.New("github_leak_monitor.enabled=true 时至少需要一条 legacy 或具名规则")
			}
		} else {
			enabledRules := 0
			...
			if enabledRules == 0 {
				return in, errors.New("github_leak_monitor.enabled=true 时至少需要一条启用的具名规则")
			}
		}
	}
	return in, nil
...
func normalizeGitHubLeakRuleName(value string) (string, error) {
	value = strings.TrimSpace(value)
	if !utf8.ValidString(value) || len(value) < 1 || len(value) > config.MaxGitHubLeakRuleNameBytes ||
		strings.IndexFunc(value, func(r rune) bool { return r < 0x20 || r == 0x7f || r == '\u2028' || r == '\u2029' }) >= 0 {
		return "", fmt.Errorf("必须为 1 到 %d 字节的单行 UTF-8 文本", config.MaxGitHubLeakRuleNameBytes)
	}
	return value, nil
}

func normalizeGitHubLeakKeywords(values []string) ([]string, string, error) {
	if len(values) > config.MaxGitHubLeakKeywords {
		return nil, "", fmt.Errorf("AND 规则最多允许 %d 个同时命中词", config.MaxGitHubLeakKeywords)
	}
	...
	for _, raw := range values {
		...
		if !utf8.ValidString(keyword) || len(keyword) < 2 || len(keyword) > config.MaxGitHubLeakKeywordBytes ||
			strings.IndexFunc(keyword, func(r rune) bool { return r < 0x20 || r == 0x7f || r == '\u2028' || r == '\u2029' }) >= 0 {
			return nil, "", fmt.Errorf("同时命中词必须为 2 到 %d 字节的单行 UTF-8 文本", config.MaxGitHubLeakKeywordBytes)
		}
		folded := strings.ToLower(keyword)
		current, exists := byFolded[folded]
		if !exists || keyword == folded || (current != folded && keyword < current) {
			byFolded[folded] = keyword
		}
	}
	keywords := make([]string, 0, len(byFolded))
	for _, keyword := range byFolded {
		keywords = append(keywords, keyword)
	}
	sort.Slice(keywords, func(i, j int) bool {
		left, right := strings.ToLower(keywords[i]), strings.ToLower(keywords[j])
		if left == right {
	...
	for _, keyword := range keywords {
		...
		literalBytes += len(escaped) + 2
		terms = append(terms, `"`+escaped+`"`)
	}
	if literalBytes > config.MaxGitHubLeakQueryLiteralBytes {
		return nil, "", fmt.Errorf("AND 规则的字面量合计不能超过 %d 字节", config.MaxGitHubLeakQueryLiteralBytes)
	}
	query := ""
	if len(terms) > 0 {
		query = strings.Join(terms, " AND ") + " in:file"
	}
	return keywords, query, nil
}

func cloneGitHubLeakRules(in []config.GitHubLeakRuleConfig) []config.GitHubLeakRuleConfig {
	out := make([]config.GitHubLeakRuleConfig, len(in))
	for i, rule := range in {
		out[i] = rule
		out[i].Keywords = append([]string(nil), rule.Keywords...)
	}
	return out
}
...
type AgentConfigUpdate struct {
	...
	MaxTaskTokens                      *int    `json:"max_task_tokens,omitempty"`
...
func applyAgentConfigUpdate(dst *config.AgentConfig, src *AgentConfigUpdate) {
	...
	if src.MaxTaskTokens != nil {
		dst.MaxTaskTokens = *src.MaxTaskTokens
	}
...
func (h *ConfigHandler) UpdateConfig(c *gin.Context) {
	...
	if req.GitHubLeakMonitor != nil {
		normalized, err := normalizeGitHubLeakMonitorConfig(*req.GitHubLeakMonitor)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		req.GitHubLeakMonitor = &normalized
	}
	...
	for _, section := range []struct {
		...
		{"ai", req.AI, &h.config.AI}, {"openai", req.OpenAI, &h.config.OpenAI},
		{"vision", req.Vision, &h.config.Vision}, {"fofa", req.FOFA, &h.config.FOFA},
		{"zoomeye", req.ZoomEye, &h.config.ZoomEye}, {"quake", req.Quake, &h.config.Quake},
		{"shodan", req.Shodan, &h.config.Shodan}, {"github_leak_monitor", req.GitHubLeakMonitor, &h.config.GitHubLeakMonitor},
		{"mcp", req.MCP, &h.config.MCP},
		{"hitl", req.Hitl, &h.config.Hitl}, {"knowledge", req.Knowledge, &h.config.Knowledge},
		{"robots", req.Robots, &h.config.Robots},
	} {
		if err := config.MergeSecretUpdates(section.next, section.previous, raw[section.name]); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
	}
	if req.GitHubLeakMonitor != nil {
		if err := req.GitHubLeakMonitor.ValidateFingerprintKey(); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		if req.GitHubLeakMonitor.Enabled && strings.TrimSpace(req.GitHubLeakMonitor.Token) == "" && strings.TrimSpace(os.Getenv("GITHUB_TOKEN")) == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "github_leak_monitor.enabled=true 时必须配置 GitHub token"})
			return
		}
	}
	...
	if req.GitHubLeakMonitor != nil {
		h.config.GitHubLeakMonitor = *req.GitHubLeakMonitor
		ruleCount := len(h.config.GitHubLeakMonitor.Rules)
		enabledRules := 0
		for _, rule := range h.config.GitHubLeakMonitor.Rules {
			if rule.Enabled {
				enabledRules++
			}
		}
		if ruleCount == 0 && len(h.config.GitHubLeakMonitor.Keywords) > 0 {
			ruleCount, enabledRules = 1, 1
		}
		h.logger.Info("更新 GitHub 泄露监控配置",
			zap.Bool("enabled", h.config.GitHubLeakMonitor.Enabled),
			zap.Int("rules", ruleCount),
			zap.Int("enabled_rules", enabledRules),
			zap.Int("interval_seconds", h.config.GitHubLeakMonitor.IntervalSecondsEffective()),
			zap.Bool("token_configured", strings.TrimSpace(h.config.GitHubLeakMonitor.Token) != "" || strings.TrimSpace(os.Getenv("GITHUB_TOKEN")) != ""),
		)
	}
	...
	if req.MultiAgent != nil {
		...
		if req.MultiAgent.ToolSearchEnable != nil {
			h.config.MultiAgent.EinoMiddleware.ToolSearchEnable = *req.MultiAgent.ToolSearchEnable
		}
		if req.MultiAgent.ReductionEnable != nil {
			h.config.MultiAgent.EinoMiddleware.ReductionEnable = *req.MultiAgent.ReductionEnable
		}
	...
	if req.GitHubLeakMonitor != nil && h.githubLeakMonitorUpdater != nil {
		h.githubLeakMonitorUpdater.UpdateGitHubLeakMonitorConfig(h.config.GitHubLeakMonitor)
	}
...
func (h *ConfigHandler) TestOpenAI(c *gin.Context) {
	...
	if codexbridge.IsProvider(req.Provider) {
		h.testCodexAccount(c, req)
		return
	}
	key, keyErr := h.resolveTestAPIKey(req.KeyRef, req.APIKey, req.Provider, req.BaseURL)
	if keyErr != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": keyErr.Error()})
		return
	}
	req.APIKey = key
...
func (h *ConfigHandler) ListModels(c *gin.Context) {
	...
	if codexbridge.IsProvider(provider) {
		...
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				...
				"supported":     true,
				"error":         err.Error(),
				"output_budget": codexbridge.OutputBudgetSupport(),
			})
			return
		}
		models := make([]string, 0, len(status.Models))
		seen := make(map[string]struct{}, len(status.Models))
	...
	key, keyErr := h.resolveTestAPIKey(req.KeyRef, req.APIKey, provider, req.BaseURL)
	if keyErr != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": keyErr.Error()})
		return
	}
...
func (h *ConfigHandler) TestVision(c *gin.Context) {
	...
	key, keyErr := h.resolveTestAPIKey(req.KeyRef, oa.APIKey, oa.Provider, oa.BaseURL)
	if keyErr != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": keyErr.Error()})
		return
	}
...
func (h *ConfigHandler) saveConfig() error {
	...
	updateGitHubLeakMonitorConfig(root, h.config.GitHubLeakMonitor)
...
func updateAgentConfig(doc *yaml.Node, agent config.AgentConfig) {
	...
	setIntInMap(agentNode, "max_task_tokens", agent.MaxTaskTokens)
...
func updateGitHubLeakMonitorConfig(doc *yaml.Node, cfg config.GitHubLeakMonitorConfig) {
	root := doc.Content[0]
	node := ensureMap(root, "github_leak_monitor")
	setBoolInMap(node, "enabled", cfg.Enabled)
	setStringInMap(node, "token", cfg.Token)
	setStringInMap(node, "fingerprint_key", cfg.FingerprintKey)
	keywords := cfg.Keywords
	if len(cfg.Rules) > 0 {
		keywords = nil
	}
	setStringSliceInMap(node, "keywords", keywords)
	setGitHubLeakRulesInMap(node, "rules", cfg.Rules)
...
func setGitHubLeakRulesInMap(mapNode *yaml.Node, key string, rules []config.GitHubLeakRuleConfig) {
	...
	for _, rule := range rules {
		ruleNode := &yaml.Node{Kind: yaml.MappingNode, Tag: "!!map"}
		setBoolInMap(ruleNode, "enabled", rule.Enabled)
		setStringInMap(ruleNode, "name", rule.Name)
		setStringSliceInMap(ruleNode, "keywords", rule.Keywords)
		valueNode.Content = append(valueNode.Content, ruleNode)
	}
}
...
func updateKnowledgeConfig(doc *yaml.Node, cfg config.KnowledgeConfig) {
	...
	setBoolInMap(mqNode, "enabled", cfg.Retrieval.MultiQuery.Enabled)
...
func updateMultiAgentConfig(doc *yaml.Node, cfg config.MultiAgentConfig) {
	...
	setBoolInMap(mwNode, "tool_search_enable", cfg.EinoMiddleware.ToolSearchEnable)
	setBoolInMap(mwNode, "reduction_enable", cfg.EinoMiddleware.ReductionEnable)
}
