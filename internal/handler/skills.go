...
func (h *SkillsHandler) GetSkill(c *gin.Context) {
	...
	if !isValidSkillName(skillName) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "无效的 skill 目录名"})
		return
	}
...
func (h *SkillsHandler) CreateSkill(c *gin.Context) {
	...
	if err := os.MkdirAll(h.skillsRootAbs(), 0755); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	root, err := os.OpenRoot(h.skillsRootAbs())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	defer root.Close()
	if _, err := root.Lstat(req.Name); err == nil || !os.IsNotExist(err) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "skill已存在或目录不可用"})
		return
	}
	if err := root.Mkdir(req.Name, 0755); err != nil {
	...
	if err := root.WriteFile(filepath.Join(req.Name, "SKILL.md"), skillMD, 0644); err != nil {
...
func (h *SkillsHandler) UpdateSkill(c *gin.Context) {
	...
	raw, err := skillpackage.ReadPackageFile(h.skillsRootAbs(), skillName, "SKILL.md", 0)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "skill不存在: " + err.Error()})
		return
	}
...
func (h *SkillsHandler) DeleteSkill(c *gin.Context) {
	...
	defer root.Close()
	info, err := root.Lstat(skillName)
	if os.IsNotExist(err) {
		c.JSON(http.StatusNotFound, gin.H{"error": "skill不存在"})
		return
	}
	if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "skill必须是有效的本地目录"})
		return
	}
	if err := root.RemoveAll(skillName); err != nil {
	...
	// Update bindings only after path validation and successful disk removal.
