...
func appendConversationAccessSQL(query string, args []interface{}, column string, access database.RBACListAccess) (string, []interface{}) {
	...
	query += ` AND ` + column + ` IS NOT NULL AND ` + column + ` <> '' AND (
		EXISTS (SELECT 1 FROM conversations c WHERE c.id = ` + column + ` AND c.owner_user_id = $1)
		OR EXISTS (
			SELECT 1 FROM rbac_resource_assignments ra
			WHERE ra.user_id = $2 AND ra.resource_type = 'conversation' AND ra.resource_id = ` + column + `
		)
		OR EXISTS (
			SELECT 1 FROM conversations c
			JOIN projects p ON p.id = c.project_id
			WHERE c.id = ` + column + ` AND p.owner_user_id = $1
		)
		OR EXISTS (
			SELECT 1 FROM conversations c
			JOIN rbac_resource_assignments pra ON pra.resource_id = c.project_id
			WHERE c.id = ` + column + ` AND pra.user_id = $1 AND pra.resource_type = 'project'
		)
	)`
...
func appendVulnerabilityNotificationAccessSQL(query string, args []interface{}, access database.RBACListAccess) (string, []interface{}) {
	...
	query += ` AND (
		...
		OR EXISTS (
			SELECT 1 FROM rbac_resource_assignments ra
			WHERE ra.user_id = $2 AND ra.resource_type = 'vulnerability' AND ra.resource_id = vulnerabilities.id
		)
		OR (
			project_id IS NOT NULL AND project_id <> '' AND (
				EXISTS (SELECT 1 FROM projects p WHERE p.id = vulnerabilities.project_id AND p.owner_user_id = $3)
				OR EXISTS (
					SELECT 1 FROM rbac_resource_assignments pra
					WHERE pra.user_id = $4 AND pra.resource_type = 'project' AND pra.resource_id = vulnerabilities.project_id
				)
			)
		)
		OR (
			conversation_id IS NOT NULL AND conversation_id <> '' AND (
				EXISTS (SELECT 1 FROM conversations c WHERE c.id = vulnerabilities.conversation_id AND c.owner_user_id = $5)
				OR EXISTS (
					SELECT 1 FROM rbac_resource_assignments cra
					WHERE cra.user_id = $6 AND cra.resource_type = 'conversation' AND cra.resource_id = vulnerabilities.conversation_id
				)
			)
		)
	)`
...
func (h *NotificationHandler) loadPendingHITLItems(limit int, english bool, access database.RBACListAccess) ([]NotificationSummaryItem, error) {
	query := `
		SELECT
			...
			conversation_id,
			tool_name,
			COALESCE(EXTRACT(EPOCH FROM created_at::timestamp)::bigint, 0)
...
func (h *NotificationHandler) loadVulnerabilityItems(sinceMs int64, limit int, english bool, access database.RBACListAccess) ([]NotificationSummaryItem, map[string]int, error) {
	...
	query := `
		...
		WHERE EXTRACT(EPOCH FROM created_at::timestamp)::bigint > $1
	`
...
func (h *NotificationHandler) loadFailedExecutionItems(sinceMs int64, limit int, english bool) ([]NotificationSummaryItem, int, error) {
	...
	rows, err := h.db.Query(`
		SELECT
			id,
			tool_name,
			COALESCE(EXTRACT(EPOCH FROM start_time::timestamp)::bigint, 0)
		FROM tool_executions
		WHERE status = 'failed'
		  AND EXTRACT(EPOCH FROM start_time::timestamp)::bigint > $1
...
func buildPlaceholders(n, start int) string {
	if n <= 0 {
		return ""
	}
	out := make([]string, 0, n)
...
func (h *NotificationHandler) readStatesByIDs(userID string, ids []string) (map[string]bool, error) {
	...
	holders := buildPlaceholders(len(ids), 2)
	query := "SELECT event_id FROM notification_reads_by_user WHERE user_id = $1 AND event_id IN (" + holders + ")"
...
func pruneNotificationReads(db *database.DB, userID string, maxRows int) error {
	...
	_, err := db.Exec(`
		...
		WHERE user_id = $1 AND event_id NOT IN (
			...
			WHERE user_id = $2
			ORDER BY read_at DESC, ctid DESC
			LIMIT $3::bigint
		)
