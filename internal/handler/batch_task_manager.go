...
// normalizeBatchQueueHITL 将创建前确认的审批策略固化在队列上。
// nil 表示旧调用方未配置，保持历史行为（批量任务不启用审批）；显式 off 会被保留。
func normalizeBatchQueueHITL(req *HITLRequest) *HITLRequest {
	if req == nil {
		return nil
	}
	mode := normalizeHitlMode(req.Mode)
	enabled := req.Enabled && mode != "off"
	if !enabled {
		mode = "off"
	}
	...
	seen := make(map[string]struct{}, len(req.SensitiveTools))
	tools := make([]string, 0, len(req.SensitiveTools))
	for _, tool := range req.SensitiveTools {
...
func encodeBatchQueueHITL(req *HITLRequest) string {
	if req == nil {
		return "{}"
	}
...
func decodeBatchQueueHITL(raw string) *HITLRequest {
	...
	return normalizeBatchQueueHITL(&req)
}
...
func (m *BatchTaskManager) CreateBatchQueue(
	...
	queue := &BatchTaskQueue{
		...
		HITL:            normalizeBatchQueueHITL(hitl),
