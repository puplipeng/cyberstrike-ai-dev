...
func (idx *Indexer) IndexItem(ctx context.Context, itemID string) error {
	...
	err := idx.db.QueryRow("SELECT content, category, title, file_path FROM knowledge_base_items WHERE id = $1", itemID).Scan(&content, &category, &title, &filePath)
	if err != nil {
		return fmt.Errorf("获取知识项失败：%w", err)
	}

	if _, err := idx.db.Exec("DELETE FROM knowledge_embeddings WHERE item_id = $1", itemID); err != nil {
		return fmt.Errorf("删除旧向量失败：%w", err)
	}

	body := knowledgeIndexBody(content, category, title)
...
// knowledgeIndexBody keeps even a deliberately empty Markdown item visible in
// index progress and title/category retrieval. The fallback contains no invented
// knowledge; it only indexes metadata already stored for the item.
func knowledgeIndexBody(content, category, title string) string {
