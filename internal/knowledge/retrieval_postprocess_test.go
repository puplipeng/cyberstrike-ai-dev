...
// Keep token-budget tests offline: one fixture token per byte, no tokenizer
// vocabulary download and no mutation of the production model encodings.
func installKnowledgeByteTokenizer(t *testing.T) string {
	t.Helper()
	ranks := make(map[string]int, 256)
	for b := 0; b < 256; b++ {
		ranks[string([]byte{byte(b)})] = b
	}
	core, err := tiktoken.NewCoreBPE(ranks, map[string]int{"<|fixture_end|>": 256}, `(?s).`)
	if err != nil {
		t.Fatal(err)
	}
	key := "test-byte-tokenizer-" + t.Name()
	enc := tiktoken.NewTiktoken(core, &tiktoken.Encoding{}, map[string]any{})
	tiktokenEncMu.Lock()
	tiktokenEncCache[key] = enc
	tiktokenEncMu.Unlock()
	t.Cleanup(func() {
		tiktokenEncMu.Lock()
		delete(tiktokenEncCache, key)
		tiktokenEncMu.Unlock()
	})
	return key
}

func TestApplyPostRetrieveEnforcesTokenBudgetOffline(t *testing.T) {
	model := installKnowledgeByteTokenizer(t)
	input := []*schema.Document{doc("1", "ab", 0.9), doc("2", "cd", 0.8)}
	out, err := ApplyPostRetrieve(input, &config.PostRetrieveConfig{MaxContextTokens: 3}, model, 5)
	if err != nil || len(out) != 1 || out[0].ID != "1" {
		t.Fatalf("token budget ignored: docs=%v err=%v", out, err)
	}
}
