...
// knowledgePipelineRetriever applies reranking and the same output budget to
// direct-vector, optional query-rewrite, and rewrite-fallback results.
...
func (p *knowledgePipelineRetriever) Retrieve(ctx context.Context, query string, opts ...retriever.Option) (out []*schema.Document, err error) {
	...
	finalTopK := 5
	if p.base.config != nil && p.base.config.TopK > 0 {
	...
	tokenModel := ""
	if p.base.wireOpenAI != nil {
		tokenModel = p.base.wireOpenAI.Model
	} else if p.base.embedder != nil {
