...
func (m *Manager) ScanKnowledgeBase() ([]string, error) {
	...
	err := filepath.WalkDir(m.basePath, func(path string, d fs.DirEntry, err error) error {
		...
		err = m.db.QueryRow(
			"SELECT id, content, updated_at FROM knowledge_base_items WHERE file_path = $1",
			path,
		...
		if err == sql.ErrNoRows {
			...
			now := time.Now()
			_, err = m.db.Exec(
				"INSERT INTO knowledge_base_items (id, category, title, file_path, content, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
			...
			if contentChanged {
				// 更新现有项
				_, err = m.db.Exec(
					"UPDATE knowledge_base_items SET category = $1, title = $2, content = $3, updated_at = $4 WHERE id = $5",
...
func (m *Manager) GetItemsWithOptions(category string, limit, offset int, includeContent bool) ([]*KnowledgeItem, error) {
	...
	if limit > 0 {
		query += " LIMIT $2::bigint"
		args = append(args, limit)
		if offset > 0 {
			query += " OFFSET $3::bigint"
...
func (m *Manager) GetItemsCount(category string) (int, error) {
	...
	if category != "" {
		err = m.db.QueryRow("SELECT COUNT(*) FROM knowledge_base_items WHERE category = $1", category).Scan(&count)
	} else {
...
func (m *Manager) SearchItemsByKeyword(keyword string, category string) ([]*KnowledgeItemSummary, error) {
	...
	query = `
		...
		FROM knowledge_base_items 
		WHERE (LOWER(title) ILIKE LOWER($1) OR LOWER(category) ILIKE LOWER($2) OR LOWER(file_path) ILIKE LOWER($3) OR LOWER(content) ILIKE LOWER($4))
	`
...
func (m *Manager) GetItem(id string) (*KnowledgeItem, error) {
	...
	var createdAt, updatedAt string
	err := m.db.QueryRow(
		"SELECT id, category, title, file_path, content, created_at, updated_at FROM knowledge_base_items WHERE id = $1",
		id,
...
func (m *Manager) UpdateItem(id, category, title, content string) (*KnowledgeItem, error) {
	...
	// 更新数据库
	_, err = m.db.Exec(
		"UPDATE knowledge_base_items SET category = $1, title = $2, file_path = $3, content = $4, updated_at = $5 WHERE id = $6",
	...
	// 删除旧的向量嵌入（需要重新索引）
	_, err = m.db.Exec("DELETE FROM knowledge_embeddings WHERE item_id = $1", id)
	if err != nil {
...
func (m *Manager) DeleteItem(id string) error {
	// 获取文件路径
	var filePath string
	err := m.db.QueryRow("SELECT file_path FROM knowledge_base_items WHERE id = $1", id).Scan(&filePath)
	if err != nil {
		return fmt.Errorf("查询知识项失败: %w", err)
	}
	...
	// 删除数据库记录（级联删除向量）
	_, err = m.db.Exec("DELETE FROM knowledge_base_items WHERE id = $1", id)
	if err != nil {
...
func (m *Manager) LogRetrieval(conversationID, messageID, query, riskType string, retrievedItems []string) error {
	...
	_, err := m.db.Exec(
		"INSERT INTO knowledge_retrieval_logs (id, conversation_id, message_id, query, risk_type, retrieved_items, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
...
func (m *Manager) GetRetrievalLogs(conversationID, messageID string, limit int) ([]*RetrievalLog, error) {
	...
	if messageID != "" {
		rows, err = m.db.Query(
			"SELECT id, conversation_id, message_id, query, risk_type, retrieved_items, created_at FROM knowledge_retrieval_logs WHERE message_id = $1 ORDER BY created_at DESC LIMIT $2::bigint",
			messageID, limit,
		)
	} else if conversationID != "" {
		rows, err = m.db.Query(
			"SELECT id, conversation_id, message_id, query, risk_type, retrieved_items, created_at FROM knowledge_retrieval_logs WHERE conversation_id = $1 ORDER BY created_at DESC LIMIT $2::bigint",
			conversationID, limit,
		)
	} else {
		rows, err = m.db.Query(
			"SELECT id, conversation_id, message_id, query, risk_type, retrieved_items, created_at FROM knowledge_retrieval_logs ORDER BY created_at DESC LIMIT $1::bigint",
			limit,
		)
	}
	...
	for rows.Next() {
		...
		// conversation_id / message_id / query / risk_type 在 PG 中可能为 NULL（如系统触发的检索无会话上下文）
		var convID, msgID, queryNS, riskType sql.NullString
		if err := rows.Scan(&log.ID, &convID, &msgID, &queryNS, &riskType, &itemsJSON, &createdAt); err != nil {
			return nil, fmt.Errorf("扫描检索日志失败: %w", err)
		}
		log.ConversationID = convID.String
		log.MessageID = msgID.String
		log.Query = queryNS.String
		log.RiskType = riskType.String
...
func (m *Manager) DeleteRetrievalLog(id string) error {
	result, err := m.db.Exec("DELETE FROM knowledge_retrieval_logs WHERE id = $1", id)
	if err != nil {
