...
func TestRobotBindingCodeExpiryAndOwnerScopedRevocation(t *testing.T) {
	...
	now := time.Now()
	if _, err := db.Exec(`INSERT INTO robot_binding_codes (code_hash, rbac_user_id, expires_at, created_at) VALUES ($1, $2, $3, $4)`, "expired-hash", u1.ID, now.Add(-time.Minute), now.Add(-2*time.Minute)); err != nil {
		t.Fatal(err)
	}
