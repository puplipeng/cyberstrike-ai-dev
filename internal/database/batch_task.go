...
func (db *DB) CreateBatchQueue(
	...
	for _, task := range tasks {
		...
		_, err = tx.Exec(
			"INSERT INTO batch_tasks (id, queue_id, message, status) VALUES ($1, $2, $3, $4)",
...
func (db *DB) ListBatchQueuesForAccess(limit, offset int, status, keyword, userID, scope string) ([]*BatchTaskQueueRow, error) {
	...
	// 关键字搜索（搜索队列ID和标题）
	if keyword != "" {
		query += fmt.Sprintf(" AND (id LIKE $%d OR title LIKE $%d)", len(args)+1, len(args)+2)
	...
	if userID != "" && scope != RBACScopeAll {
		query += fmt.Sprintf(` AND (
			...
			OR EXISTS (
				...
				WHERE ra.user_id = $%d AND ra.resource_type = 'batch_task' AND ra.resource_id = batch_task_queues.id
			)
			OR (
				project_id IS NOT NULL AND project_id <> '' AND (
					EXISTS (SELECT 1 FROM projects p WHERE p.id = batch_task_queues.project_id AND p.owner_user_id = $%d)
					OR EXISTS (
						SELECT 1 FROM rbac_resource_assignments pra
						WHERE pra.user_id = $%d AND pra.resource_type = 'project' AND pra.resource_id = batch_task_queues.project_id
					)
				)
			)
	...
	query += fmt.Sprintf(" ORDER BY created_at DESC LIMIT $%d::bigint OFFSET $%d::bigint", len(args)+1, len(args)+2)
...
func (db *DB) GetBatchTasks(queueID string) ([]*BatchTaskRow, error) {
	rows, err := db.Query(
		"SELECT id, queue_id, message, conversation_id, status, started_at, completed_at, error, result FROM batch_tasks WHERE queue_id = $1 ORDER BY ctid ASC",
		queueID,
	)
	if err != nil {
...
func (db *DB) UpdateBatchQueueStatus(queueID, status string) error {
	...
	if status == "running" {
		_, err = db.Exec(
			"UPDATE batch_task_queues SET status = $1, started_at = COALESCE(started_at, $2) WHERE id = $3",
			status, now, queueID,
		)
	} else if status == "completed" || status == "cancelled" {
		_, err = db.Exec(
			"UPDATE batch_task_queues SET status = $1, completed_at = COALESCE(completed_at, $2) WHERE id = $3",
			status, now, queueID,
		)
	} else {
		_, err = db.Exec(
			"UPDATE batch_task_queues SET status = $1 WHERE id = $2",
			status, queueID,
		)
	}
...
func (db *DB) UpdateBatchTaskStatus(queueID, taskID, status string, conversationID, result, errorMsg string) error {
	...
	if conversationID != "" {
		updates = append(updates, fmt.Sprintf("conversation_id = $%d", len(args)+1))
		args = append(args, conversationID)
	}

	if result != "" {
		updates = append(updates, fmt.Sprintf("result = $%d", len(args)+1))
		args = append(args, result)
	}

	if errorMsg != "" {
		updates = append(updates, fmt.Sprintf("error = $%d", len(args)+1))
		args = append(args, errorMsg)
	}

	if status == "running" {
		updates = append(updates, fmt.Sprintf("started_at = COALESCE(started_at, $%d)", len(args)+1))
		args = append(args, now)
	}

	if status == "completed" || status == "failed" || status == "cancelled" {
		updates = append(updates, fmt.Sprintf("completed_at = COALESCE(completed_at, $%d)", len(args)+1))
		args = append(args, now)
	}
	...
	sql += fmt.Sprintf(" WHERE queue_id = $%d AND id = $%d", len(args)-1, len(args))
...
func (db *DB) UpdateBatchQueueCurrentIndex(queueID string, currentIndex int) error {
	_, err := db.Exec(
		"UPDATE batch_task_queues SET current_index = $1 WHERE id = $2",
...
func (db *DB) UpdateBatchQueueSchedule(queueID, scheduleMode, cronExpr string, nextRunAt *time.Time) error {
	...
	_, err := db.Exec(
		"UPDATE batch_task_queues SET schedule_mode = $1, cron_expr = $2, next_run_at = $3 WHERE id = $4",
...
func (db *DB) UpdateBatchQueueScheduleEnabled(queueID string, enabled bool) error {
	...
	_, err := db.Exec(
		"UPDATE batch_task_queues SET schedule_enabled = $1 WHERE id = $2",
		v, queueID,
	)
	if err != nil {
...
func (db *DB) RecordBatchQueueScheduledTriggerStart(queueID string, at time.Time) error {
	_, err := db.Exec(
		"UPDATE batch_task_queues SET last_schedule_trigger_at = $1, last_schedule_error = NULL WHERE id = $2",
		at, queueID,
	)
	if err != nil {
...
func (db *DB) SetBatchQueueLastScheduleError(queueID, msg string) error {
	_, err := db.Exec(
		"UPDATE batch_task_queues SET last_schedule_error = $1 WHERE id = $2",
		msg, queueID,
	)
	if err != nil {
...
func (db *DB) SetBatchQueueLastRunError(queueID, msg string) error {
	...
	_, err := db.Exec(
		"UPDATE batch_task_queues SET last_run_error = $1 WHERE id = $2",
		v, queueID,
	)
	if err != nil {
...
func (db *DB) ResetBatchQueueForRerun(queueID string) error {
	...
	_, err = tx.Exec(
		"UPDATE batch_task_queues SET status = $1, current_index = 0, started_at = NULL, completed_at = NULL, last_run_error = NULL, last_schedule_error = NULL WHERE id = $2",
		"pending", queueID,
	)
	if err != nil {
	...
	_, err = tx.Exec(
		"UPDATE batch_tasks SET status = $1, conversation_id = NULL, started_at = NULL, completed_at = NULL, error = NULL, result = NULL WHERE queue_id = $2",
		"pending", queueID,
	)
	if err != nil {
...
func (db *DB) UpdateBatchTaskMessage(queueID, taskID, message string) error {
	_, err := db.Exec(
		"UPDATE batch_tasks SET message = $1 WHERE queue_id = $2 AND id = $3",
...
func (db *DB) CancelPendingBatchTasks(queueID string, completedAt time.Time) error {
	_, err := db.Exec(
		"UPDATE batch_tasks SET status = $1, completed_at = $2 WHERE queue_id = $3 AND status = $4",
...
func (db *DB) PrepareBatchSingleTaskRun(queueID, taskID string, taskIndex int, resetTask, resumeQueue bool) error {
	...
	if resetTask {
		_, err = tx.Exec(
			"UPDATE batch_tasks SET status = $1, conversation_id = NULL, started_at = NULL, completed_at = NULL, error = NULL, result = NULL WHERE queue_id = $2 AND id = $3",
	...
	if resumeQueue {
		_, err = tx.Exec(
			"UPDATE batch_task_queues SET status = $1, current_index = $2, completed_at = NULL, last_run_error = NULL WHERE id = $3",
			"paused", taskIndex, queueID,
		)
	} else {
		_, err = tx.Exec(
			"UPDATE batch_task_queues SET current_index = $1, last_run_error = NULL WHERE id = $2",
			taskIndex, queueID,
		)
	}
	if err != nil {
...
func (db *DB) DeleteBatchTask(queueID, taskID string) error {
	_, err := db.Exec(
		"DELETE FROM batch_tasks WHERE queue_id = $1 AND id = $2",
		queueID, taskID,
	)
	if err != nil {
...
func (db *DB) DeleteBatchQueue(queueID string) error {
	...
	// 删除任务（外键会自动级联删除）
	_, err = tx.Exec("DELETE FROM batch_tasks WHERE queue_id = $1", queueID)
	if err != nil {
		return fmt.Errorf("删除批量任务失败: %w", err)
	}

	// 删除队列
	_, err = tx.Exec("DELETE FROM batch_task_queues WHERE id = $1", queueID)
	if err != nil {
