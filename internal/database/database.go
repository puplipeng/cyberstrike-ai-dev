...
// NewDB 创建 PostgreSQL 数据库连接（兼容旧接口，dsn 为 PostgreSQL 连接串）。
func NewDB(dsn string, logger *zap.Logger) (*DB, error) {
	return NewPostgresDB(dsn, "UTC", logger)
}

// NewPostgresDB 创建 PostgreSQL 数据库连接。
// dsn 形如 postgres://user:pass@host:5432/dbname?sslmode=disable；
// localTZ 保留用于配置兼容；数据库会话按 UTC 运行。SQL 已使用 PostgreSQL 方言。
func NewPostgresDB(dsn, localTZ string, logger *zap.Logger) (*DB, error) {
	sqlDB, err := openPostgres(dsn, localTZ)
	if err != nil {
	...
	ensurePostgresCollations(sqlDB, logger)
	...
	// 会话产物目录沿用默认 data/ 布局。
	baseDir := filepath.Join("data", "conversation_artifacts")
	...
	if logger != nil {
		logger.Info("PostgreSQL 数据库已连接", zap.String("local_tz", localTZ))
	}
	return database, nil
}
...
func (db *DB) initTables() error {
	...
	CREATE TABLE IF NOT EXISTS model_token_usage (
		...
		FOREIGN KEY (message_id) REFERENCES messages(id) ON DELETE CASCADE,
		FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE CASCADE
		-- project_id FK 移除：建表顺序上 projects 在其后，PostgreSQL 严格 FK 校验会失败（原版 MySQL 宽松）
	);`
	...
	CREATE TABLE IF NOT EXISTS tool_executions (
		...
		partial_output_updated_at TEXT,
		owner_user_id TEXT,
	...
	CREATE TABLE IF NOT EXISTS assets (
		...
		tags_json TEXT NOT NULL DEFAULT '[]', first_seen_at TEXT NOT NULL, last_seen_at TEXT NOT NULL,
		created_at TEXT NOT NULL, updated_at TEXT NOT NULL, owner_user_id TEXT,
	...
	CREATE TABLE IF NOT EXISTS batch_task_queues (
		...
		last_schedule_trigger_at TEXT,
		last_schedule_error TEXT,
		last_run_error TEXT,
		project_id TEXT,
		concurrency INTEGER NOT NULL DEFAULT 1,
		hitl_config TEXT NOT NULL DEFAULT '{}',
		status TEXT NOT NULL,
		created_at TEXT NOT NULL,
		started_at TEXT,
		completed_at TEXT,
	...
	CREATE TABLE IF NOT EXISTS c2_sessions (
		...
		last_check_in TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
	...
	CREATE TABLE IF NOT EXISTS workflow_package_imports (
		...
		error_code TEXT, error_message TEXT, created_at TEXT NOT NULL, applied_at TEXT,
	...
	workflowTableDDLs := []struct{ name, ddl string }{
	...
	for _, t := range workflowTableDDLs {
		if _, err := db.Exec(t.ddl); err != nil {
			return fmt.Errorf("创建%s表失败: %w", t.name, err)
		}
	}

	// 依赖顺序：c2_listeners → c2_sessions → c2_tasks → c2_files → c2_events → c2_profiles
	// 不可用 map 迭代（Go map 顺序随机会导致外键引用的表尚未创建而失败）
	c2TableDDLs := []struct{ name, ddl string }{
...
func (db *DB) migrateRobotUserSessionsTable() error {
	var count int
	if err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='robot_user_sessions' AND column_name='agent_mode'").Scan(&count); err != nil {
		return err
	}
	if count == 0 {
...
func (db *DB) migrateToolExecutionsPartialOutputColumns() error {
	for _, col := range []struct {
		...
		{"partial_output_updated_at", "ALTER TABLE tool_executions ADD COLUMN partial_output_updated_at TEXT"},
	} {
...
func (db *DB) migrateAssetsTable() error {
	columns := []struct {
		...
		{"last_scan_at", "ALTER TABLE assets ADD COLUMN last_scan_at TEXT"},
	...
	for _, column := range columns {
		var count int
		if err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='assets' AND column_name=$1", column.name).Scan(&count); err != nil {
			return err
		}
		if count == 0 {
...
func (db *DB) migrateMessagesTable() error {
	var count int
	err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='messages' AND column_name='updated_at'").Scan(&count)
	if err != nil {
		// 如果查询失败，尝试添加字段
		if _, addErr := db.Exec("ALTER TABLE messages ADD COLUMN updated_at TEXT"); addErr != nil {
		...
		if _, err := db.Exec("ALTER TABLE messages ADD COLUMN updated_at TEXT"); err != nil {
	...
	var rcColCount int
	errRC := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='messages' AND column_name='reasoning_content'").Scan(&rcColCount)
	if errRC != nil {
...
func (db *DB) migrateConversationsTable() error {
	...
	var count int
	err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='conversations' AND column_name='last_react_input'").Scan(&count)
	if err != nil {
		// 如果查询失败，尝试添加字段
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='conversations' AND column_name='last_react_output'").Scan(&count)
	if err != nil {
		// 如果查询失败，尝试添加字段
	...
	// 检查pinned字段是否存在
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='conversations' AND column_name='pinned'").Scan(&count)
	if err != nil {
		// 如果查询失败，尝试添加字段
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='conversations' AND column_name='webshell_connection_id'").Scan(&count)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='conversations' AND column_name='role_name'").Scan(&count)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='conversations' AND column_name='agent_mode'").Scan(&count)
	if err != nil {
...
func (db *DB) migrateBatchTaskQueuesTable() error {
	// 检查title字段是否存在
	var count int
	err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='title'").Scan(&count)
	if err != nil {
		// 如果查询失败，尝试添加字段
	...
	// 检查role字段是否存在
	var roleCount int
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='role'").Scan(&roleCount)
	if err != nil {
		// 如果查询失败，尝试添加字段
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='agent_mode'").Scan(&agentModeCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='schedule_mode'").Scan(&scheduleModeCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='cron_expr'").Scan(&cronExprCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='next_run_at'").Scan(&nextRunAtCount)
	if err != nil {
		if _, addErr := db.Exec("ALTER TABLE batch_task_queues ADD COLUMN next_run_at TEXT"); addErr != nil {
			errMsg := strings.ToLower(addErr.Error())
			if !strings.Contains(errMsg, "duplicate column") && !strings.Contains(errMsg, "already exists") {
		...
		if _, err := db.Exec("ALTER TABLE batch_task_queues ADD COLUMN next_run_at TEXT"); err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='schedule_enabled'").Scan(&scheduleEnCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='last_schedule_trigger_at'").Scan(&lastTrigCount)
	if err != nil {
		if _, addErr := db.Exec("ALTER TABLE batch_task_queues ADD COLUMN last_schedule_trigger_at TEXT"); addErr != nil {
			errMsg := strings.ToLower(addErr.Error())
			if !strings.Contains(errMsg, "duplicate column") && !strings.Contains(errMsg, "already exists") {
		...
		if _, err := db.Exec("ALTER TABLE batch_task_queues ADD COLUMN last_schedule_trigger_at TEXT"); err != nil {
			db.logger.Warn("添加last_schedule_trigger_at字段失败", zap.Error(err))
		}
	}

	var lastSchedErrCount int
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='last_schedule_error'").Scan(&lastSchedErrCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='last_run_error'").Scan(&lastRunErrCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='project_id'").Scan(&projectIDCount)
	if err != nil {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='batch_task_queues' AND column_name='concurrency'").Scan(&concurrencyCount)
	if err != nil {
	...
	if err != nil {
		if _, addErr := db.Exec("ALTER TABLE batch_task_queues ADD COLUMN hitl_config TEXT NOT NULL DEFAULT '{}'"); addErr != nil {
			errMsg := strings.ToLower(addErr.Error())
			if !strings.Contains(errMsg, "duplicate column") && !strings.Contains(errMsg, "already exists") {
				db.logger.Warn("添加batch_task_queues.hitl_config字段失败", zap.Error(addErr))
			}
		}
	} else if hitlConfigCount == 0 {
		if _, addErr := db.Exec("ALTER TABLE batch_task_queues ADD COLUMN hitl_config TEXT NOT NULL DEFAULT '{}'"); addErr != nil {
			db.logger.Warn("添加batch_task_queues.hitl_config字段失败", zap.Error(addErr))
		}
	}
...
func (db *DB) migrateProjectsTable() error {
	for _, col := range []struct {
		...
		var count int
		err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name=$1 AND column_name=$2", col.table, col.name).Scan(&count)
		if err != nil {
...
func vulnerabilitiesConversationFKOnDeleteSetNull(db *sql.DB) (bool, error) {
	// PostgreSQL：information_schema 检查 vulnerabilities.conversation_id 是否存在 FK 约束
	rows, err := db.Query(`SELECT 1 FROM information_schema.table_constraints tc
		JOIN information_schema.key_column_usage kcu ON tc.constraint_name = kcu.constraint_name AND tc.table_schema = kcu.table_schema
		WHERE tc.table_name = 'vulnerabilities' AND tc.constraint_type = 'FOREIGN KEY' AND kcu.column_name = 'conversation_id'`)
	if err != nil {
		return false, err
	}
	defer rows.Close()
...
func (db *DB) migrateVulnerabilitiesTable() error {
	...
	for _, col := range columns {
		var count int
		err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='vulnerabilities' AND column_name=$1", col.name).Scan(&count)
		if err != nil {
			if _, addErr := db.Exec(col.stmt); addErr != nil {
				errMsg := strings.ToLower(addErr.Error())
				if !strings.Contains(errMsg, "duplicate column") && !strings.Contains(errMsg, "already exists") {
...
func (db *DB) migrateWebshellConnectionsTable() error {
	...
	for _, col := range columns {
		var count int
		err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='webshell_connections' AND column_name=$1", col.name).Scan(&count)
		if err != nil {
			if _, addErr := db.Exec(col.stmt); addErr != nil {
				errMsg := strings.ToLower(addErr.Error())
				if !strings.Contains(errMsg, "duplicate column") && !strings.Contains(errMsg, "already exists") {
...
// NewKnowledgeDB 创建知识库 PostgreSQL 数据库连接（兼容旧接口，dsn 为 PostgreSQL 连接串）。
func NewKnowledgeDB(dsn string, logger *zap.Logger) (*DB, error) {
	return NewKnowledgePostgresDB(dsn, "UTC", logger)
}

// NewKnowledgePostgresDB 创建 PostgreSQL 知识库连接（只包含知识库相关的表）。
func NewKnowledgePostgresDB(dsn, localTZ string, logger *zap.Logger) (*DB, error) {
	sqlDB, err := openPostgres(dsn, localTZ)
	if err != nil {
		return nil, fmt.Errorf("打开 PostgreSQL 知识库数据库失败: %w", err)
	}
	configureDBPool(sqlDB)

	if err := sqlDB.Ping(); err != nil {
		_ = sqlDB.Close()
		return nil, fmt.Errorf("连接 PostgreSQL 知识库数据库失败: %w", err)
	}
	...
	if logger != nil {
		logger.Info("PostgreSQL 知识库数据库已连接")
	}
	return database, nil
}

// ensurePostgresCollations 尽力创建 nocase 排序规则（SQLite COLLATE NOCASE 的等价物，
// 供知识库检索等处使用）。已存在或无权限时静默跳过，仅记录警告。
func ensurePostgresCollations(sqlDB *sql.DB, logger *zap.Logger) {
	const ddl = `DO $$ BEGIN
		CREATE COLLATION public.nocase (provider = icu, locale = 'und-u-ks-level2');
	EXCEPTION WHEN duplicate_object THEN NULL; END $$`
	if _, err := sqlDB.Exec(ddl); err != nil && logger != nil {
		logger.Warn("创建 PostgreSQL nocase collation 失败（使用 COLLATE NOCASE 的查询可能报错）", zap.Error(err))
	}
}

// EnsureKnowledgeTables initializes the optional knowledge schema on a shared
// database. Call it only when knowledge is enabled and uses this database, so a
// disabled or separately hosted knowledge base does not require pgvector here.
func (db *DB) EnsureKnowledgeTables() error {
...
func (db *DB) migrateKnowledgeEmbeddingsColumns() error {
	var n int
	if err := db.QueryRow(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public' AND table_name='knowledge_embeddings'`).Scan(&n); err != nil {
		return err
	}
	if n == 0 {
		return nil
	...
	for _, m := range migrations {
		var colCount int
		q := `SELECT COUNT(*) FROM information_schema.columns WHERE table_name='knowledge_embeddings' AND column_name=$1`
