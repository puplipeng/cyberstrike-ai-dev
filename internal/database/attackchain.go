...
func (db *DB) SaveAttackChainNode(conversationID, nodeID, nodeType, nodeName, toolExecutionID, metadata string, riskScore int) error {
	...
	query := `
		...
		ON CONFLICT (id) DO UPDATE SET
			conversation_id = excluded.conversation_id, node_type = excluded.node_type,
			node_name = excluded.node_name, tool_execution_id = excluded.tool_execution_id,
			metadata = excluded.metadata, risk_score = excluded.risk_score, created_at = excluded.created_at
	`
...
func (db *DB) SaveAttackChainEdge(conversationID, edgeID, sourceNodeID, targetNodeID, edgeType string, weight int) error {
	query := `
		...
		ON CONFLICT (id) DO UPDATE SET
			conversation_id = excluded.conversation_id, source_node_id = excluded.source_node_id,
			target_node_id = excluded.target_node_id, edge_type = excluded.edge_type,
...
func (db *DB) DeleteAttackChain(conversationID string) error {
	// 先删除边（因为有外键约束）
	_, err := db.Exec("DELETE FROM attack_chain_edges WHERE conversation_id = $1", conversationID)
	if err != nil {
		db.logger.Warn("删除攻击链边失败", zap.Error(err))
	}

	// 再删除节点
	_, err = db.Exec("DELETE FROM attack_chain_nodes WHERE conversation_id = $1", conversationID)
	if err != nil {
