...
func (db *DB) addColumnIfMissing(table, name, stmt string) error {
	var count int
	err := db.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name=$1 AND column_name=$2", table, name).Scan(&count)
	if err != nil || count == 0 {
...
func (db *DB) BootstrapRBAC(adminPasswordHash string, permissions map[string]string) error {
	...
	for key, desc := range permissions {
		...
		if _, err := tx.Exec(`INSERT INTO rbac_permissions (key, description, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, key, desc, now); err != nil {
			return err
		}
		if _, err := tx.Exec(`UPDATE rbac_permissions SET description = $1 WHERE key = $2`, desc, key); err != nil {
			return err
		}
	}
	...
	for _, key := range stalePermissionKeys {
		if _, err := tx.Exec(`DELETE FROM rbac_role_permissions WHERE permission_key = $1`, key); err != nil {
			return err
		}
		if _, err := tx.Exec(`DELETE FROM rbac_permissions WHERE key = $1`, key); err != nil {
			return err
		}
	}
	...
	if userCount == 0 {
		...
		if _, err := tx.Exec(`
			...
			VALUES ($1, 'admin', '管理员', $2, 1, 1, $3, $4)
		`, "admin", adminPasswordHash, now, now); err != nil {
			return err
		}
		if _, err := tx.Exec(`INSERT INTO rbac_user_roles (user_id, role_id, created_at) VALUES ('admin', $1, $2) ON CONFLICT DO NOTHING`, RBACSystemRoleAdmin, now); err != nil {
			return err
		}
	} else if strings.TrimSpace(adminPasswordHash) != "" {
		if _, err := tx.Exec(`UPDATE rbac_users SET password_hash = $1, updated_at = $2 WHERE username = 'admin' AND is_builtin = 1 AND (password_hash = '' OR password_hash IS NULL)`, adminPasswordHash, now); err != nil {
			return err
		}
	}
...
func grantSystemRolePermissions(tx *sql.Tx, permissions map[string]string) error {
	...
	if _, err := tx.Exec(`DELETE FROM rbac_role_permissions WHERE role_id IN ($1, $2, $3, $4)`, RBACSystemRoleAdmin, RBACSystemRoleOperator, RBACSystemRoleAuditor, RBACSystemRoleViewer); err != nil {
		return err
	}
	for key := range permissions {
		if _, err := tx.Exec(`INSERT INTO rbac_role_permissions (role_id, permission_key, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, RBACSystemRoleAdmin, key, now); err != nil {
			return err
		}
		switch {
		case key == "auth:self":
			for _, roleID := range []string{RBACSystemRoleOperator, RBACSystemRoleAuditor, RBACSystemRoleViewer} {
				if _, err := tx.Exec(`INSERT INTO rbac_role_permissions (role_id, permission_key, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, roleID, key, now); err != nil {
					return err
				}
			}
		case key == "audit:read":
			if _, err := tx.Exec(`INSERT INTO rbac_role_permissions (role_id, permission_key, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, RBACSystemRoleAuditor, key, now); err != nil {
				return err
			}
		...
		default:
			if _, err := tx.Exec(`INSERT INTO rbac_role_permissions (role_id, permission_key, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, RBACSystemRoleOperator, key, now); err != nil {
				return err
			}
		}
	}
...
func (db *DB) GetRBACUserByUsername(username string) (*RBACUser, error) {
	...
	return db.scanRBACUser(db.QueryRow(`
		...
		FROM rbac_users WHERE username = $1
	`, username))
}
...
func (db *DB) UserCanAccessResource(userID, scope, resourceType, resourceID string) bool {
	...
	var n int
	err := db.QueryRow(`SELECT COUNT(*) FROM rbac_resource_assignments WHERE user_id = $1 AND resource_type = $2 AND resource_id = $3`, userID, resourceType, resourceID).Scan(&n)
	if err == nil && n > 0 {
		return true
	}
...
func (db *DB) userCanAccessAssetViaParent(userID, scope, assetID string) bool {
	...
	if err := db.QueryRow(`SELECT project_id FROM assets WHERE id = $1`, assetID).Scan(&projectID); err != nil {
		return false
	}
...
func (db *DB) userCanAccessConversationViaParent(userID, scope, conversationID string) bool {
	var projectID sql.NullString
	if err := db.QueryRow(`SELECT project_id FROM conversations WHERE id = $1`, conversationID).Scan(&projectID); err != nil {
		return false
	}
	return projectID.Valid && strings.TrimSpace(projectID.String) != "" &&
		db.UserCanAccessResource(userID, scope, "project", strings.TrimSpace(projectID.String))
...
func (db *DB) userCanAccessVulnerabilityViaParent(userID, scope, vulnerabilityID string) bool {
	...
	err := db.QueryRow(`SELECT project_id, conversation_id FROM vulnerabilities WHERE id = $1`, vulnerabilityID).Scan(&projectID, &conversationID)
	if err != nil {
		return false
	}
...
func (db *DB) UserCanAccessMessage(userID, scope, messageID string) bool {
	...
	err := db.QueryRow(`SELECT conversation_id FROM messages WHERE id = $1`, strings.TrimSpace(messageID)).Scan(&conversationID)
	if err != nil {
		return false
	}
...
func (db *DB) UserCanAccessProcessDetail(userID, scope, processDetailID string) bool {
	var conversationID string
	err := db.QueryRow(`SELECT conversation_id FROM process_details WHERE id = $1`, strings.TrimSpace(processDetailID)).Scan(&conversationID)
	if err != nil {
		return false
	}
	return db.UserCanAccessResource(userID, scope, "conversation", conversationID)
...
func (db *DB) userCanAccessC2ViaParent(userID, scope, resourceType, resourceID string) bool {
	...
	case "c2_session":
		...
		if err := db.QueryRow(`SELECT listener_id FROM c2_sessions WHERE id = $1`, resourceID).Scan(&listenerID); err != nil {
			return false
		}
		return db.UserCanAccessResource(userID, scope, "c2_listener", listenerID)
	case "c2_task":
		var sessionID string
		if err := db.QueryRow(`SELECT session_id FROM c2_tasks WHERE id = $1`, resourceID).Scan(&sessionID); err != nil {
			return false
		}
		return db.UserCanAccessResource(userID, scope, "c2_session", sessionID)
	case "c2_file":
		var sessionID string
		if err := db.QueryRow(`SELECT session_id FROM c2_files WHERE id = $1`, resourceID).Scan(&sessionID); err != nil {
			return false
		}
		return db.UserCanAccessResource(userID, scope, "c2_session", sessionID)
	case "c2_event":
		var sessionID, taskID sql.NullString
		if err := db.QueryRow(`SELECT session_id, task_id FROM c2_events WHERE id = $1`, resourceID).Scan(&sessionID, &taskID); err != nil {
			return false
		}
...
func (db *DB) userOwnsResource(userID, resourceType, resourceID string) bool {
	...
	var n int
	err := db.QueryRow(`SELECT COUNT(*) FROM `+table+` WHERE id = $1 AND owner_user_id = $2`, resourceID, userID).Scan(&n)
	return err == nil && n > 0
}
...
func (db *DB) SetResourceOwner(resourceType, resourceID, userID string) error {
	...
	_, err := db.Exec(`UPDATE `+table+` SET owner_user_id = COALESCE(NULLIF(owner_user_id, ''), $1) WHERE id = $2`, userID, resourceID)
	return err
}
...
func (db *DB) GetResourceOwner(resourceType, resourceID string) string {
	...
	if err := db.QueryRow(`SELECT owner_user_id FROM `+table+` WHERE id = $1`, strings.TrimSpace(resourceID)).Scan(&owner); err != nil {
		return ""
	}
...
func (db *DB) ListAssignableRBACResourcesPage(resourceType, search string, limit, offset int) ([]RBACResourceOption, error) {
	...
	case "project":
		query = `SELECT id, name, status FROM projects
			WHERE LOWER(name) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'
			ORDER BY updated_at DESC LIMIT $3::bigint OFFSET $4::bigint`
	case "conversation":
		query = `SELECT id, COALESCE(NULLIF(TRIM(title), ''), '未命名对话'), COALESCE(project_id, '') FROM conversations
			WHERE LOWER(COALESCE(NULLIF(TRIM(title), ''), id)) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'
			ORDER BY updated_at DESC LIMIT $3::bigint OFFSET $4::bigint`
	case "vulnerability":
		query = `SELECT id, title, severity FROM vulnerabilities
			WHERE LOWER(title) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'
			ORDER BY updated_at DESC LIMIT $3::bigint OFFSET $4::bigint`
	case "asset":
		query = `SELECT id, COALESCE(NULLIF(host,''),NULLIF(domain,''),NULLIF(ip,''),id), protocol || CASE WHEN port>0 THEN ':' || port ELSE '' END FROM assets
			WHERE LOWER(host) ILIKE $1 ESCAPE '\' OR LOWER(domain) ILIKE $2 ESCAPE '\' OR LOWER(ip) ILIKE $3 ESCAPE '\'
			ORDER BY updated_at DESC LIMIT $4::bigint OFFSET $5::bigint`
	case "webshell":
		query = `SELECT id, COALESCE(NULLIF(remark, ''), url), type FROM webshell_connections
			WHERE LOWER(COALESCE(NULLIF(remark, ''), url)) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'
			ORDER BY created_at DESC LIMIT $3::bigint OFFSET $4::bigint`
	case "batch_task":
		query = `SELECT id, COALESCE(NULLIF(title, ''), id), status FROM batch_task_queues
			WHERE LOWER(COALESCE(NULLIF(title, ''), id)) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'
			ORDER BY created_at DESC LIMIT $3::bigint OFFSET $4::bigint`
	case "c2_listener":
...
func (db *DB) CountAssignableRBACResources(resourceType, search string) (int, error) {
	...
	switch resourceType {
	case "project":
		query = `SELECT COUNT(*) FROM projects WHERE LOWER(name) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'`
	case "conversation":
		query = `SELECT COUNT(*) FROM conversations WHERE LOWER(COALESCE(NULLIF(TRIM(title), ''), id)) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'`
	case "vulnerability":
		query = `SELECT COUNT(*) FROM vulnerabilities WHERE LOWER(title) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'`
	case "asset":
		query = `SELECT COUNT(*) FROM assets WHERE LOWER(host) ILIKE $1 ESCAPE '\' OR LOWER(domain) ILIKE $2 ESCAPE '\' OR LOWER(ip) ILIKE $3 ESCAPE '\'`
	case "webshell":
		query = `SELECT COUNT(*) FROM webshell_connections WHERE LOWER(COALESCE(NULLIF(remark, ''), url)) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'`
	case "batch_task":
		query = `SELECT COUNT(*) FROM batch_task_queues WHERE LOWER(COALESCE(NULLIF(title, ''), id)) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'`
	case "c2_listener":
		query = `SELECT COUNT(*) FROM c2_listeners WHERE LOWER(name) ILIKE $1 ESCAPE '\' OR LOWER(id) ILIKE $2 ESCAPE '\'`
	}
	var total int
	queryArgs := []interface{}{pattern, pattern}
	if resourceType == "asset" {
...
func (db *DB) AssignResourcesToUser(userID, resourceType string, resourceIDs []string) (int64, error) {
	...
	var userExists int
	if err := tx.QueryRow(`SELECT COUNT(*) FROM rbac_users WHERE id = $1`, userID).Scan(&userExists); err != nil {
		return 0, err
	}
	if userExists == 0 {
	...
	for _, resourceID := range uniqueIDs {
		var exists int
		if err := tx.QueryRow(`SELECT COUNT(*) FROM `+table+` WHERE id = $1`, resourceID).Scan(&exists); err != nil {
			return 0, err
		}
		if exists == 0 {
	...
	for _, resourceID := range uniqueIDs {
		result, err := tx.Exec(`
			INSERT INTO rbac_resource_assignments (id, user_id, resource_type, resource_id, created_at)
			VALUES ($1, $2, $3, $4, $5)
...
func (db *DB) AssignResourcesToUserAuto(userID string, resourceIDs []string) (int64, map[string]string, error) {
	...
	for _, resourceID := range uniqueIDs {
		for _, pair := range typeTablePairs {
			var exists int
			if err := tx.QueryRow(`SELECT COUNT(*) FROM `+pair.table+` WHERE id = $1`, resourceID).Scan(&exists); err != nil {
				return 0, nil, err
			}
			if exists > 0 {
...
func (db *DB) GetRBACRoleByID(id string) (*RBACRole, error) {
	...
	var system int
	var createdAt, updatedAt string
	err := db.QueryRow(`SELECT id, name, description, scope, is_system, created_at, updated_at FROM rbac_roles WHERE id = $1`, id).
...
func (db *DB) UpsertRBACRole(id, name, description, scope string, permissionKeys []string) (*RBACRole, error) {
	...
	defer func() { _ = tx.Rollback() }()
	var isSystem int
	_ = tx.QueryRow(`SELECT is_system FROM rbac_roles WHERE id = $1`, id).Scan(&isSystem)
	if _, err := tx.Exec(`
		INSERT INTO rbac_roles (id, name, description, scope, is_system, created_at, updated_at)
	...
	if _, err := tx.Exec(`DELETE FROM rbac_role_permissions WHERE role_id = $1`, id); err != nil {
		return nil, err
	}
	for _, key := range permissionKeys {
		...
		if err := tx.QueryRow(`SELECT COUNT(*) FROM rbac_permissions WHERE key = $1`, key).Scan(&permissionExists); err != nil {
			return nil, err
		}
		if permissionExists == 0 {
			return nil, fmt.Errorf("unknown permission: %s", key)
		}
		if _, err := tx.Exec(`INSERT INTO rbac_role_permissions (role_id, permission_key, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, id, key, now); err != nil {
			return nil, err
		}
	}
	if err := tx.Commit(); err != nil {
...
func (db *DB) DeleteRBACRole(id string) error {
	...
	_, err := db.Exec(`DELETE FROM rbac_roles WHERE id = $1 AND is_system = 0`, id)
	return err
}
...
func (db *DB) UpdateRBACUserPassword(userID, passwordHash string) error {
	...
	_, err := db.Exec(`UPDATE rbac_users SET password_hash = $1, updated_at = $2 WHERE id = $3`, passwordHash, time.Now(), userID)
	return err
}
...
func (db *DB) CreateRBACUser(username, displayName, passwordHash string, enabled bool, roleIDs []string) (*RBACUser, error) {
	...
	for _, roleID := range roleIDs {
		...
		if _, err := tx.Exec(`INSERT INTO rbac_user_roles (user_id, role_id, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, id, roleID, now); err != nil {
			return nil, err
		}
	}
	if err := tx.Commit(); err != nil {
...
func (db *DB) UpdateRBACUser(userID, displayName string, enabled *bool, roleIDs *[]string) error {
	...
	defer func() { _ = tx.Rollback() }()
	if enabled != nil {
		if _, err := tx.Exec(`UPDATE rbac_users SET display_name = $1, enabled = $2, updated_at = $3 WHERE id = $4`, strings.TrimSpace(displayName), boolToInt(*enabled), time.Now(), userID); err != nil {
			return err
		}
	} else {
		if _, err := tx.Exec(`UPDATE rbac_users SET display_name = $1, updated_at = $2 WHERE id = $3`, strings.TrimSpace(displayName), time.Now(), userID); err != nil {
			return err
		}
	}
	if roleIDs != nil {
		if _, err := tx.Exec(`DELETE FROM rbac_user_roles WHERE user_id = $1`, userID); err != nil {
			return err
		}
		for _, roleID := range *roleIDs {
			roleID = strings.TrimSpace(roleID)
			...
			if _, err := tx.Exec(`INSERT INTO rbac_user_roles (user_id, role_id, created_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, userID, roleID, time.Now()); err != nil {
				return err
			}
		}
	}
...
func (db *DB) DeleteRBACUser(userID string) error {
	...
	_, err := db.Exec(`DELETE FROM rbac_users WHERE id = $1 AND is_builtin = 0`, userID)
	return err
}

func (db *DB) ListRBACUserRoleIDs(userID string) ([]string, error) {
	rows, err := db.Query(`SELECT role_id FROM rbac_user_roles WHERE user_id = $1 ORDER BY role_id ASC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) ListRBACRolePermissionKeys(roleID string) ([]string, error) {
	rows, err := db.Query(`SELECT permission_key FROM rbac_role_permissions WHERE role_id = $1 ORDER BY permission_key ASC`, roleID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) DeleteRBACResourceAssignmentWithDetails(id string) (*RBACResourceAssignment, error) {
	...
	result, err := tx.Exec(`DELETE FROM rbac_resource_assignments WHERE id = $1`, id)
	if err != nil {
		return nil, err
	}
