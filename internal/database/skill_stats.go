...
func (db *DB) UpdateSkillStats(skillName string, totalCalls, successCalls, failedCalls int, lastCallTime *time.Time) error {
	...
	query := `
		...
		VALUES ($1, $2, $3, $4, $5, $6)
		ON CONFLICT(skill_name) DO UPDATE SET
			total_calls = total_calls + $7,
			success_calls = success_calls + $8,
			failed_calls = failed_calls + $9,
			last_call_time = COALESCE($10, last_call_time),
			updated_at = $11
	`
...
func (db *DB) ClearSkillStatsByName(skillName string) error {
	query := `DELETE FROM skill_stats WHERE skill_name = $1`
