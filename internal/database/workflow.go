...
func (db *DB) GetWorkflowDefinition(id string) (*WorkflowDefinition, error) {
	...
	wf, err := scanWorkflowDefinition(db.QueryRow("SELECT "+workflowDefinitionColumns+" FROM workflow_definitions WHERE id = $1", id))
	...
	if existing == nil {
		...
		_, err = db.Exec(
			`UPDATE workflow_definitions
			 SET name = $1, description = $2, version = $3, graph_json = $4, enabled = $5, updated_at = $6
			 WHERE id = $7`,
...
func (db *DB) DeleteWorkflowDefinition(id string) error {
	...
	if _, err := db.Exec("DELETE FROM workflow_definitions WHERE id = $1", id); err != nil {
...
func (db *DB) FinishWorkflowRun(runID, status, outputJSON, errText string) error {
	...
	now := time.Now()
	_, err := db.Exec(
		`UPDATE workflow_runs SET status = $1, output_json = $2, error = $3, finished_at = $4 WHERE id = $5`,
...
func (db *DB) FinishWorkflowNodeRun(nodeRunID, status, outputJSON, errText string) error {
	...
	now := time.Now()
	_, err := db.Exec(
		`UPDATE workflow_node_runs SET status = $1, output_json = $2, error = $3, finished_at = $4 WHERE id = $5`,
...
func (db *DB) ListWorkflowNodeRuns(runID string) ([]*WorkflowNodeRun, error) {
	...
	rows, err := db.Query(
		`SELECT id, run_id, node_id, status, input_json, output_json, error, started_at, finished_at
		 FROM workflow_node_runs WHERE run_id = $1 ORDER BY started_at ASC`,
		runID,
	)
	if err != nil {
...
func (db *DB) GetWorkflowRun(runID string) (*WorkflowRun, error) {
	...
	row, err := scanWorkflowRun(db.QueryRow("SELECT "+workflowRunColumns+" FROM workflow_runs WHERE id = $1", runID))
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func (db *DB) SetWorkflowRunStatus(runID, status string) error {
	...
	_, err := db.Exec(`UPDATE workflow_runs SET status = $1 WHERE id = $2`, strings.TrimSpace(status), runID)
	if err != nil {
...
func (db *DB) SetWorkflowRunAwaitingHITL(runID, nodeID, pendingJSON string) error {
	...
	_, err := db.Exec(
		`UPDATE workflow_runs SET status = 'awaiting_hitl', pending_hitl_node_id = $1, pending_hitl_json = $2, finished_at = NULL WHERE id = $3`,
...
func (db *DB) RecordWorkflowRunHITLDecision(runID string, approved bool, comment string) error {
	...
	_, err = db.Exec(
		`UPDATE workflow_runs SET pending_hitl_json = $1 WHERE id = $2 AND status = 'awaiting_hitl'`,
		string(raw), runID,
	)
	if err != nil {
...
func (db *DB) ListWorkflowRunsAwaitingHITLFiltered(conversationID string, limit int) ([]*WorkflowRun, error) {
	...
	if conversationID != "" {
		rows, err = db.Query(
			`SELECT `+workflowRunColumns+` FROM workflow_runs WHERE status = 'awaiting_hitl' AND conversation_id = $1 ORDER BY started_at DESC LIMIT $2::bigint`,
			conversationID, limit,
		)
	} else {
		rows, err = db.Query(
			`SELECT `+workflowRunColumns+` FROM workflow_runs WHERE status = 'awaiting_hitl' ORDER BY started_at DESC LIMIT $1::bigint`,
			limit,
		)
	}
	if err != nil {
