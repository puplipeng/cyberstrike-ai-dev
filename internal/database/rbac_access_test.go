...
func TestRoleRejectsUnknownPermission(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO rbac_permissions (key, description, created_at) VALUES ('stale:permission', '', $1)`, time.Now()); err != nil {
		t.Fatal(err)
	}
	if err := db.BootstrapRBAC("hash", map[string]string{"auth:self": "self"}); err != nil {
		t.Fatal(err)
...
func TestRBACVulnerabilityAccessInheritsProject(t *testing.T) {
	...
	options, err := db.GetVulnerabilityFilterOptionsForAccess(RBACListAccess{UserID: user.ID, Scope: RBACScopeAssigned})
	if err != nil {
		t.Fatal(err)
	}
	if got := options["vulnerability_ids"]; len(got) != 1 || got[0] != v1.ID {
		t.Fatalf("scoped vulnerability filter options = %#v, want only %s", got, v1.ID)
	}
	if got := options["project_ids"]; len(got) != 1 || got[0] != p1.ID {
		t.Fatalf("scoped project filter options = %#v, want only %s", got, p1.ID)
	}
}
