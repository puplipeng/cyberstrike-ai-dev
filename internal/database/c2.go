...
func (db *DB) UpdateC2Listener(l *C2Listener) error {
	...
	query := `
		UPDATE c2_listeners SET
			project_id = $1, name = $2, type = $3, bind_host = $4, bind_port = $5, profile_id = $6, encryption_key = $7,
			implant_token = $8, status = $9, config_json = $10, remark = $11, owner_user_id = $12, started_at = $13, last_error = $14
		WHERE id = $15
	`
...
func (db *DB) SetC2ListenerStatus(id, status, lastError string, startedAt *time.Time) error {
	query := `
		UPDATE c2_listeners SET status = $1, last_error = $2, started_at = COALESCE($3, started_at)
		WHERE id = $4
	`
...
func (db *DB) ListC2ListenersForAccess(access RBACListAccess, projectID string) ([]*C2Listener, error) {
	...
	if projectID = strings.TrimSpace(projectID); projectID == ProjectFilterUnbound {
		...
		conditions = append(conditions, "COALESCE(project_id, '') = $1")
...
func appendC2ListenerAccessFilter(conditions *[]string, args *[]interface{}, access RBACListAccess) {
	...
	clauses := []string{fmt.Sprintf("owner_user_id = $%d", len(*args)+1)}
	*args = append(*args, access.UserID)
	if access.Scope == RBACScopeAssigned {
		clauses = append(clauses, fmt.Sprintf(`EXISTS (
			SELECT 1 FROM rbac_resource_assignments ra
			WHERE ra.user_id = $%d AND ra.resource_type = 'c2_listener' AND ra.resource_id = c2_listeners.id
		)`, len(*args)+1))
		*args = append(*args, access.UserID)
	}
...
func (db *DB) DeleteC2Listener(id string) error {
	res, err := db.Exec(`DELETE FROM c2_listeners WHERE id = $1`, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
...
func (db *DB) TouchC2Session(id, status string, t time.Time) error {
	...
	res, err := db.Exec(`UPDATE c2_sessions SET last_check_in = $1, status = $2 WHERE id = $3`, t, status, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
...
func (db *DB) SetC2SessionStatus(id, status string) error {
	res, err := db.Exec(`UPDATE c2_sessions SET status = $1 WHERE id = $2`, status, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
...
func (db *DB) SetC2SessionSleep(id string, sleepSeconds, jitterPercent int) error {
	...
	res, err := db.Exec(`UPDATE c2_sessions SET sleep_seconds = $1, jitter_percent = $2 WHERE id = $3`,
...
func (db *DB) ListC2Sessions(filter ListC2SessionsFilter) ([]*C2Session, error) {
	...
	if filter.ListenerID != "" {
		conditions = append(conditions, "listener_id = $1")
	...
	if strings.TrimSpace(filter.ProjectID) == ProjectFilterUnbound {
		...
		conditions = append(conditions, `EXISTS (
			SELECT 1 FROM c2_listeners l
			WHERE l.id = c2_sessions.listener_id AND COALESCE(l.project_id, '') = $1
		)`)
	...
	if filter.OS != "" {
		conditions = append(conditions, "os = $1")
		args = append(args, filter.OS)
	}
	if filter.Search != "" {
		conditions = append(conditions, fmt.Sprintf("(hostname LIKE $%d OR username LIKE $%d OR internal_ip LIKE $%d)", len(args)+1, len(args)+2, len(args)+3))
		kw := "%" + filter.Search + "%"
		args = append(args, kw, kw, kw)
	}
	if filter.Suspicious {
		conditions = append(conditions, `status = 'dead' AND (
			hostname ILIKE 'tcp_%' OR LOWER(COALESCE(username,'')) = 'unknown' OR COALESCE(pid, 0) = 0
		)`)
	}
	query := `
		SELECT id, listener_id, implant_uuid, COALESCE(hostname,''), COALESCE(username,''),
...
func appendC2SessionAccessFilter(conditions *[]string, args *[]interface{}, access RBACListAccess) {
	...
	clauses := []string{fmt.Sprintf(`EXISTS (
		SELECT 1 FROM c2_listeners
		WHERE c2_listeners.id = c2_sessions.listener_id AND c2_listeners.owner_user_id = $%d
	)`, len(*args)+1)}
	*args = append(*args, access.UserID)
	if access.Scope == RBACScopeAssigned {
		clauses = append(clauses, fmt.Sprintf(`EXISTS (
			SELECT 1 FROM rbac_resource_assignments ra
			WHERE ra.user_id = $%d AND ra.resource_type = 'c2_listener' AND ra.resource_id = c2_sessions.listener_id
		)`, len(*args)+1))
		*args = append(*args, access.UserID)
	}
	*conditions = append(*conditions, "("+strings.Join(clauses, " OR ")+")")
...
func (db *DB) DeleteC2Session(id string) error {
	res, err := db.Exec(`DELETE FROM c2_sessions WHERE id = $1`, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
...
func (db *DB) DeleteC2SessionsByIDs(ids []string) (int64, error) {
	...
	placeholders := postgresPlaceholders(1, len(clean))
...
func (db *DB) UpdateC2Task(id string, u C2TaskUpdate) error {
	...
	if u.ResultText != nil {
		sets = append(sets, "result_text = $1")
		args = append(args, *u.ResultText)
	}
	if u.ResultBlobPath != nil {
		sets = append(sets, "result_blob_path = $1")
		args = append(args, *u.ResultBlobPath)
	}
	if u.Error != nil {
		sets = append(sets, "error = $1")
		args = append(args, *u.Error)
	}
	if u.ApprovalStatus != nil {
		sets = append(sets, "approval_status = $1")
		args = append(args, *u.ApprovalStatus)
	}
	if u.SentAt != nil {
		sets = append(sets, "sent_at = $1")
		args = append(args, *u.SentAt)
	}
	if u.StartedAt != nil {
		sets = append(sets, "started_at = $1")
		args = append(args, *u.StartedAt)
	}
	if u.CompletedAt != nil {
		sets = append(sets, "completed_at = $1")
		args = append(args, *u.CompletedAt)
	}
	if u.DurationMS != nil {
		sets = append(sets, "duration_ms = $1")
		args = append(args, *u.DurationMS)
	}
	if len(sets) == 0 {
		return nil
	}
	query := "UPDATE c2_tasks SET " + strings.Join(sets, ", ") + " WHERE id = $1"
	args = append(args, id)
	res, err := db.Exec(query, args...)
	if err != nil {
		return err
...
func buildC2TasksWhere(filter ListC2TasksFilter) (where string, args []interface{}) {
	...
	if strings.TrimSpace(filter.ProjectID) == ProjectFilterUnbound {
		...
		conditions = append(conditions, `EXISTS (
			SELECT 1 FROM c2_sessions s
			JOIN c2_listeners l ON l.id = s.listener_id
			WHERE s.id = c2_tasks.session_id AND COALESCE(l.project_id, '') = $1
		)`)
		args = append(args, strings.TrimSpace(filter.ProjectID))
	}
	if filter.Status != "" {
	...
	if strings.TrimSpace(filter.TaskType) != "" {
		conditions = append(conditions, "task_type = $1")
		args = append(args, strings.TrimSpace(filter.TaskType))
	}
	if filter.Since != nil {
		conditions = append(conditions, sqliteEpochGE("created_at", ">=", len(args)+1))
...
func appendC2TaskAccessFilter(conditions *[]string, args *[]interface{}, access RBACListAccess) {
	...
	clauses := []string{fmt.Sprintf(`EXISTS (
		SELECT 1 FROM c2_sessions s
		JOIN c2_listeners l ON l.id = s.listener_id
		WHERE s.id = c2_tasks.session_id AND l.owner_user_id = $%d
	)`, len(*args)+1)}
	*args = append(*args, access.UserID)
	if access.Scope == RBACScopeAssigned {
		clauses = append(clauses, fmt.Sprintf(`EXISTS (
			...
			WHERE s.id = c2_tasks.session_id
				AND ra.user_id = $%d AND ra.resource_type = 'c2_listener'
		)`, len(*args)+1))
		*args = append(*args, access.UserID)
	}
	*conditions = append(*conditions, "("+strings.Join(clauses, " OR ")+")")
...
func (db *DB) ListC2Tasks(filter ListC2TasksFilter) ([]*C2Task, error) {
	...
	if limit > 0 {
		...
		query += fmt.Sprintf(` LIMIT $%d::bigint OFFSET $%d::bigint`, len(args)+1, len(args)+2)
...
func (db *DB) PopQueuedC2Tasks(sessionID string, limit int) ([]*C2Task, error) {
	...
	query := `
		SELECT id, session_id, task_type, COALESCE(payload_json, '{}'),
			...
			created_at
		FROM c2_tasks
		WHERE session_id = $1 AND (status = 'queued' AND (approval_status = '' OR approval_status = 'approved'))
	...
	for _, t := range list {
		if _, err := tx.Exec(
			`UPDATE c2_tasks SET status = 'sent', sent_at = $1 WHERE id = $2`, now, t.ID,
		); err != nil {
			return nil, err
		}
		t.Status = "sent"
...
func (db *DB) DeleteC2Task(id string) error {
	res, err := db.Exec(`DELETE FROM c2_tasks WHERE id = $1`, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
...
func (db *DB) ListC2FilesBySession(sessionID string) ([]*C2File, error) {
	query := `
		...
		FROM c2_files WHERE session_id = $1 ORDER BY created_at DESC
	`
...
func buildC2EventsWhere(filter ListC2EventsFilter) (where string, args []interface{}) {
	...
	if strings.TrimSpace(filter.ProjectID) == ProjectFilterUnbound {
		conditions = append(conditions, `(
			...
			OR EXISTS (
				SELECT 1 FROM c2_listeners l
				WHERE c2_events.data_json IS NOT NULL AND c2_events.data_json <> ''
					AND l.id = COALESCE(NULLIF(c2_events.data_json::jsonb ->> 'listener_id', ''), '')
					AND COALESCE(l.project_id, '') = ''
			)
		)`)
	} else if pid := strings.TrimSpace(filter.ProjectID); pid != "" {
		idx := len(args) + 1
		conditions = append(conditions, fmt.Sprintf(`(
			EXISTS (
				SELECT 1 FROM c2_sessions s
				JOIN c2_listeners l ON l.id = s.listener_id
				WHERE s.id = c2_events.session_id AND COALESCE(l.project_id, '') = $%d
			)
			OR EXISTS (
				SELECT 1 FROM c2_tasks t
				JOIN c2_sessions s ON s.id = t.session_id
				JOIN c2_listeners l ON l.id = s.listener_id
				WHERE t.id = c2_events.task_id AND COALESCE(l.project_id, '') = $%d
			)
			OR EXISTS (
				SELECT 1 FROM c2_listeners l
				WHERE c2_events.data_json IS NOT NULL AND c2_events.data_json <> ''
					AND l.id = COALESCE(NULLIF(c2_events.data_json::jsonb ->> 'listener_id', ''), '')
					AND COALESCE(l.project_id, '') = $%d
			)
		)`, idx, idx, idx))
...
func appendC2EventAccessFilter(conditions *[]string, args *[]interface{}, access RBACListAccess) {
	...
	clauses := []string{fmt.Sprintf(`EXISTS (
		SELECT 1 FROM c2_sessions s
		JOIN c2_listeners l ON l.id = s.listener_id
		WHERE s.id = c2_events.session_id AND l.owner_user_id = $%d
	)`, idx)}
	if access.Scope == RBACScopeAssigned {
		clauses = append(clauses, fmt.Sprintf(`EXISTS (
			SELECT 1 FROM c2_sessions s
	...
	clauses = append(clauses, fmt.Sprintf(`EXISTS (
		...
		JOIN c2_sessions s ON s.id = t.session_id
		JOIN c2_listeners l ON l.id = s.listener_id
		WHERE t.id = c2_events.task_id AND l.owner_user_id = $%d
	)`, idx))
	if access.Scope == RBACScopeAssigned {
		clauses = append(clauses, fmt.Sprintf(`EXISTS (
			SELECT 1 FROM c2_tasks t
...
func (db *DB) ListC2Events(filter ListC2EventsFilter) ([]*C2Event, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE `+where+`
		ORDER BY created_at DESC
		LIMIT $%d::bigint OFFSET $%d::bigint
	`, limitIdx, offsetIdx)
	args = append(args, limit, offset)
	rows, err := db.Query(query, args...)
	if err != nil {
...
func (db *DB) UpdateC2Profile(p *C2Profile) error {
	...
	resHdrJSON, _ := json.Marshal(p.ResponseHeaders)
	query := `
		UPDATE c2_profiles SET name = $1, user_agent = $2, uris_json = $3,
			request_headers_json = $4, response_headers_json = $5, body_template = $6,
			jitter_min_ms = $7, jitter_max_ms = $8
		WHERE id = $9
	`
...
func (db *DB) DeleteC2Profile(id string) error {
	if _, err := db.Exec(`UPDATE c2_listeners SET profile_id = '' WHERE profile_id = $1`, id); err != nil {
		return err
	}
	res, err := db.Exec(`DELETE FROM c2_profiles WHERE id = $1`, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
