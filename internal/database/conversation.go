...
func (db *DB) CreateConversationWithWebshell(webshellConnectionID, title string, meta ConversationCreateMeta) (*Conversation, error) {
	...
	case wsID != "" && projectID != "":
		_, err = db.Exec(
			"INSERT INTO conversations (id, title, created_at, updated_at, webshell_connection_id, project_id, role_name, agent_mode) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
			id, title, now, now, wsID, projectID, roleName, agentMode,
		)
	case wsID != "":
		_, err = db.Exec(
			"INSERT INTO conversations (id, title, created_at, updated_at, webshell_connection_id, role_name, agent_mode) VALUES ($1, $2, $3, $4, $5, $6, $7)",
			id, title, now, now, wsID, roleName, agentMode,
		)
	case projectID != "":
		_, err = db.Exec(
			"INSERT INTO conversations (id, title, created_at, updated_at, project_id, role_name, agent_mode) VALUES ($1, $2, $3, $4, $5, $6, $7)",
			id, title, now, now, projectID, roleName, agentMode,
		)
	default:
		_, err = db.Exec(
			"INSERT INTO conversations (id, title, created_at, updated_at, role_name, agent_mode) VALUES ($1, $2, $3, $4, $5, $6)",
...
func (db *DB) GetConversationByWebshellConnectionID(connectionID string) (*Conversation, error) {
	...
	var pinned int
	err := db.QueryRow(
		"SELECT id, title, pinned, created_at, updated_at FROM conversations WHERE webshell_connection_id = $1 ORDER BY updated_at DESC LIMIT 1",
		connectionID,
...
func (db *DB) ListConversationsByWebshellConnectionID(connectionID string) ([]WebShellConversationItem, error) {
	...
	rows, err := db.Query(
		"SELECT id, title, updated_at FROM conversations WHERE webshell_connection_id = $1 ORDER BY updated_at DESC",
		connectionID,
	)
	if err != nil {
...
func (db *DB) ConversationExists(id string) (bool, error) {
	...
	var one int
	err := db.QueryRow("SELECT 1 FROM conversations WHERE id = $1 LIMIT 1", id).Scan(&one)
	if err == sql.ErrNoRows {
		return false, nil
	}
	if err != nil {
...
func (db *DB) GetConversation(id string) (*Conversation, error) {
	...
	err := db.QueryRow(
		"SELECT id, title, pinned, created_at, updated_at, project_id, role_name, agent_mode FROM conversations WHERE id = $1",
		id,
...
func (db *DB) SetConversationRoleName(id, roleName string) error {
	...
	_, err := db.Exec(
		"UPDATE conversations SET role_name = $1, updated_at = $2 WHERE id = $3",
...
func appendConversationProjectFilter(where string, args []interface{}, projectID, alias string) (string, []interface{}) {
	...
	return where + fmt.Sprintf(" AND %s = $%d", col, len(args)+1), append(args, pid)
}
...
func appendConversationAccessFilter(where string, args []interface{}, userID, scope, alias string) (string, []interface{}) {
	...
	where += fmt.Sprintf(` AND (%[1]sowner_user_id = $%[2]d OR EXISTS (
		SELECT 1 FROM rbac_resource_assignments ra
		WHERE ra.user_id = $%[2]d AND ra.resource_type = 'conversation' AND ra.resource_id = %[1]sid
	) OR EXISTS (
		SELECT 1 FROM projects p
		WHERE p.id = %[1]sproject_id AND (
			p.owner_user_id = $%[2]d OR EXISTS (
				SELECT 1 FROM rbac_resource_assignments pra
				WHERE pra.user_id = $%[2]d AND pra.resource_type = 'project' AND pra.resource_id = p.id
			)
		)
	))`, prefix, len(args)+1)
...
func (db *DB) CountConversations(search, projectID string) (int, error) {
	...
	if search != "" {
		...
		where := ` WHERE (c.title ILIKE $1
			    OR EXISTS (SELECT 1 FROM messages m WHERE m.conversation_id = c.id AND m.content ILIKE $2))`
...
func (db *DB) ListConversations(limit, offset int, search, sortBy, projectID string) ([]*Conversation, error) {
	...
	if search != "" {
		...
		rows, err = db.Query(
			fmt.Sprintf(`SELECT c.id, c.title, COALESCE(c.pinned, 0), c.created_at, c.updated_at, c.project_id, c.role_name, c.agent_mode
			 FROM conversations c`+where+`
			 `+orderClause+`
			 LIMIT $%d::bigint OFFSET $%d::bigint`, len(args)+1, len(args)+2),
			args...,
		)
	} else {
		...
		args = append(args, limit, offset)
		rows, err = db.Query(
			fmt.Sprintf("SELECT id, title, COALESCE(pinned, 0), created_at, updated_at, project_id, role_name, agent_mode FROM conversations"+where+" "+orderClause+" LIMIT $%d::bigint OFFSET $%d::bigint", len(args)-1, len(args)),
			args...,
		)
	}
	...
	if search != "" {
		...
		rows, err = db.Query(
			`SELECT c.id, c.title, COALESCE(c.pinned, 0), c.created_at, c.updated_at, c.project_id, c.role_name, c.agent_mode
			 FROM conversations c`+where+`
			 `+orderClause+fmt.Sprintf(" LIMIT $%d::bigint OFFSET $%d::bigint", len(args)-1, len(args)), args...)
	} else {
		orderClause := conversationOrderClause(sortBy, "")
		where := ""
		args := []interface{}{}
...
func (db *DB) GetConversationTitle(id string) (string, error) {
	var title string
	err := db.QueryRow("SELECT title FROM conversations WHERE id = $1", id).Scan(&title)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (db *DB) DeleteConversation(id string) error {
	...
	_, err := db.Exec(`
		...
		SET conversation_tag = COALESCE(NULLIF(TRIM(conversation_tag), ''), (SELECT title FROM conversations WHERE id = $1))
	...
	_, err = db.Exec("DELETE FROM knowledge_retrieval_logs WHERE conversation_id = $1", id)
	if err != nil {
...
func (db *DB) SaveAgentTrace(conversationID, traceInputJSON, assistantOutput string) error {
	_, err := db.Exec(
		"UPDATE conversations SET last_react_input = $1, last_react_output = $2, updated_at = $3 WHERE id = $4",
...
func (db *DB) GetAgentTrace(conversationID string) (traceInputJSON, assistantOutput string, err error) {
	...
	err = db.QueryRow(
		"SELECT last_react_input, last_react_output FROM conversations WHERE id = $1",
		conversationID,
	).Scan(&input, &output)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (db *DB) ConversationHasToolProcessDetails(conversationID string) (bool, error) {
	var n int
	err := db.QueryRow(
		`SELECT COUNT(*) FROM process_details WHERE conversation_id = $1 AND event_type IN ('tool_call', 'tool_result')`,
		conversationID,
	).Scan(&n)
	if err != nil {
...
func (db *DB) AddMessage(conversationID, role, content string, mcpExecutionIDs []string) (*Message, error) {
	...
	_, err := db.Exec(
		"INSERT INTO messages (id, conversation_id, role, content, reasoning_content, mcp_execution_ids, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
...
func (db *DB) UpdateAssistantMessageFinalize(messageID, content string, mcpExecutionIDs []string, reasoningContent string) error {
	...
	_, err := db.Exec(
		"UPDATE messages SET content = $1, mcp_execution_ids = $2, reasoning_content = $3, updated_at = $4 WHERE id = $5",
...
func (db *DB) GetMessages(conversationID string) ([]Message, error) {
	rows, err := db.Query(
		"SELECT id, conversation_id, role, content, reasoning_content, mcp_execution_ids, created_at, updated_at FROM messages WHERE conversation_id = $1 ORDER BY created_at ASC, ctid ASC",
		conversationID,
	)
	if err != nil {
...
func (db *DB) GetMessagesLite(conversationID string) ([]Message, error) {
	rows, err := db.Query(
		"SELECT id, conversation_id, role, content, mcp_execution_ids, created_at, updated_at FROM messages WHERE conversation_id = $1 ORDER BY created_at ASC, ctid ASC",
		conversationID,
	)
	if err != nil {
		return nil, fmt.Errorf("查询消息失败: %w", err)
...
func (db *DB) DeleteConversationTurn(conversationID, anchorMessageID string) (deletedIDs []string, err error) {
	...
	ph := postgresPlaceholders(2, len(deletedIDs))
	...
	res, err := tx.Exec(
		"DELETE FROM messages WHERE conversation_id = $1 AND id IN ("+ph+")",
		args...,
	)
	if err != nil {
	...
	_, err = tx.Exec(
		`UPDATE conversations SET last_react_input = NULL, last_react_output = NULL, updated_at = $1 WHERE id = $2`,
...
WHERE m.conversation_id = $1 AND m.role = 'user'
  AND m.created_at <= COALESCE((SELECT created_at FROM messages WHERE id = $2 AND conversation_id = $3), m.created_at)
ORDER BY m.created_at DESC, m.ctid DESC
...
WHERE message_id = $1 AND event_type IN ('thinking', 'reasoning_chain', 'planning')
ORDER BY created_at ASC, ctid ASC`, assistantMessageID)
	if err != nil {
...
func (db *DB) AddProcessDetailWithID(messageID, conversationID, eventType, message string, data interface{}) (string, error) {
	...
	_, err := db.Exec(
		"INSERT INTO process_details (id, message_id, conversation_id, event_type, message, data, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
...
func (db *DB) UpdateProcessDetailContent(id, message string, data interface{}) error {
	...
	result, err := db.Exec(
		"UPDATE process_details SET message = $1, data = $2 WHERE id = $3",
...
func (db *DB) DeleteProcessDetail(id string) error {
	_, err := db.Exec("DELETE FROM process_details WHERE id = $1", strings.TrimSpace(id))
	if err != nil {
...
func (db *DB) GetProcessDetails(messageID string) ([]ProcessDetail, error) {
	rows, err := db.Query(
		"SELECT id, message_id, conversation_id, event_type, message, data, created_at FROM process_details WHERE message_id = $1 ORDER BY created_at ASC, ctid ASC",
		messageID,
	)
	if err != nil {
...
func (db *DB) GetProcessDetailByID(id string) (*ProcessDetail, error) {
	...
	var createdAt string
	err := db.QueryRow(
		"SELECT id, message_id, conversation_id, event_type, message, data, created_at FROM process_details WHERE id = $1",
		id,
...
func (db *DB) GetProcessDetailsSummary(messageID string) (*ProcessDetailsSummary, error) {
	var total int
	if err := db.QueryRow(
		"SELECT COUNT(*) FROM process_details WHERE message_id = $1",
		messageID,
	).Scan(&total); err != nil {
	...
	if err := db.QueryRow(
		"SELECT created_at, updated_at, content FROM messages WHERE id = $1",
		messageID,
...
WHERE message_id = $1 AND event_type IN ('cancelled', 'timeout', 'error')
...
LIMIT 1`, messageID).Scan(&terminalEvent, &terminalCreatedAt)
	...
	if err := db.QueryRow(
		"SELECT COUNT(*) FROM process_details WHERE message_id = $1 AND event_type = 'tool_call'",
		messageID,
	...
	execRows, err := db.Query(
		"SELECT id, event_type, data FROM process_details WHERE message_id = $1 AND event_type IN ('tool_call', 'tool_result') ORDER BY created_at ASC, ctid ASC",
		messageID,
	)
	if err != nil {
	...
	rows, err := db.Query(
		"SELECT data FROM process_details WHERE message_id = $1 AND event_type = 'iteration' ORDER BY created_at ASC, ctid ASC",
		messageID,
	)
	if err != nil {
...
func (db *DB) applyPersistedToolExecutionStatuses(executions []ProcessDetailsToolExecution) {
	for i := range executions {
		...
		var status string
		if err := db.QueryRow(`SELECT status FROM tool_executions WHERE id = $1`, execID).Scan(&status); err != nil {
			continue
		}
...
func (db *DB) GetProcessDetailsPage(messageID string, limit, offset int) ([]ProcessDetail, int, error) {
	...
	rows, err := db.Query(
		"SELECT id, message_id, conversation_id, event_type, message, data, created_at FROM process_details WHERE message_id = $1 ORDER BY created_at ASC, ctid ASC LIMIT $2::bigint OFFSET $3::bigint",
...
func (db *DB) GetProcessDetailOffset(messageID, detailID string) (int, error) {
	...
	var createdAt string
	// PG 无 ctid；用 ctid 作为插入序的稳定并列裁决（与 SQLite ctid 语义等价）
	var ctidText string
	if err := db.QueryRow(
		"SELECT created_at, ctid::text FROM process_details WHERE message_id = $1 AND id = $2",
		messageID, detailID,
	).Scan(&createdAt, &ctidText); err != nil {
		if err == sql.ErrNoRows {
	...
	if err := db.QueryRow(
		`SELECT COUNT(*) FROM process_details
		 WHERE message_id = $1
		   AND (created_at < $2 OR (created_at = $3 AND ctid < $4::tid))`,
		messageID, createdAt, createdAt, ctidText,
	).Scan(&offset); err != nil {
...
func (db *DB) GetProcessDetailsByConversation(conversationID string) (map[string][]ProcessDetail, error) {
	rows, err := db.Query(
		"SELECT id, message_id, conversation_id, event_type, message, data, created_at FROM process_details WHERE conversation_id = $1 ORDER BY created_at ASC, ctid ASC",
		conversationID,
	)
	if err != nil {
		return nil, fmt.Errorf("查询过程详情失败: %w", err)
