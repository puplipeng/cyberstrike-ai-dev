...
func TestAssetStatsPostgresPeriodsScopesAndUTCBoundaries(t *testing.T) {
	db, err := NewDB(testpostgres.DSN(t), zap.NewNop())
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	for _, id := range []string{"stats-owner", "stats-other"} {
		if _, err := db.Exec(`INSERT INTO rbac_users (id,username,display_name,password_hash,enabled,is_builtin,created_at,updated_at) VALUES ($1,$1,$1,'unused',1,0,$2,$2)`, id, time.Now()); err != nil {
			t.Fatal(err)
		}
	}
	assets := []*Asset{{Domain: "own.example.com"}, {Domain: "assigned.example.com"}, {Domain: "foreign.example.com"}}
	if _, err := db.UpsertAssets(assets, "", true); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`UPDATE assets SET owner_user_id='stats-other'`); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`UPDATE assets SET owner_user_id='stats-owner' WHERE id=$1`, assets[0].ID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`INSERT INTO rbac_resource_assignments (id,user_id,resource_type,resource_id,created_at) VALUES ('stats-assignment','stats-owner','asset',$1,$2)`, assets[1].ID, time.Now()); err != nil {
		t.Fatal(err)
	}
	for _, days := range []int{7, 30, 90} {
		t.Run(fmt.Sprint(days), func(t *testing.T) {
			...
			// The instant is midnight UTC, represented with an explicit +08:00 offset.
			first := start.In(time.FixedZone("fixture", 8*60*60)).Format(time.RFC3339)
			if _, err := db.Exec(`UPDATE assets SET first_seen_at=$1,last_seen_at=$2,last_scan_at=''`, first, time.Now()); err != nil {
				t.Fatal(err)
			}
			if _, err := db.Exec(`UPDATE assets SET last_scan_at=$1 WHERE id=$2`, time.Now(), assets[0].ID); err != nil {
				t.Fatal(err)
			}
			if _, err := db.Exec(`UPDATE assets SET first_seen_at=$1,last_scan_at=NULL WHERE id=$2`, start.Add(-time.Second), assets[2].ID); err != nil {
				t.Fatal(err)
			}
			if _, err := db.Exec(`DELETE FROM vulnerabilities`); err != nil {
				t.Fatal(err)
			}
			if _, err := db.Exec(`INSERT INTO vulnerabilities (id,title,severity,created_at,owner_user_id) VALUES ('stats-own','own','high',$1,'stats-owner'),('stats-foreign','foreign','high',$1,'stats-other')`, first); err != nil {
				t.Fatal(err)
			}
			for _, scope := range []string{RBACScopeOwn, RBACScopeAssigned, RBACScopeAll} {
				stats, err := db.GetAssetStats(RBACListAccess{UserID: "stats-owner", Scope: scope}, days)
				if err != nil {
					t.Fatalf("scope %s: %v", scope, err)
				}
				wantTotal, wantRisks, wantNever := 2, 1, 1
				if scope == RBACScopeAll {
					wantTotal, wantRisks, wantNever = 3, 2, 2
				}
				if stats["total"] != wantTotal {
					t.Fatalf("scope %s total=%v", scope, stats["total"])
				}
				trend := stats["asset_trend"].([]map[string]interface{})
				if len(trend) != days || trend[0]["date"] != start.Format("2006-01-02") || trend[0]["added"] != 2 {
					t.Fatalf("scope %s bad UTC boundary: %#v", scope, trend[0])
				}
				risks := stats["risk_trend"].([]map[string]interface{})
				if len(risks) != days || risks[0]["high_risk"] != wantRisks {
					t.Fatalf("scope %s risk boundary: %#v", scope, risks[0])
				}
				coverage := stats["coverage"].(map[string]interface{})
				if coverage["scanned"] != 1 || coverage["never_scanned"] != wantNever {
					t.Fatalf("scope %s empty/null scan times: %#v", scope, coverage)
				}
			}
		})
	}
...
func TestReconAssetIgnoresInvalidOptionalStructuredFields(t *testing.T) {
	db, err := NewDB(testpostgres.DSN(t), zap.NewNop())
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, source := range []string{"fofa", "quake", "zoomeye", "shodan"} {
		ip := fmt.Sprintf("203.0.113.%d", 59+i)
		asset := &Asset{
			Host:     "https://" + ip + ":8443",
			IP:       ip,
		...
		result, err := db.UpsertAssets([]*Asset{asset}, "")
		if err != nil || result.Created != 1 {
			t.Fatalf("%s asset with dirty optional domain was not created: result=%#v err=%v", source, result, err)
		}
		if asset.Domain != "" || asset.IP != ip {
			t.Fatalf("%s structured fields were not sanitized: %#v", source, asset)
		}
	}
}
...
func TestAssetAccessFiltersOwners(t *testing.T) {
	...
	defer db.Close()
	now := time.Now()
	if _, err := db.Exec(`INSERT INTO rbac_users (id,username,display_name,password_hash,enabled,is_builtin,created_at,updated_at) VALUES ('user-a','user-a','User A','hash',1,0,$1,$2)`, now, now); err != nil {
		t.Fatal(err)
	}
...
func TestAssetListFlexibleFiltersAndOldestScanPagination(t *testing.T) {
	...
	if _, err := db.Exec(`UPDATE assets SET last_scan_at=$1 WHERE id=$2`, old, assets[0].ID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`UPDATE assets SET last_scan_at=$1 WHERE id=$2`, recent, assets[1].ID); err != nil {
		t.Fatal(err)
	}
