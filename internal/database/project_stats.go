...
func (db *DB) GetProjectStatsCounts(projectID string) (*ProjectStats, error) {
	...
	stats := &ProjectStats{}
	if err := db.QueryRow(
		`SELECT COUNT(*) FROM project_facts WHERE project_id = $1 AND confidence != 'deprecated'`,
		projectID,
	).Scan(&stats.FactCount); err != nil {
		return nil, fmt.Errorf("统计事实失败: %w", err)
	}
	if err := db.QueryRow(
		`SELECT COUNT(*) FROM vulnerabilities WHERE project_id = $1`,
		projectID,
...
func (db *DB) ListProjectFactsForSparseCheck(projectID string) ([]struct {
	...
	rows, err := db.Query(
		`SELECT category, fact_key, COALESCE(body,'') FROM project_facts WHERE project_id = $1 AND confidence != 'deprecated'`,
		projectID,
	)
	if err != nil {
		return nil, err
...
func (db *DB) ListConversationsByProjectID(projectID string, limit, offset int) ([]*Conversation, error) {
	...
	rows, err := db.Query(
		`SELECT id, title, COALESCE(pinned, 0), created_at, updated_at, project_id, role_name
		 FROM conversations WHERE project_id = $1 ORDER BY updated_at DESC LIMIT $2::bigint OFFSET $3::bigint`,
...
func (db *DB) CountConversationsByProjectID(projectID string) (int, error) {
	var n int
	err := db.QueryRow(`SELECT COUNT(*) FROM conversations WHERE project_id = $1`, projectID).Scan(&n)
	return n, err
}
