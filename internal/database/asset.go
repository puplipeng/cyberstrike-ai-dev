...
func normalizeAsset(a *Asset) {
	...
	if isReconAssetSource(a.Source) {
...
// Recon providers expose optional fields with different shapes. For example,
// Quake may return an array-like domain value while Shodan can return several
// hostnames. A valid host should still be importable when one of those optional
// enrichments cannot be normalized into the asset schema.
func isReconAssetSource(source string) bool {
	switch strings.ToLower(strings.TrimSpace(source)) {
	case "fofa", "quake", "zoomeye", "shodan":
		return true
	default:
		return false
	}
...
func appendAssetAccess(query string, args []interface{}, access RBACListAccess, alias string) (string, []interface{}) {
	...
	query += fmt.Sprintf(` AND (`+prefix+`owner_user_id = $%[1]d OR EXISTS (
		SELECT 1 FROM rbac_resource_assignments ra
		WHERE ra.user_id = $%[1]d AND ra.resource_type = 'asset' AND ra.resource_id = `+prefix+`id
	) OR (`+prefix+`project_id IS NOT NULL AND `+prefix+`project_id <> '' AND (
		EXISTS (SELECT 1 FROM projects ap WHERE ap.id=`+prefix+`project_id AND ap.owner_user_id=$%[1]d)
		OR EXISTS (SELECT 1 FROM rbac_resource_assignments pra WHERE pra.user_id=$%[1]d AND pra.resource_type='project' AND pra.resource_id=`+prefix+`project_id)
	)))`, len(args)+1)
	return query, append(args, access.UserID)
}
...
func (db *DB) UpsertAssets(assets []*Asset, ownerUserID string, allowGlobal ...bool) (AssetImportResult, error) {
	...
	for _, asset := range assets {
		...
		err := tx.QueryRow(`SELECT id,owner_user_id FROM assets WHERE dedup_key = $1`, key).Scan(&existingID, &existingOwner)
		...
		if err == sql.ErrNoRows {
			...
			if ownerUserID != "" {
				if _, err := tx.Exec(`INSERT INTO rbac_resource_assignments (id,user_id,resource_type,resource_id,created_at) SELECT $1,id,$2,$3,$4 FROM rbac_users WHERE id=$5 ON CONFLICT DO NOTHING`, uuid.NewString(), "asset", asset.ID, now, ownerUserID); err != nil {
		...
		_, err = tx.Exec(`UPDATE assets SET
			host=CASE WHEN $1<>'' THEN $2 ELSE host END, ip=CASE WHEN $3<>'' THEN $4 ELSE ip END,
			domain=CASE WHEN $5<>'' THEN $6 ELSE domain END, protocol=CASE WHEN $7<>'' THEN $8 ELSE protocol END,
			title=CASE WHEN $9<>'' THEN $10 ELSE title END, server=CASE WHEN $11<>'' THEN $12 ELSE server END,
			country=CASE WHEN $13<>'' THEN $14 ELSE country END, province=CASE WHEN $15<>'' THEN $16 ELSE province END,
			city=CASE WHEN $17<>'' THEN $18 ELSE city END, source=CASE WHEN $19<>'' THEN $20 ELSE source END,
			source_query=CASE WHEN $21<>'' THEN $22 ELSE source_query END, project_id=CASE WHEN $23<>'' THEN $24 ELSE project_id END,
			responsible_person=CASE WHEN $25<>'' THEN $26 ELSE responsible_person END,
			department=CASE WHEN $27<>'' THEN $28 ELSE department END,
			business_system=CASE WHEN $29<>'' THEN $30 ELSE business_system END,
			environment=CASE WHEN $31<>'' THEN $32 ELSE environment END,
			criticality=CASE WHEN $33<>'' THEN $34 ELSE criticality END,
			tags_json=CASE WHEN $35<>'[]' THEN $36 ELSE tags_json END,
			last_seen_at=$37, updated_at=$38 WHERE id=$39`,
		...
		if ownerUserID != "" && (!existingOwner.Valid || strings.TrimSpace(existingOwner.String) == ownerUserID) {
			if _, err := tx.Exec(`INSERT INTO rbac_resource_assignments (id,user_id,resource_type,resource_id,created_at) SELECT $1,id,$2,$3,$4 FROM rbac_users WHERE id=$5 ON CONFLICT DO NOTHING`, uuid.NewString(), "asset", existingID, now, ownerUserID); err != nil {
...
func assetWhere(filter AssetListFilter, access RBACListAccess) (string, []interface{}) {
	...
	if q := strings.TrimSpace(filter.Search); q != "" {
		...
		query += fmt.Sprintf(` AND (LOWER(assets.host) ILIKE $%d ESCAPE '\' OR LOWER(assets.ip) ILIKE $%d ESCAPE '\' OR LOWER(assets.domain) ILIKE $%d ESCAPE '\'
			OR LOWER(assets.title) ILIKE $%d ESCAPE '\' OR LOWER(assets.server) ILIKE $%d ESCAPE '\' OR LOWER(assets.tags_json) ILIKE $%d ESCAPE '\'
			OR LOWER(assets.responsible_person) ILIKE $%d ESCAPE '\' OR LOWER(assets.department) ILIKE $%d ESCAPE '\' OR LOWER(assets.business_system) ILIKE $%d ESCAPE '\')`, len(args)+1, len(args)+2, len(args)+3, len(args)+4, len(args)+5, len(args)+6, len(args)+7, len(args)+8, len(args)+9)
		for i := 0; i < 9; i++ {
			args = append(args, pattern)
		}
	}
	if filter.Status != "" {
		query += fmt.Sprintf(" AND assets.status = $%d", len(args)+1)
		args = append(args, filter.Status)
	}
	if filter.Protocol != "" {
		query += fmt.Sprintf(" AND assets.protocol = $%d", len(args)+1)
		args = append(args, filter.Protocol)
	}
	if filter.ProjectID != "" {
		query += fmt.Sprintf(" AND assets.project_id = $%d", len(args)+1)
		args = append(args, filter.ProjectID)
	}
	if filter.Source != "" {
		query += fmt.Sprintf(" AND LOWER(assets.source) = LOWER($%d)", len(args)+1)
	...
	if tag := strings.TrimSpace(filter.Tag); tag != "" {
		...
		query += fmt.Sprintf(` AND LOWER(assets.tags_json) ILIKE $%d ESCAPE '\'`, len(args)+1)
		args = append(args, pattern)
	}
	if filter.Host != "" {
		query += fmt.Sprintf(" AND LOWER(assets.host) = LOWER($%d)", len(args)+1)
		args = append(args, strings.TrimSpace(filter.Host))
	}
	if filter.IP != "" {
		query += fmt.Sprintf(" AND LOWER(assets.ip) = LOWER($%d)", len(args)+1)
		args = append(args, strings.TrimSpace(filter.IP))
	}
	if filter.Domain != "" {
		query += fmt.Sprintf(" AND LOWER(assets.domain) = LOWER($%d)", len(args)+1)
		args = append(args, strings.TrimSpace(filter.Domain))
	}
	if filter.Port != nil {
		query += fmt.Sprintf(" AND assets.port = $%d", len(args)+1)
		args = append(args, *filter.Port)
	}
	if filter.RiskLevel != "" {
		query += fmt.Sprintf(" AND %s = $%d", assetRiskLevelCachedExpr, len(args)+1)
		args = append(args, strings.ToLower(strings.TrimSpace(filter.RiskLevel)))
	}
	if filter.MinVulnerabilities != nil {
		query += fmt.Sprintf(" AND %s >= $%d", assetVulnerabilityCountCachedExpr, len(args)+1)
	...
	for _, item := range []struct {
		...
		if strings.TrimSpace(item.value) != "" {
			query += fmt.Sprintf(" AND LOWER(%s) = LOWER($%d)", item.column, len(args)+1)
	...
	if filter.ScanOverdueDays != nil {
		query += fmt.Sprintf(" AND (NULLIF(%s,'') IS NULL OR NULLIF(%s,'')::timestamptz < now() - ($%d::integer * INTERVAL '1 day'))", assetEffectiveLastScanExpr, assetEffectiveLastScanExpr, len(args)+1)
		args = append(args, *filter.ScanOverdueDays)
	}
	if filter.LastScanBefore != nil {
		query += fmt.Sprintf(" AND NULLIF(%s,'')::timestamptz < $%d::timestamptz", assetEffectiveLastScanExpr, len(args)+1)
	...
	if filter.FirstSeenBefore != nil {
		query += fmt.Sprintf(" AND assets.first_seen_at < $%d", len(args)+1)
	...
	if filter.LastSeenBefore != nil {
		query += fmt.Sprintf(" AND assets.last_seen_at < $%d", len(args)+1)
...
func (db *DB) MarkAssetScanned(id, conversationID, queueID, taskID string, access RBACListAccess) error {
	args := []interface{}{time.Now(), strings.TrimSpace(conversationID), strings.TrimSpace(queueID), strings.TrimSpace(taskID), time.Now()}
	query, args := appendAssetAccess(`UPDATE assets SET last_scan_at=$1,last_scan_conversation_id=$2,last_scan_queue_id=$3,last_scan_task_id=$4,updated_at=$5`+fmt.Sprintf(" WHERE id = $%d", len(args)+1), append(args, strings.TrimSpace(id)), access, "assets")
...
func (db *DB) CompleteAssetScan(id, conversationID string, access RBACListAccess) error {
	...
	now := time.Now()
	args := []interface{}{now, conversationID, conversationID, conversationID, now}
	query, args := appendAssetAccess(`UPDATE assets SET
		last_scan_at=$1,last_scan_conversation_id=$2,
		last_scan_queue_id=CASE WHEN EXISTS (SELECT 1 FROM batch_tasks bt WHERE bt.id=assets.last_scan_task_id AND bt.conversation_id=$3) THEN last_scan_queue_id ELSE '' END,
		last_scan_task_id=CASE WHEN EXISTS (SELECT 1 FROM batch_tasks bt WHERE bt.id=assets.last_scan_task_id AND bt.conversation_id=$4) THEN last_scan_task_id ELSE '' END,
		updated_at=$5`+fmt.Sprintf(" WHERE id = $%d", len(args)+1), append(args, id), access, "assets")
	res, err := db.Exec(query, args...)
	if err != nil {
		return err
	}
...
func (db *DB) BatchTaskBelongsToQueue(taskID, queueID string) bool {
	var count int
	err := db.QueryRow(`SELECT COUNT(*) FROM batch_tasks WHERE id=$1 AND queue_id=$2`, strings.TrimSpace(taskID), strings.TrimSpace(queueID)).Scan(&count)
...
func (db *DB) RefreshAssetRiskCache(assetID string) error {
	...
	var count int
	if err := db.QueryRow("SELECT "+assetVulnerabilityCountExpr+" FROM assets WHERE assets.id=$1", assetID).Scan(&count); err != nil {
		if err == sql.ErrNoRows {
			return nil
		}
	...
	var score int
	if err := db.QueryRow("SELECT "+assetRiskScoreQueryExpr+" FROM assets WHERE assets.id=$1", assetID).Scan(&score); err != nil {
		return fmt.Errorf("刷新资产风险分数失败: %w", err)
	}
	var lastScan interface{}
	if err := db.QueryRow("SELECT "+assetEffectiveLastScanExpr+" FROM assets WHERE assets.id=$1", assetID).Scan(&lastScan); err != nil {
		return fmt.Errorf("刷新资产扫描状态失败: %w", err)
	}
	level := assetRiskLevelFromScore(score, lastScan != nil)
	if _, err := db.Exec(`UPDATE assets SET vulnerability_count=$1, risk_score=$2, risk_level=$3 WHERE id=$4`, count, score, level, assetID); err != nil {
...
func (db *DB) AssetIDsForVulnerabilityConversations(conversationIDs []string) ([]string, error) {
	...
	placeholders := postgresPlaceholders(1, len(cleaned))
...
func (db *DB) ListAssets(limit, offset int, filter AssetListFilter, access RBACListAccess) ([]*Asset, int, error) {
	...
	rows, err := db.Query(fmt.Sprintf("SELECT "+assetSelectColumns+" FROM assets LEFT JOIN projects p ON p.id=assets.project_id"+where+" ORDER BY "+orderBy+" LIMIT $%d::bigint OFFSET $%d::bigint", len(args)+1, len(args)+2), append(args, limit, offset)...)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (db *DB) GetAsset(id string, access RBACListAccess) (*Asset, error) {
	query, args := appendAssetAccess("SELECT "+assetSelectColumns+" FROM assets LEFT JOIN projects p ON p.id=assets.project_id WHERE assets.id = $1", []interface{}{id}, access, "assets")
...
func (db *DB) UpdateAsset(id string, a *Asset, access RBACListAccess) error {
	...
	args := []interface{}{key, nullIfEmpty(a.ProjectID), a.Host, a.IP, a.Port, a.Domain, a.Protocol, a.Title, a.Server, a.Country, a.Province, a.City,
		a.ResponsiblePerson, a.Department, a.BusinessSystem, a.Environment, a.Criticality, a.Source, a.SourceQuery, a.Status, string(tags), time.Now()}
	query, args := appendAssetAccess(`UPDATE assets SET dedup_key=$1,project_id=$2,host=$3,ip=$4,port=$5,domain=$6,protocol=$7,title=$8,server=$9,country=$10,province=$11,city=$12,
		responsible_person=$13,department=$14,business_system=$15,environment=$16,criticality=$17,source=$18,source_query=$19,status=$20,tags_json=$21,updated_at=$22`+fmt.Sprintf(" WHERE id = $%d", len(args)+1), append(args, id), access, "assets")
	res, err := db.Exec(query, args...)
	if err != nil {
		return err
	}
...
func (db *DB) UpdateAssetsBulk(ids []string, patch AssetBulkPatch, access RBACListAccess) (int, error) {
	...
	defer tx.Rollback()
	placeholders := postgresPlaceholders(1, len(unique))
	...
	for _, id := range unique {
		var rawTags string
		if err := tx.QueryRow("SELECT tags_json FROM assets WHERE id=$1", id).Scan(&rawTags); err != nil {
			return 0, err
		}
		tags := []string{}
		...
		_, err := tx.Exec(`UPDATE assets SET
			...
			responsible_person=CASE WHEN $3::bigint <> 0 THEN $4 ELSE responsible_person END,
			department=CASE WHEN $5::bigint <> 0 THEN $6 ELSE department END,
			business_system=CASE WHEN $7::bigint <> 0 THEN $8 ELSE business_system END,
			environment=CASE WHEN $9::bigint <> 0 THEN $10 ELSE environment END,
			criticality=CASE WHEN $11::bigint <> 0 THEN $12 ELSE criticality END,
			tags_json=$13,updated_at=$14 WHERE id=$15`,
...
func (db *DB) MergeAssets(primary *Asset, duplicateIDs []string, writeAccess, deleteAccess RBACListAccess) (int, error) {
	...
	defer tx.Rollback()
	primaryQuery, primaryArgs := appendAssetAccess("SELECT COUNT(*) FROM assets WHERE id=$1", []interface{}{primary.ID}, writeAccess, "assets")
	var primaryCount int
	if err := tx.QueryRow(primaryQuery, primaryArgs...).Scan(&primaryCount); err != nil || primaryCount != 1 {
		return 0, fmt.Errorf("主资产不存在或无权更新")
	}
	placeholders := postgresPlaceholders(1, len(duplicates))
	...
	updateQuery, updateScopeArgs := appendAssetAccess(`UPDATE assets SET dedup_key=$1,project_id=$2,host=$3,ip=$4,port=$5,domain=$6,protocol=$7,title=$8,server=$9,country=$10,province=$11,city=$12,
		responsible_person=$13,department=$14,business_system=$15,environment=$16,criticality=$17,source=$18,source_query=$19,status=$20,tags_json=$21,updated_at=$22 WHERE id=$23`,
...
func (db *DB) UpdateAssetsProject(ids []string, projectID string, access RBACListAccess) (int, error) {
	...
	updateQuery, updateArgs := appendAssetAccess("UPDATE assets SET project_id=$1,updated_at=$2 WHERE id IN ("+postgresPlaceholders(3, len(unique))+")", updateArgs, access, "assets")
...
func (db *DB) DeleteAsset(id string, access RBACListAccess) error {
	where, args := appendAssetAccess(" WHERE id = $1", []interface{}{id}, access, "assets")
...
func (db *DB) GetAssetStats(access RBACListAccess, requestedDays ...int) (map[string]interface{}, error) {
	...
	row := db.QueryRow(`SELECT COUNT(*),COUNT(DISTINCT NULLIF(ip,'')),COUNT(DISTINCT NULLIF(domain,'')),
		...
		COALESCE(SUM(CASE WHEN NULLIF(last_seen_at,'')::timestamptz >= now() - INTERVAL '7 days' THEN 1 ELSE 0 END),0) FROM assets`+where, args...)
	...
	coverageRow := db.QueryRow(`SELECT
		COALESCE(SUM(CASE WHEN NULLIF(last_scan_at,'') IS NOT NULL THEN 1 ELSE 0 END),0),
		COALESCE(SUM(CASE WHEN NULLIF(last_scan_at,'')::timestamptz >= now() - INTERVAL '7 days' THEN 1 ELSE 0 END),0),
		COALESCE(SUM(CASE WHEN NULLIF(last_scan_at,'')::timestamptz >= now() - INTERVAL '30 days' THEN 1 ELSE 0 END),0),
		COALESCE(SUM(CASE WHEN NULLIF(last_scan_at,'') IS NULL THEN 1 ELSE 0 END),0),
		COALESCE(SUM(CASE WHEN NULLIF(last_scan_at,'') IS NOT NULL AND NULLIF(last_scan_at,'')::timestamptz < now() - INTERVAL '30 days' THEN 1 ELSE 0 END),0),
	...
	trendWhere, trendArgs := appendAssetAccess(" WHERE NULLIF(first_seen_at,'')::timestamptz >= $1::timestamptz AND NULLIF(first_seen_at,'')::timestamptz < $2::timestamptz", []interface{}{start, end}, access, "assets")
	trendRows, err := db.Query(`SELECT to_char(NULLIF(first_seen_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD'), COUNT(*)
		FROM assets`+trendWhere+` GROUP BY to_char(NULLIF(first_seen_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD') ORDER BY to_char(NULLIF(first_seen_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD')`, trendArgs...)
	if err != nil {
		return nil, err
	}
	for trendRows.Next() {
	...
	inactiveWhere, inactiveArgs := appendAssetAccess(" WHERE status='inactive' AND NULLIF(updated_at,'')::timestamptz >= $1::timestamptz AND NULLIF(updated_at,'')::timestamptz < $2::timestamptz", []interface{}{start, end}, access, "assets")
	inactiveRows, err := db.Query(`SELECT to_char(NULLIF(updated_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD'), COUNT(*) FROM assets`+inactiveWhere+` GROUP BY to_char(NULLIF(updated_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD') ORDER BY to_char(NULLIF(updated_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD')`, inactiveArgs...)
	if err != nil {
		return nil, err
	}
	...
	riskWhere, riskArgs := appendVulnerabilityAccessFilter(" WHERE NULLIF(created_at,'')::timestamptz >= $1::timestamptz AND NULLIF(created_at,'')::timestamptz < $2::timestamptz", []interface{}{start, end}, access)
	riskRows, err := db.Query(`SELECT to_char(NULLIF(created_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD'), COUNT(*),
		COALESCE(SUM(CASE WHEN LOWER(severity) IN ('critical','high') THEN 1 ELSE 0 END),0)
		FROM vulnerabilities`+riskWhere+` GROUP BY to_char(NULLIF(created_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD') ORDER BY to_char(NULLIF(created_at,'')::timestamptz AT TIME ZONE 'UTC','YYYY-MM-DD')`, riskArgs...)
	if err != nil {
		return nil, err
	}
	for riskRows.Next() {
