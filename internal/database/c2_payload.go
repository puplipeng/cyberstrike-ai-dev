...
func (db *DB) UserCanAccessC2Payload(userID, scope, filename string) bool {
	...
	if err := db.QueryRow(`SELECT listener_id, owner_user_id FROM c2_payload_artifacts WHERE filename = $1`, strings.TrimSpace(filename)).Scan(&listenerID, &ownerUserID); err != nil {
		return false
	}
