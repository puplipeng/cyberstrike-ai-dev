...
func buildAuditLogsWhere(filter ListAuditLogsFilter) (string, []interface{}) {
	...
	args := []interface{}{}
	if filter.Actor != "" {
		conditions = append(conditions, fmt.Sprintf("actor = $%d", len(args)+1))
	...
	if filter.Result != "" {
		conditions = append(conditions, fmt.Sprintf("result = $%d", len(args)+1))
	...
	if relatedUserID := strings.TrimSpace(filter.RelatedUserID); relatedUserID != "" {
		conditions = append(conditions, fmt.Sprintf("(resource_id = $%d OR detail_json ILIKE $%d OR detail_json ILIKE $%d)", len(args)+1, len(args)+2, len(args)+3))
	...
	if q := strings.TrimSpace(filter.Query); q != "" {
		like := "%" + q + "%"
		conditions = append(conditions, fmt.Sprintf("(message LIKE $%d OR resource_id LIKE $%d OR action LIKE $%d OR category LIKE $%d OR detail_json LIKE $%d)", len(args)+1, len(args)+2, len(args)+3, len(args)+4, len(args)+5))
...
func (db *DB) ListAuditLogs(filter ListAuditLogsFilter) ([]*AuditLog, error) {
	...
	` + fmt.Sprintf(" LIMIT $%d::bigint OFFSET $%d::bigint", len(args)+1, len(args)+2)
...
func (db *DB) DeleteAuditLogsBefore(cutoff time.Time) (int64, error) {
	res, err := db.Exec(`DELETE FROM audit_logs WHERE `+sqliteEpochGE("created_at", "<", 1), formatSQLiteUTC(cutoff))
	if err != nil {
		return 0, err
	}
