...
func (db *DB) SaveToolExecution(exec *mcp.ToolExecution) error {
	// PostgreSQL TEXT requires UTF-8. Native Windows tools can still emit bytes in
	// an OEM/legacy code page; keep the execution record instead of losing it.
	safeError := strings.ToValidUTF8(exec.Error, "\uFFFD")
	safePartialOutput := strings.ToValidUTF8(exec.PartialOutput, "\uFFFD")
	...
	if safeError != "" {
		errorText = sql.NullString{String: safeError, Valid: true}
	}
	...
	query := `
		...
		ON CONFLICT (id) DO UPDATE SET
			tool_name = excluded.tool_name, arguments = excluded.arguments, status = excluded.status,
			result = excluded.result, error = excluded.error, start_time = excluded.start_time, end_time = excluded.end_time,
			duration_ms = excluded.duration_ms, partial_output = excluded.partial_output, partial_output_bytes = excluded.partial_output_bytes,
			partial_output_truncated = excluded.partial_output_truncated, partial_output_updated_at = excluded.partial_output_updated_at,
			owner_user_id = excluded.owner_user_id, conversation_id = excluded.conversation_id, created_at = excluded.created_at
	`
	...
	_, err = db.Exec(query,
		...
		endTime,
		durationMs,
		sqlNullString(safePartialOutput),
...
func (db *DB) UpdateToolExecutionResult(id string, result *mcp.ToolResult) error {
	...
	_, err = db.Exec(`UPDATE tool_executions SET result = $1 WHERE id = $2`, string(resultBytes), id)
	if err != nil {
...
func (db *DB) CountToolExecutionsForAccess(status, toolName string, access RBACListAccess) (int, error) {
	...
	if toolName != "" {
		// 支持部分匹配（模糊搜索），不区分大小写
		conditions = append(conditions, "LOWER(tool_name) LIKE $1")
...
func (db *DB) LoadToolExecutionsWithPagination(offset, limit int, status, toolName string) ([]*mcp.ToolExecution, error) {
	...
	query += fmt.Sprintf(` ORDER BY start_time DESC LIMIT $%d::bigint OFFSET $%d::bigint`, len(args)+1, len(args)+2)
...
func (db *DB) LoadToolStatsSummaryForAccess(topN int, access RBACListAccess) (*ToolStatsSummaryResult, error) {
	...
	rows, err := db.Query(`SELECT tool_name, COUNT(*),
		...
		fromSQL+fmt.Sprintf(` GROUP BY tool_name ORDER BY COUNT(*) DESC, tool_name ASC LIMIT $%d::bigint`, len(args)+1), append(args, topN)...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func appendToolExecutionAccessSQL(query string, args []interface{}, access RBACListAccess, hasWhere bool) (string, []interface{}) {
	...
	query += joiner + fmt.Sprintf(`(
		owner_user_id = $%d
		OR (conversation_id IS NOT NULL AND conversation_id <> '' AND (
			EXISTS (SELECT 1 FROM conversations c WHERE c.id = tool_executions.conversation_id AND c.owner_user_id = $%d)
			OR EXISTS (SELECT 1 FROM rbac_resource_assignments ra WHERE ra.user_id = $%d AND ra.resource_type = 'conversation' AND ra.resource_id = tool_executions.conversation_id)
			OR EXISTS (SELECT 1 FROM conversations c JOIN projects p ON p.id = c.project_id WHERE c.id = tool_executions.conversation_id AND p.owner_user_id = $%d)
			OR EXISTS (SELECT 1 FROM conversations c JOIN rbac_resource_assignments pra ON pra.resource_id = c.project_id WHERE c.id = tool_executions.conversation_id AND pra.user_id = $%d AND pra.resource_type = 'project')
		))
...
func (db *DB) UserCanAccessToolExecution(userID, scope, executionID string) bool {
	...
	if err := db.QueryRow(`SELECT owner_user_id, conversation_id FROM tool_executions WHERE id = $1`, executionID).Scan(&ownerUserID, &conversationID); err != nil {
		return false
	}
...
func (db *DB) CancelOrphanedRunningToolExecutions(endTime time.Time, errMsg string) (int64, error) {
	...
	query := `
		...
		SET status = 'orphaned',
		    error = $1,
		    end_time = $2,
		    duration_ms = GREATEST(0, CAST((EXTRACT(EPOCH FROM $3::timestamp) - EXTRACT(EPOCH FROM start_time::timestamp)) * 1000 AS INTEGER))
...
func (db *DB) FinalizeStaleRunningToolExecutions(endTime time.Time, minAge time.Duration, activeIDs map[string]struct{}, errMsg string) (int64, error) {
	...
	for _, row := range stale {
		...
		res, err := db.Exec(`
			UPDATE tool_executions
			SET status = 'orphaned', error = $1, end_time = $2, duration_ms = $3
...
func (db *DB) DeleteToolExecution(id string) error {
	query := `DELETE FROM tool_executions WHERE id = $1`
...
func (db *DB) PurgeToolExecutionsBefore(cutoff time.Time) (int64, error) {
	query := `
		...
		FROM tool_executions
		WHERE ` + sqliteEpochGE("start_time", "<", 1) + `
	...
	res, err := db.Exec(`DELETE FROM tool_executions WHERE `+sqliteEpochGE("start_time", "<", 1), formatSQLiteUTC(cutoff))
	if err != nil {
		return 0, err
	}
...
func (db *DB) SaveToolStats(toolName string, stats *mcp.ToolStats) error {
	...
	query := `
		...
		ON CONFLICT (tool_name) DO UPDATE SET
			total_calls = excluded.total_calls, success_calls = excluded.success_calls,
			failed_calls = excluded.failed_calls, last_call_time = excluded.last_call_time,
...
func (db *DB) UpdateToolStats(toolName string, totalCalls, successCalls, failedCalls int, lastCallTime *time.Time) error {
	...
	query := `
		...
		VALUES ($1, $2, $3, $4, $5, $6)
		ON CONFLICT(tool_name) DO UPDATE SET
			total_calls = tool_stats.total_calls + $7,
			success_calls = tool_stats.success_calls + $8,
			failed_calls = tool_stats.failed_calls + $9,
			last_call_time = COALESCE($10, tool_stats.last_call_time),
			updated_at = $11
	`
...
func (db *DB) LoadCallsTimeline(since time.Time, dailyBuckets bool) ([]CallsTimelineBucket, error) {
	...
	if dailyBuckets {
		...
		query = `
			SELECT to_char(start_time::timestamp AT TIME ZONE 'Asia/Shanghai', 'YYYY-MM-DD HH24:00:00') AS bucket,
				COUNT(*) AS total,
				SUM(CASE WHEN status IN ('failed', 'hard_timeout', 'orphaned') THEN 1 ELSE 0 END) AS failed
			FROM tool_executions
			WHERE start_time >= $1
...
func (db *DB) DecreaseToolStats(toolName string, totalCalls, successCalls, failedCalls int) error {
	...
	query := `
		UPDATE tool_stats SET
			total_calls = CASE WHEN total_calls - $1 < 0 THEN 0 ELSE total_calls - $2 END,
			success_calls = CASE WHEN success_calls - $3 < 0 THEN 0 ELSE success_calls - $4 END,
			failed_calls = CASE WHEN failed_calls - $5 < 0 THEN 0 ELSE failed_calls - $6 END,
			updated_at = $7
		WHERE tool_name = $8
	`
	...
	checkQuery := `SELECT total_calls FROM tool_stats WHERE tool_name = $1`
	...
	if newTotalCalls == 0 {
		deleteQuery := `DELETE FROM tool_stats WHERE tool_name = $1`
