...
func (db *DB) CreateRobotBindingCode(userID, codeHash string, expiresAt time.Time) error {
	...
	if _, err = tx.Exec(`DELETE FROM robot_binding_codes WHERE rbac_user_id = $1 OR expires_at <= $2 OR used_at IS NOT NULL`, userID, now); err != nil {
		return err
	}
	if _, err = tx.Exec(`INSERT INTO robot_binding_codes (code_hash, rbac_user_id, expires_at, created_at) VALUES ($1, $2, $3, $4)`, codeHash, userID, expiresAt, now); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (db *DB) ConsumeRobotBindingCode(platform, externalUserID, codeHash string) (*RBACUser, error) {
	...
	if err = tx.QueryRow(`
		...
		WHERE c.code_hash = $1 AND c.used_at IS NULL AND c.expires_at > $2 AND u.enabled = 1
	...
	result, err := tx.Exec(`UPDATE robot_binding_codes SET used_at = $1 WHERE code_hash = $2 AND used_at IS NULL`, now, codeHash)
	if err != nil {
		return nil, err
	}
...
func (db *DB) ResolveRobotRBACAccess(platform, externalUserID string) (*RBACAccess, error) {
	...
	err = db.QueryRow(`
		...
		WHERE b.platform = $1 AND b.external_user_id = $2 AND b.enabled = 1 AND u.enabled = 1
...
func (db *DB) ListRobotUserBindings(userID string) ([]RobotUserBinding, error) {
	rows, err := db.Query(`
		...
		FROM robot_user_bindings WHERE rbac_user_id = $1 ORDER BY updated_at DESC
...
func (db *DB) DeleteRobotUserBindingForUser(bindingID, userID string) error {
	result, err := db.Exec(`DELETE FROM robot_user_bindings WHERE id = $1 AND rbac_user_id = $2`, strings.TrimSpace(bindingID), strings.TrimSpace(userID))
	if err != nil {
		return err
	}
	if affected, _ := result.RowsAffected(); affected != 1 {
...
func (db *DB) DeleteRobotIdentityBinding(platform, externalUserID string) error {
	...
	_, err = db.Exec(`DELETE FROM robot_user_bindings WHERE platform = $1 AND external_user_id = $2`, platform, externalUserID)
	return err
}
