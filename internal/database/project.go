...
func (db *DB) GetProjectName(id string) (string, error) {
	var name string
	err := db.QueryRow(`SELECT name FROM projects WHERE id = $1`, id).Scan(&name)
	if err != nil {
		if err == sql.ErrNoRows {
...
func appendProjectListFilters(query string, args []interface{}, status, search string) (string, []interface{}) {
	...
	if pattern := projectListSearchPattern(search); pattern != "" {
		query += fmt.Sprintf(` AND (LOWER(name) ILIKE LOWER($%d) ESCAPE '\' OR LOWER(COALESCE(description,'')) ILIKE LOWER($%d) ESCAPE '\' OR LOWER(id) ILIKE LOWER($%d) ESCAPE '\')`, len(args)+1, len(args)+2, len(args)+3)
...
func appendProjectAccessFilter(query string, args []interface{}, userID, scope string) (string, []interface{}) {
	...
	query += fmt.Sprintf(` AND (owner_user_id = $%d OR EXISTS (
		SELECT 1 FROM rbac_resource_assignments ra
		WHERE ra.user_id = $%d AND ra.resource_type = 'project' AND ra.resource_id = projects.id
...
func (db *DB) ListProjects(status, search string, limit, offset int) ([]*Project, error) {
	...
	args := []interface{}{}
	query, args = appendProjectListFilters(query, args, status, search)
	query += fmt.Sprintf(" ORDER BY pinned DESC, updated_at DESC LIMIT $%d::bigint OFFSET $%d::bigint", len(args)+1, len(args)+2)
...
func (db *DB) UpdateProject(p *Project) error {
	...
	_, err := db.Exec(
		`UPDATE projects SET name = $1, description = $2, scope_json = $3, status = $4, pinned = $5, updated_at = $6 WHERE id = $7`,
...
func (db *DB) DeleteProject(id string) error {
	if _, err := db.Exec(`UPDATE vulnerabilities SET project_id = NULL WHERE project_id = $1`, id); err != nil {
		return fmt.Errorf("解除漏洞项目关联失败: %w", err)
	}
	if _, err := db.Exec(`UPDATE assets SET project_id = NULL WHERE project_id = $1`, id); err != nil {
		return fmt.Errorf("解除资产项目关联失败: %w", err)
	}
	if _, err := db.Exec(`UPDATE webshell_connections SET project_id = NULL WHERE project_id = $1`, id); err != nil {
		return fmt.Errorf("解除 WebShell 项目关联失败: %w", err)
	}
	if _, err := db.Exec(`UPDATE c2_listeners SET project_id = NULL WHERE project_id = $1`, id); err != nil {
...
func (db *DB) GetConversationProjectID(conversationID string) (string, error) {
	...
	err := db.QueryRow(`SELECT project_id FROM conversations WHERE id = $1`, conversationID).Scan(&pid)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (db *DB) SetConversationProjectID(conversationID, projectID string) error {
	...
	_, err := db.Exec(`UPDATE conversations SET project_id = $1, updated_at = $2 WHERE id = $3`, val, time.Now(), conversationID)
	if err != nil {
...
func (db *DB) ListProjectFacts(projectID string, filter ProjectFactListFilter, limit, offset int) ([]*ProjectFact, error) {
	...
	if c := strings.TrimSpace(filter.Confidence); c != "" {
		query += fmt.Sprintf(" AND confidence = $%d", len(args)+1)
		args = append(args, c)
	}
	...
	if rid := strings.TrimSpace(filter.RelatedVulnerabilityID); rid != "" {
		query += fmt.Sprintf(" AND related_vulnerability_id = $%d", len(args)+1)
		args = append(args, rid)
	}
	if s := strings.TrimSpace(filter.Search); s != "" {
		pat := "%" + s + "%"
		query += fmt.Sprintf(" AND (fact_key LIKE $%d OR summary LIKE $%d OR body LIKE $%d)", len(args)+1, len(args)+2, len(args)+3)
...
func (db *DB) GetProjectFactByKey(projectID, factKey string) (*ProjectFact, error) {
	row := db.QueryRow(
		`SELECT id, project_id, fact_key, category, summary, COALESCE(body,''), confidence,
			COALESCE(source_conversation_id,''), COALESCE(source_message_id,''), pinned,
			COALESCE(related_vulnerability_id,''), created_at, updated_at
		 FROM project_facts WHERE project_id = $1 AND fact_key = $2`,
		projectID, factKey,
	)
...
func (db *DB) GetProjectFact(id string) (*ProjectFact, error) {
	row := db.QueryRow(
		`SELECT id, project_id, fact_key, category, summary, COALESCE(body,''), confidence,
			COALESCE(source_conversation_id,''), COALESCE(source_message_id,''), pinned,
			COALESCE(related_vulnerability_id,''), created_at, updated_at
		 FROM project_facts WHERE id = $1`, id,
	)
	return scanProjectFactRow(row)
}
...
func (db *DB) UpsertProjectFact(f *ProjectFact) (*ProjectFact, error) {
	...
	if err == nil && existing != nil {
		...
		_, err = db.Exec(
			`UPDATE project_facts SET category = $1, summary = $2, body = $3, confidence = $4,
				source_conversation_id = COALESCE($5, source_conversation_id),
				source_message_id = COALESCE($6, source_message_id),
				pinned = $7, related_vulnerability_id = $8, updated_at = $9
			 WHERE id = $10`,
...
func (db *DB) DeprecateProjectFact(projectID, factKey string) error {
	res, err := db.Exec(
		`UPDATE project_facts SET confidence = 'deprecated', updated_at = $1 WHERE project_id = $2 AND fact_key = $3`,
...
func (db *DB) RestoreProjectFact(projectID, factKey, confidence string) error {
	...
	_, err = db.Exec(
		`UPDATE project_facts SET confidence = $1, updated_at = $2 WHERE project_id = $3 AND fact_key = $4`,
...
func (db *DB) DeleteProjectFact(id string) error {
	...
	_, err = db.Exec(`DELETE FROM project_facts WHERE id = $1`, id)
	return err
}
...
func parseDBTime(s string) time.Time {
	...
	// go-sqlite3 读 TEXT 常返回 RFC3339（含 T），写入时可能是空格分隔格式，需兼容多种形态
	layouts := []string{
		time.RFC3339Nano,
		time.RFC3339,
