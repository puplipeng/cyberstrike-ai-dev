...
func (db *DB) GetRobotSessionBinding(sessionKey string) (*RobotSessionBinding, error) {
	...
	err := db.QueryRow(
		"SELECT session_key, conversation_id, role_name, agent_mode, updated_at FROM robot_user_sessions WHERE session_key = $1",
		sessionKey,
...
func (db *DB) DeleteRobotSessionBinding(sessionKey string) error {
	...
	if _, err := db.Exec("DELETE FROM robot_user_sessions WHERE session_key = $1", sessionKey); err != nil {
