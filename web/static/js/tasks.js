...
// 清空任务历史
async function clearTasksHistory() {
    if (!await appConfirm(_t('tasks.clearHistoryConfirm'))) {
        return;
    }
    tasksState.completedTasksHistory = [];
    saveCompletedTasksHistory();
...
async function batchCancelTasks() {
    ...
    if (!await appConfirm(_t('tasks.confirmCancelTasks', { n: selected.length }))) {
        return;
    }
    ...
    // 显示结果
    if (failCount > 0) {
        appAlert(_t('tasks.batchCancelResultPartial', { success: successCount, fail: failCount }));
    } else {
        appAlert(_t('tasks.batchCancelResultSuccess', { n: successCount }));
    }
}
...
async function cancelTask(conversationId, button) {
    ...
    try {
        ...
        appAlert(_t('tasks.cancelTaskFailed') + ': ' + error.message);
...
async function createBatchQueue() {
    ...
    if (!text) {
        appAlert(_t('tasks.enterTaskPrompt'));
        return;
    }
    ...
    if (scheduleMode === 'cron' && !cronExpr) {
        appAlert(_t('batchImportModal.cronExprRequired'));
        return;
    }
    if (scheduleMode === 'cron' && !/^\S+\s+\S+\s+\S+\s+\S+\s+\S+$/.test(cronExpr)) {
        appAlert(_t('batchImportModal.cronExprInvalid') || 'Cron 表达式格式错误，需要 5 段（分 时 日 月 周）');
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.createBatchQueueFailed') + ': ' + error.message);
    }
}
...
async function showBatchQueueDetail(queueId) {
    ...
    const content = document.getElementById('batch-queue-detail-content');
        ...
        appAlert(_t('tasks.getQueueDetailFailed') + ': ' + error.message);
    }
}
...
async function startBatchQueue() {
    ...
    try {
        ...
        if (isCronPending) {
            const okNow = await appConfirm(_t('batchQueueDetailModal.startExecuteNowConfirm'));
            if (!okNow) return;
        }
        ...
        appAlert(_t('tasks.startBatchQueueFailed') + ': ' + error.message);
    } finally {
...
async function pauseBatchQueue() {
    ...
    if (!await appConfirm(_t('tasks.pauseQueueConfirm'))) {
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.pauseQueueFailed') + ': ' + error.message);
    } finally {
        if (btn) { btn.disabled = false; }
    }
}
...
async function rerunBatchQueue() {
    ...
    if (!await appConfirm(_t('tasks.rerunQueueConfirm'))) {
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.rerunQueueFailed') + ': ' + error.message);
    } finally {
        if (btn) { btn.disabled = false; }
    }
}
...
async function deleteBatchQueue() {
    ...
    if (!await appConfirm(_t('tasks.deleteQueueConfirm'))) {
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.deleteQueueFailed') + ': ' + error.message);
    } finally {
        if (btn) { btn.disabled = false; }
    }
}
...
async function saveInlineTask(queueId, taskId) {
    ...
    if (!message) {
        ...
        appAlert(_t('tasks.taskMessageRequired'));
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.saveTaskFailed') + ': ' + error.message);
    }
}
...
function showAddBatchTaskModal() {
    ...
    const queueId = batchQueuesState.currentQueueId;
    if (!queueId) {
        appAlert(_t('tasks.queueInfoMissing'));
        return;
    }
...
async function saveAddBatchTask() {
    ...
    if (!messageInput) {
        appAlert(_t('tasks.cannotGetTaskMessageInput'));
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.addTaskFailed') + ': ' + error.message);
    }
}

// 从元素获取任务信息并删除任务
async function deleteBatchTaskFromElement(button) {
    const taskItem = button.closest('.batch-task-item');
    if (!taskItem) {
    ...
    if (!await appConfirm(_t('tasks.confirmDeleteTask', { message: displayMessage }))) {
        return;
    }
...
async function deleteBatchTask(queueId, taskId) {
    if (!queueId || !taskId) {
        appAlert(_t('tasks.taskIncomplete'));
        return;
    }
    ...
    try {
        ...
        appAlert(_t('tasks.deleteTaskFailed') + ': ' + error.message);
    }
}
...
async function updateBatchQueueScheduleEnabled(enabled) {
    ...
    try {
        ...
        console.error(e);
        appAlert(_t('batchQueueDetailModal.scheduleToggleFailed') + ': ' + e.message);
        showBatchQueueDetail(queueId);
    }
}
...
async function runSingleBatchTask(queueId, taskId) {
    if (!queueId || !taskId) return;
    if (!await appConfirm(_t('tasks.confirmRunSingleTask'))) return;
    try {
...
async function saveInlineSchedule() {
    ...
    try {
        ...
        _bqInlineSaving = false;
        console.error(e);
        appAlert(_t('batchQueueDetailModal.editScheduleError') + ': ' + e.message);
    }
}
