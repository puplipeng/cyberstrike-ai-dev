...
async function saveCurrentAssetView() {
    const name = await appPrompt('请输入筛选视图名称');
    ...
    try { localStorage.setItem(ASSET_SAVED_VIEWS_KEY, JSON.stringify(views)); } catch (error) { return appAlert('保存筛选视图失败'); }
...
async function deleteCurrentAssetView() {
    const select = document.getElementById('asset-saved-view-select');
    const name = select?.value || '';
    if (!name || !await appConfirm(`删除筛选视图“${name}”吗？`)) return;
...
async function selectAllMatchingAssets() {
    ...
    try {
        ...
        appAlert('选择全部结果失败: ' + error.message);
    } finally {
...
async function openAssetProjectModal() {
    ...
    if (!assets.length) {
        appAlert(assetT('assets.selectAssetsFirst', '请先选择资产'));
        return;
    }
    await ensureAssetProjects(true);
...
async function submitAssetProjectBinding() {
    ...
    if (!selectedProject) {
        appAlert(assetT('assets.selectProjectRequired', '请选择要绑定的项目'));
        return;
    }
    ...
    try {
        ...
        appAlert(assetT('assets.bindProjectFailed', '绑定项目失败') + ': ' + error.message);
    } finally {
        if (button) button.disabled = false;
    }
}
...
function openAssetBulkEdit() {
    const count = assetPageState.selected.size;
    if (!count) return appAlert('请先选择资产');
...
async function submitAssetBulkEdit() {
    ...
    if (Object.keys(body).length === 3 && body.add_tags.length === 0 && body.remove_tags.length === 0) return appAlert('请至少填写一项修改');
...
function exportSelectedAssets(format) {
    ...
    if (!rows.length) return appAlert('请先选择资产');
    ...
    if (!window.XLSX) return appAlert('表格组件加载失败，请刷新后重试');
    const workbook = XLSX.utils.book_new();
...
async function deleteSelectedAssets() {
    const ids = Array.from(assetPageState.selected.keys());
    if (!ids.length || !await appConfirm(`确定永久删除选中的 ${ids.length} 个资产吗？`)) return;
    try {
        ...
        appAlert('批量删除失败: ' + error.message);
    }
}

async function mergeSelectedAssets() {
    const assets = Array.from(assetPageState.selected.values());
    if (assets.length < 2) return appAlert('请至少选择两个重复资产');
    const primary = assets[0];
    if (!await appConfirm(`将保留“${assetTargetLabel(primary)}”作为主资产，并合并其余 ${assets.length - 1} 个资产。继续吗？`)) return;
    try {
...
function normalizeAssetScanHITLConfig(config) {
    ...
    const mode = ['approval', 'review_edit'].includes(String(source.mode || '').toLowerCase())
        ...
        : 'off';
    const reviewer = String(source.reviewer || '').toLowerCase() === 'audit_agent' ? 'audit_agent' : 'human';
    const timeout = Number(source.timeoutSeconds);
    return {
        enabled: mode !== 'off',
        mode,
        reviewer,
        sensitiveTools: [],
        timeoutSeconds: [0, 300, 900].includes(timeout) ? timeout : 300
    };
}

function storedAssetScanHITLConfig() {
    try {
        const raw = localStorage.getItem(ASSET_SCAN_HITL_STORAGE_KEY);
        return normalizeAssetScanHITLConfig(raw ? JSON.parse(raw) : null);
    } catch (error) {
        return normalizeAssetScanHITLConfig(null);
    }
}

function readAssetScanHITLConfig() {
    return normalizeAssetScanHITLConfig({
        mode: document.getElementById('asset-scan-hitl-mode')?.value || 'off',
        reviewer: document.getElementById('asset-scan-hitl-reviewer')?.value || 'human',
        timeoutSeconds: document.getElementById('asset-scan-hitl-timeout')?.value || 300
    });
}

function rememberAssetScanHITLConfig(config) {
    try {
        localStorage.setItem(ASSET_SCAN_HITL_STORAGE_KEY, JSON.stringify(normalizeAssetScanHITLConfig(config)));
    } catch (error) { /* 浏览器禁止存储时仍允许本次扫描 */ }
}

function updateAssetScanApprovalFields() {
    const modeEl = document.getElementById('asset-scan-hitl-mode');
    const reviewerEl = document.getElementById('asset-scan-hitl-reviewer');
    const timeoutEl = document.getElementById('asset-scan-hitl-timeout');
    const hintEl = document.getElementById('asset-scan-approval-hint');
    if (!modeEl || !reviewerEl || !timeoutEl) return;
    const enabled = modeEl.value !== 'off';
    reviewerEl.disabled = !enabled;
    timeoutEl.disabled = !enabled || reviewerEl.value === 'audit_agent';
    if (hintEl) {
        if (!enabled) {
            hintEl.textContent = assetT('assets.approvalOffHint', '本次扫描不创建人工审批，确认后直接执行工具调用。');
        } else if (reviewerEl.value === 'audit_agent') {
            hintEl.textContent = assetT('assets.approvalAgentHint', '工具调用由审计 Agent 自动裁决，不等待人工操作。');
        } else {
            hintEl.textContent = assetT('assets.approvalHumanHint', '工具调用会暂停并等待人工审批，请先确认等待时限。');
        }
    }
}
...
function openAssetScanModal(mode, index) {
    ...
    const hitl = storedAssetScanHITLConfig();
    document.getElementById('asset-scan-hitl-mode').value = hitl.mode;
    document.getElementById('asset-scan-hitl-reviewer').value = hitl.reviewer;
    document.getElementById('asset-scan-hitl-timeout').value = String(hitl.timeoutSeconds);
    updateAssetScanApprovalFields();
...
async function submitAssetScan() {
    ...
    const hitl = readAssetScanHITLConfig();
    if (!assets.length || !template) {
        appAlert(assetT('assets.promptRequired', '请输入用户提示词'));
        return;
    }
    ...
    try {
        if (assetPageState.scanMode === 'task') {
            await createAssetScanTasks(assets, template, hitl);
        } else {
            await sendAssetsToChat(assets, template, hitl);
        }
        rememberAssetScanHITLConfig(hitl);
        closeAssetScanModal();
        clearAssetSelection();
        await loadAssets(assetPageState.page);
    } catch (error) {
        console.error('提交资产扫描失败:', error);
        appAlert(assetT('assets.scanSubmitFailed', '提交扫描失败') + ': ' + error.message);
    } finally {
        button.disabled = false;
    }
}

async function persistAssetScanConversationHITL(conversationId, config) {
    const hitl = normalizeAssetScanHITLConfig(config);
    if (typeof window.saveHitlConversationConfig === 'function') {
        await window.saveHitlConversationConfig(conversationId, hitl);
    } else {
        const response = await apiFetch('/api/hitl/config', {
            method: 'PUT',
    ...
    if (typeof window.saveHitlConfigForConversation === 'function') {
        window.saveHitlConfigForConversation(conversationId, hitl);
    }
    return hitl;
}
...
async function sendAssetsToChat(assets, template, hitlConfig) {
    ...
    const confirmedHITL = await persistAssetScanConversationHITL(conversation.id, hitlConfig);
    ...
    if (typeof window.applyHitlConfigToUI === 'function') {
        window.applyHitlConfigToUI(confirmedHITL);
    }
    ...
    window.__csNextChatHITLConfig = confirmedHITL;
    void sendMessage();
}
...
async function closeAssetEditor(force) {
    if (!force && assetPageState.editorDirty && !await appConfirm(assetT('assets.discardChanges', '放弃尚未保存的更改吗？'))) return;
    closeAssetCustomSelects();
...
async function deleteAsset(index) {
    ...
    if (!asset || !await appConfirm(assetT('assets.deleteConfirm', '确定删除该资产吗？'))) return;
    const response = await apiFetch('/api/assets/' + encodeURIComponent(asset.id), { method: 'DELETE' });
    if (!response.ok) {
        appAlert(assetT('assets.deleteFailed', '删除资产失败'));
        return;
    }
    await loadAssets(assetPageState.page);
}
...
async function importFofaAssetsByIndexes(indexes) {
    ...
    if (!current || !current.results || !indexes.length) {
        appAlert(assetT('assets.selectFirst', '请先选择需要入库的结果'));
        return;
    }
    ...
    if (!assets.length) {
        appAlert(assetT('assets.noValidImportTarget', '所选结果中没有可入库的有效资产目标'));
        return;
    }
    ...
    if (!response.ok) {
        appAlert(assetT('assets.importFailed', '资产入库失败') + ': ' + await response.text());
        return;
    }
    const result = await response.json();
    if (typeof showInlineToast === 'function') {
...
window.updateAssetScanApprovalFields = updateAssetScanApprovalFields;
