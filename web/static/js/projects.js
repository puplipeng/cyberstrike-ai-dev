...
/** 兼容自定义 t 函数（config.t）的 fallback 版本：缺键/返回 key 原文时回退默认值 */
function tFmt(t, key, fallback, opts) {
    const text = typeof t === 'function' ? t(key, opts) : '';
    if (!text || text === key) return fallback;
    return text;
}
...
async function notifyProjectApiFailure(response, fallbackKey, fallbackText) {
    ...
    appAlert(tpFmt(fallbackKey, fallbackText));
    return false;
}
...
async function insertFactBodyTemplate(kind) {
    ...
    if (ta.value.trim() && !await appConfirm(tp('projects.confirmOverwriteBodyTemplate'))) return;
    ta.value = tpl;
    updateFactFormHints();
    ta.focus();
}
...
function formatVulnStatusBadge(status) {
    ...
    const labelMap = {
		...
		submitted: 'vulnerabilityPage.statusSubmitted',
		unsubmitted: 'vulnerabilityPage.statusUnsubmitted',
	};
    const label = labelMap[s] ? tp(labelMap[s]) : status || '—';
	const cls = ['open', 'confirmed', 'fixed', 'false_positive', 'ignored', 'submitted', 'unsubmitted'].includes(s) ? s : 'open';
...
async function unbindAssetFromProject(index) {
    ...
    try {
        ...
        appAlert(`${tp('projects.unbindFailed')}: ${error.message || error}`);
    }
}
...
async function handleGraphConnectNodePick(factKey) {
    ...
    const edgeType = await appPrompt(tp('projects.graphEdgeTypePrompt'), 'leads_to');
    if (!edgeType) {
        _graphConnectSource = null;
        return;
    }
...
async function deleteProjectFactEdge(edgeId) {
    ...
    if (!await appConfirm(tp('projects.confirmDeleteGraphEdge'))) return;
...
async function promoteConversationAttackChain(conversationId) {
    ...
    if (!requireProjectWrite()) return;
    if (!await appConfirm(tp('projects.confirmPromoteAttackChain'))) return;
...
async function unbindConversationFromProject(conversationId) {
    if (!conversationId || !await appConfirm(tp('projects.confirmUnbindConversation'))) return;
    const res = await apiFetch(`/api/conversations/${encodeURIComponent(conversationId)}/project`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId: '' }),
    });
    if (!res.ok) return appAlert(tp('projects.unbindFailed'));
...
async function viewProjectFactBody(factKey) {
    ...
    if (!res.ok) {
        ...
        return appAlert(tp('common.loadFailed'));
    }
...
async function linkFactToExistingVulnerability() {
    ...
    if (!res.ok) return appAlert(tp('projects.loadVulnerabilityListFailed'));
    const data = await res.json();
    const items = data.Vulnerabilities || data.vulnerabilities || data.items || [];
    if (!items.length) return appAlert(tp('projects.noVulnerabilitiesInProject'));
    ...
    if (Number.isNaN(idx) || idx < 0 || idx >= items.length) return appAlert(tp('projects.invalidIndex'));
    ...
    if (!upd.ok) return appAlert(tp('projects.linkFailed'));
    appAlert(tp('projects.linkSuccess'));
    closeFactDetailModal();
    loadProjectFacts();
}
...
async function createVulnerabilityFromCurrentFact() {
    ...
    if (!convId) {
        convId = await appPrompt(tp('projects.promptConversationIdForVulnCreate'), '')?.trim() || '';
    }
    if (!convId) return appAlert(tp('projects.cancelledNoConversationId'));
    ...
    appAlert(successMsg || `Created and linked vulnerability: ${createdVulnLabel}`);
    closeFactDetailModal();
    loadProjectFacts();
...
async function viewFactsForVulnerability(vulnId) {
    ...
    if (!res.ok) return appAlert(tp('projects.loadRelatedFactsFailed'));
    const facts = await res.json();
    if (!facts.length) {
        appAlert(tp('projects.noFactsForVulnerability'));
        loadProjectFacts();
        return;
    }
    if (facts.length === 1) {
...
async function showEditProjectModal(projectId, options = {}) {
    ...
    if (!p) {
        try {
            ...
            closeProjectModal();
            appAlert(e.message || tp('projects.projectNotFound'));
            window._projectModalEditId = null;
            return;
        }
    }
...
async function saveProjectModal() {
    ...
    if (!name) return appAlert(tp('projects.enterProjectName'));
    const body = {
        name,
    ...
    try {
        ...
        if (typeof notifyApiError === 'function') {
            ...
            appAlert(error?.message || tpFmt('projects.saveFailed', '保存失败'));
        }
    } finally {
...
function formatProjectScopeJson() {
    ...
    try {
        ...
        appAlert(tp('projects.invalidJson') + ': ' + (e.message || String(e)));
    }
}
...
async function saveProjectSettings() {
    ...
    if (scopeRaw) {
        try {
            ...
            appAlert(tp('projects.invalidScopeJson') + ': ' + (e.message || String(e)));
            return;
        }
    }
    const body = {
...
async function toggleProjectArchiveById(projectId) {
    ...
    if (!await appConfirm(next === 'archived' ? tp('projects.confirmArchiveProject') : tp('projects.confirmRestoreProjectActive'))) return;
    const res = await apiFetch(`/api/projects/${projectId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
async function deleteProjectById(projectId) {
    ...
    if (!projectId || !await appConfirm(tp('projects.confirmDeleteProject'))) return;
...
function showAddFactModal() {
    if (!currentProjectId) return appAlert(tp('projects.selectProjectFirst'));
    if (!requireProjectWrite()) return;
    resetFactModalForm();
...
async function showEditFactModal(factKey) {
    ...
    if (!res.ok) {
        closeFactModal();
        return appAlert(tp('projects.loadFactFailed'));
    }
    const f = await res.json();
    deferModalContent(() => {
        fillFactModalForm(f);
...
async function saveFactModal() {
    ...
    const body = document.getElementById('fact-modal-body').value;
    if (!fact_key || !summary) return appAlert(tp('projects.factKeySummaryRequired'));
...
async function deleteProjectFact(id) {
    if (!requireProjectWrite()) return;
    if (!await appConfirm(tp('projects.confirmDeleteFact'))) return;
...
function renderChatProjectFolders(projects) {
    ...
    const unassignedProject = {
        id: '',
        name: tpFmt('projects.noProject', '无项目'),
        description: tpFmt('projects.noProjectDescription', '不绑定项目黑板'),
        _isUnassigned: true,
    };
...
async function renderProjectPickerPanel(panelKey, config) {
    ...
    const renderPinned = () => {
        appendChatProjectPanelItem(
            ...
            {
                id: '',
                name: tFmt(t, 'projects.noProject', '无项目'),
                description: tFmt(t, 'projects.noProjectDescription', '不绑定项目黑板'),
            },
            selectedId,
            config.onSelect,
            t
...
function updateChatProjectButtonLabel() {
    ...
    window.applyProjectNameDisplay(
        textEl,
        id && projectNameById[id] ? projectNameById[id] : tpFmt('projects.noProject', '无项目')
    );
...
async function applyChatProjectSelection(projectId) {
    ...
    if (window.currentConversationId) {
        try {
            ...
            console.error(e);
            appAlert(tp('projects.updateProjectBindingFailed') + ': ' + (e.message || e));
            updateChatProjectButtonLabel();
            return;
        }
    } else {
