...
var DASHBOARD_SEVERITY_STATUS_FILTER_VALUES = ['', 'open', 'unsubmitted', 'submitted', 'confirmed', 'fixed', 'ignored', 'false_positive'];
...
var dashboardState = {
    ...
    severityStatusFilter: null,  // 严重程度分布当前状态筛选
...
// 漏洞状态映射：把 status 字符串规整到已知状态（避免脏数据）
function statusKey(s) {
	...
	if (s === 'unsubmitted') return 'unsubmitted';
    return 'open';
}
...
function statusShortLabel(s) {
	...
	if (k === 'submitted') return dt('dashboard.statusSubmitted', null, '已提交');
	if (k === 'unsubmitted') return dt('dashboard.statusUnsubmitted', null, '未提交');
...
// 漏洞处置状态 + 修复进度面板
// byStatus: { open, unsubmitted, submitted, confirmed, fixed, false_positive, ignored }（任一字段缺失视作 0）
...
function renderVulnStatusPanel(byStatus, total) {
	...
	var unsubmitted = get('unsubmitted');
	...
	setEl('dashboard-status-submitted', formatNumber(submitted));
	setEl('dashboard-status-unsubmitted', formatNumber(unsubmitted));

    // 修复率只按需要处置的有效漏洞计算；误报/已忽略属于中性闭环，不拉低修复率。
	var actionableTotal = open + unsubmitted + submitted + confirmed + fixed;
