...
(function () {
    ...
    // Login tokens never appear in URLs. The socket uses a one-use ticket.
    ...
    async function ensureTerminalWS(tab) {
        ...
        tab.connecting = true;
        try {
            var response = await fetch('/api/terminal/ws-ticket', {method: 'POST', headers: {Authorization: 'Bearer ' + (getStoredAuthToken() || '')}, cache: 'no-store'});
            if (!response.ok) throw new Error('Terminal authorization failed (' + response.status + ')');
            var ticket = await response.json();
            if (!tab.term) return;
            var ws = new WebSocket(buildTerminalWSURL(), ['csai-terminal', 'csai-ticket.' + ticket.ticket]);
            tab.ws = ws;
            tab.running = true;
