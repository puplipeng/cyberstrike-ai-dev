...
function switchPage(pageId) {
    ...
    if (pageId !== 'chat') {
        ...
        if (
            ...
            typeof window.CyberStrikeChatScroll.onPageLeave === 'function'
        ) {
            window.CyberStrikeChatScroll.onPageLeave();
        }
    ...
    if (
        pageId === 'chat' &&
        window.CyberStrikeChatScroll &&
        typeof window.CyberStrikeChatScroll.onPageEnter === 'function'
    ) {
        window.CyberStrikeChatScroll.onPageEnter();
    }
...
function updateNavState(pageId) {
    ...
    } else if (pageId === 'skills-monitor' || pageId === 'skills-management' || pageId === 'skill-library') {
        // Skills子菜单项
...
async function initPage(pageId) {
    ...
    switch(pageId) {
        ...
        case 'vulnerability-intel':
            if (typeof initVulnerabilityIntelligence === 'function') {
                initVulnerabilityIntelligence();
            }
            break;
        case 'github-leaks':
            if (typeof window.initGitHubLeaks === 'function') {
                window.initGitHubLeaks();
            }
            break;
        case 'vulnerabilities':
            ...
            break;
        case 'webshell':
            if (window.SSHManager) window.SSHManager.init();
            // 初始化 WebShell 管理页面
        ...
        case 'skills-monitor':
            ...
            break;
        case 'skill-library':
            if (typeof initSkillLibrary === 'function') initSkillLibrary();
            break;
