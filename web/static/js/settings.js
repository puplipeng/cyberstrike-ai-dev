...
let githubLeakRulesEditor = null;

function ensureGitHubLeakRulesEditor() {
    if (githubLeakRulesEditor) return githubLeakRulesEditor;
    if (typeof GitHubLeakRules === 'undefined' || !GitHubLeakRules || typeof GitHubLeakRules.createEditor !== 'function') return null;
    githubLeakRulesEditor = GitHubLeakRules.createEditor({
...
async function loadConfig(loadTools = true, options = {}) {
    ...
    try {
        ...
        const githubLeak = currentConfig.github_leak_monitor || {};
        const githubLeakEnabled = document.getElementById('github-leak-enabled');
        ...
        if (githubLeakToken) {
            githubLeakToken.value = '';
            githubLeakToken.placeholder = githubLeak.token_configured ? '已配置；留空保留现有 Token' : '输入专用只读 Token';
        }
        if (githubLeakTokenHint) {
            githubLeakTokenHint.textContent = githubLeak.token_configured
                ? 'Token 已配置。留空会保留现有值，页面不会回显凭据。'
                : 'Token 尚未配置；启用监控前需要填写。';
        }
        ...
        if (currentConfig.knowledge) {
            ...
            const mqEnabledInput = document.getElementById('knowledge-multi-query-enabled');
            if (mqEnabledInput) mqEnabledInput.checked = mq.enabled === true;
        ...
        if (!silent) {
            ...
            if (typeof notifyApiError === 'function') {
                ...
                appAlert(baseMsg + ': ' + error.message);
            }
        }
        throw error;
    }
...
async function applySettings() {
    try {
        ...
        const githubLeakRulesResult = ensureGitHubLeakRulesEditor()?.read() || {
            valid: false,
            rules: [],
            globalErrors: ['GitHub 监控规则编辑器未加载'],
        };
        ...
        if (!githubLeakRulesResult.valid) {
            hasError = true;
        }
        const githubLeakHasEnabledRules = githubLeakRules.some(rule => rule.enabled === true);
        const githubLeakActivationError = typeof GitHubLeakRules !== 'undefined' && GitHubLeakRules
            ? GitHubLeakRules.activationError(githubLeakEnabled, githubLeakRules)
            : 'GitHub 监控规则编辑器未加载';
        if (githubLeakActivationError) {
            const rulesErrorOutput = document.getElementById('github-leak-rules-error');
            if (rulesErrorOutput) {
                const existing = String(rulesErrorOutput.textContent || '').trim();
                rulesErrorOutput.textContent = existing ? `${existing}；${githubLeakActivationError}` : githubLeakActivationError;
            }
            hasError = true;
        }
        if (githubLeakEnabled && githubLeakHasEnabledRules && !String(githubLeakTokenEl?.value || '').trim() && currentConfig?.github_leak_monitor?.token_configured !== true) {
            githubLeakTokenEl?.classList.add('error');
            hasError = true;
        }
        ...
        if (!Number.isInteger(githubLeakTimeout) || githubLeakTimeout < 30 || githubLeakTimeout > 120) {
            githubLeakTimeoutEl?.classList.add('error');
            hasError = true;
        }
        if (!Number.isInteger(githubLeakPerPage) || githubLeakPerPage < 1 || githubLeakPerPage > 100) {
            githubLeakPerPageEl?.classList.add('error');
            hasError = true;
        }
        
        if (provider !== 'codex_account' && !apiKey) {
            document.getElementById('openai-api-key').classList.add('error');
            hasError = true;
        }
        
        if (provider !== 'codex_account' && !baseUrl) {
        ...
        if (visionPayload.enabled && !visionPayload.model) {
            ...
            appAlert((typeof window.t === 'function') ? window.t('settingsBasic.visionModelRequired') : '启用视觉分析时请填写视觉模型名称');
            return;
        }
        ...
        const knowledgeConfig = {
            ...
            retrieval: {
                ...
                multi_query: {
                    enabled: document.getElementById('knowledge-multi-query-enabled')?.checked ?? currentConfig?.knowledge?.retrieval?.multi_query?.enabled ?? false,
                    max_queries: (() => {
        ...
        const config = {
            ...
            github_leak_monitor: (() => {
                ...
                const token = String(githubLeakTokenEl?.value || '').trim();
...
function normalizeAIChannelProviderProfile(channel) {
    ...
    if (channel.provider === 'codex_account') {
        channel.api_key = '';
        channel.base_url = '';
        return channel;
    }
...
function updateAIChannelEditorChrome(id) {
    ...
    if (meta) {
        const provider = ch.provider === 'codex_account' ? 'Codex 当前账号' : ch.provider === 'claude' ? 'Claude' : settingsT('settingsBasic.aiChannelOpenAICompat', 'OpenAI 兼容');
...
async function probeSelectedAIChannels() {
    ...
    if (!ids.length) {
        appAlert(settingsT('settingsBasic.aiChannelProbeNoComplete', '没有可探活的完整通道，请先填写 Base URL、API Key 和模型'));
        return;
    }
        ...
        try {
            const response = await apiFetch('/api/config/test-openai', {
                ...
                body: JSON.stringify({
                    ...
                    api_key: ch.api_key || '',
                    key_ref: '/ai/channels/' + String(id).replace(/~/g, '~0').replace(/\//g, '~1') + '/api_key',
                    model: ch.model || ''
                })
            });
...
async function deleteCheckedAIChannels() {
    ...
    if (!await appConfirm(`确定删除 ${deletable.length} 个 AI 通道吗？\n${names}`)) {
        return;
    }
...
// Masked credentials are resolved on the server; no saved key enters this page.
function savedModelKeyRef(scope) {
    const id = String(selectedAIChannelId || currentConfig?.ai?.default_channel || 'default').replace(/~/g, '~0').replace(/\//g, '~1');
    if (scope === 'vision' && (document.getElementById('vision-api-key')?.value || '').trim()) return '/vision/api_key';
    if (scope === 'hitlAudit' && (document.getElementById('hitl-audit-model-api-key')?.value || '').trim()) return '/hitl/audit_model/api_key';
    if (scope === 'knowledgeEmbedding' && (document.getElementById('knowledge-embedding-api-key')?.value || '').trim()) return '/knowledge/embedding/api_key';
    return '/ai/channels/' + id + '/api_key';
}
...
function resolveModelListCredentials(scope) {
    if (scope === 'vision') {
        ...
        return { provider, base_url: baseUrl, api_key: apiKey, key_ref: savedModelKeyRef(scope) };
    }
    if (scope === 'hitlAudit') {
    ...
    return {
        ...
        base_url: (document.getElementById('openai-base-url')?.value || '').trim(),
        api_key: (document.getElementById('openai-api-key')?.value || '').trim(),
        key_ref: savedModelKeyRef('openai')
    };
}

function syncModelListFetchButtons() {
    syncCodexAccountFields();
...
async function fetchModelList(scope) {
    ...
    if (creds.provider !== 'codex_account' && !creds.api_key) {
        if (resultEl) {
    ...
    try {
        const response = await apiFetch('/api/config/list-models', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(creds.provider === 'codex_account' ? { provider: creds.provider, base_url: '', api_key: '' } : creds)
        });
        const result = await response.json();
        if (!response.ok) {
...
async function testVisionConnection() {
    ...
    try {
        const response = await apiFetch('/api/config/test-vision', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ vision: vision, openai: openai, key_ref: savedModelKeyRef('vision') })
        });
        const result = await response.json();
        if (result.success) {
...
function collectHitlAuditModelEffectiveConfig() {
    ...
    return {
        ...
        key_ref: savedModelKeyRef('hitlAudit'),
...
async function testOpenAIConnection() {
    ...
    if ((provider !== 'codex_account' && (!baseUrl || !apiKey)) || !model) {
    ...
    try {
        const response = await apiFetch('/api/config/test-openai', {
            ...
            body: JSON.stringify({
                provider: provider,
                base_url: provider === 'codex_account' ? '' : baseUrl,
                api_key: provider === 'codex_account' ? '' : apiKey,
                key_ref: savedModelKeyRef('openai'),
                model: model
            })
        });
...
async function saveToolsConfig() {
    ...
    try {
        ...
        appAlert(typeof window.t === 'function' ? window.t('mcp.toolsConfigSaved') : '工具配置已成功保存！');
        ...
        appAlert((typeof window.t === 'function' ? window.t('mcp.saveToolsConfigFailed') : '保存工具配置失败') + ': ' + error.message);
    }
}
...
async function changePassword() {
    ...
    if (hasError) {
        appAlert(typeof window.t === 'function' ? window.t('settings.security.fillPasswordHint') : '请正确填写当前密码和新密码，新密码至少 8 位且需要两次输入一致。');
        return;
    }
    ...
    try {
        ...
        appAlert((typeof window.t === 'function' ? window.t('settings.security.changePasswordFailed') : '修改密码失败') + ': ' + error.message);
    } finally {
        if (submitBtn) {
...
async function editExternalMCP(name) {
    try {
        ...
        appAlert((typeof window.t === 'function' ? window.t('mcp.operationFailed') : '编辑失败') + ': ' + error.message);
    }
}
...
async function saveExternalMCP() {
    ...
    try {
        ...
        appAlert(typeof window.t === 'function' ? window.t('mcp.saveSuccess') : '保存成功');
    } catch (error) {
...
async function deleteExternalMCP(name) {
    if (!await appConfirm((typeof window.t === 'function' ? window.t('mcp.deleteExternalConfirm', { name: name }) : `确定要删除外部MCP "${name}" 吗？`))) {
        return;
    }
    ...
    try {
        ...
        appAlert(typeof window.t === 'function' ? window.t('mcp.deleteSuccess') : '删除成功');
    } catch (error) {
        console.error('删除外部MCP失败:', error);
        appAlert((typeof window.t === 'function' ? window.t('mcp.operationFailed') : '删除失败') + ': ' + error.message);
    }
}
...
async function toggleExternalMCP(name, currentStatus) {
    ...
    try {
        ...
        appAlert((typeof window.t === 'function' ? window.t('mcp.operationFailed') : '操作失败') + ': ' + error.message);
...
async function pollExternalMCPStatus(name, maxAttempts = 30) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (status === 'connected') {
                    ...
                    if (status === 'error') {
                        appAlert(typeof window.t === 'function' ? window.t('mcp.connectionFailedCheck') : '连接失败，请检查配置和网络连接');
                    }
                    return;
    ...
    appAlert(typeof window.t === 'function' ? window.t('mcp.connectionTimeout') : '连接超时，请检查配置和网络连接');
}
...
function readTokenOptimizationSettings() {
    return {
        tool_search_enable: document.getElementById('eino-tool-search-enable')?.checked ?? currentConfig?.multi_agent?.tool_search_enable ?? false,
        reduction_enable: document.getElementById('eino-reduction-enable')?.checked ?? currentConfig?.multi_agent?.reduction_enable ?? false
    };
}

function syncCodexAccountFields() {
    const accountMode = document.getElementById('openai-provider')?.value === 'codex_account';
    for (const id of ['openai-api-key', 'openai-base-url']) {
        const input = document.getElementById(id);
        if (!input) continue;
        input.required = !accountMode;
        input.disabled = accountMode;
        const group = input.closest('.form-group');
        if (group) group.style.display = accountMode ? 'none' : '';
    }
    const hint = document.getElementById('codex-account-hint');
    if (hint) hint.style.display = accountMode ? '' : 'none';
    const outputHint = document.getElementById('codex-output-budget-hint');
    if (outputHint) outputHint.style.display = accountMode ? '' : 'none';
}
