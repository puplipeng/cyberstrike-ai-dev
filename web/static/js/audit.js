    ...
    if (willOpen) {
        ...
        if (!window._auditExportMenuDocBound) {
            ...
            document.addEventListener('click', function (event) {
                const wrapper = document.querySelector('.audit-export-dropdown');
                if (!wrapper || !wrapper.contains(event.target)) closeAuditExportMenu();
            });
            document.addEventListener('keydown', function (event) {
                if (event.key === 'Escape') closeAuditExportMenu();
            });
        }
    } else {
