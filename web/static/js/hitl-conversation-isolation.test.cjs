...
test('服务端默认审批人只更新默认值，不覆盖最近会话选择', () => {
    ...
    vm.runInContext("applyHitlDefaultReviewerFromServer('audit_agent')", context);
    assert.equal(window.csaiHitlDefaultReviewer, 'audit_agent');
    assert.equal(window.csaiHitlDefaultConfig.reviewer, 'audit_agent');
    assert.deepEqual(writes, [], 'server defaults must not overwrite saved conversation choices');
    vm.runInContext("applyHitlDefaultReviewerFromServer('invalid')", context);
    assert.equal(window.csaiHitlDefaultReviewer, 'human');
});
