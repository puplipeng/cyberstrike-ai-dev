...
function createScrollRuntime() {
    ...
    const pageChat = {
        classList: {
            add(name) { pageClasses.add(name); },
            remove(name) { pageClasses.delete(name); },
            toggle(name, force) {
                if (force === true) pageClasses.add(name);
                else if (force === false) pageClasses.delete(name);
                else if (pageClasses.has(name)) pageClasses.delete(name);
                else pageClasses.add(name);
            },
    ...
    const document = {
        ...
        getElementById(id) {
            ...
            if (id === 'turn-anchor') return anchorEl;
            return null;
        },
    ...
    const context = {
        ...
        MutationObserver: class {
            constructor(handler) { mutationObserverCallback = handler; }
            observe() {}
            disconnect() {}
        },
        ResizeObserver: class {
            constructor(handler) { resizeObserverCallback = handler; }
            observe() {}
            disconnect() {}
        },
        requestAnimationFrame,
...
test('发送新消息后锚定当前轮次，流式增长不再把视口持续拖到底部', () => {
    const runtime = createScrollRuntime();
    runtime.flushAnimationFrames();
    runtime.chatEl.scrollHeight = 2200;
    runtime.chatEl.scrollTop = 1700;

    runtime.api.onUserSendMessage('turn-anchor');
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 802, '本轮问题应固定在阅读区上方');
    assert.equal(runtime.api.captureScrollPinState(), false, '锚定模式不应追随每次流式增高');

    runtime.chatEl.scrollHeight = 2600;
    runtime.api.scrollIfPinned(true);
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 802, '回复继续增长时保持稳定阅读位置');

    runtime.anchorEl.offsetTop = 1180;
    runtime.chatEl.scrollHeight = 3000;
    runtime.triggerMutation();
    runtime.triggerResize();
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 802, '流式重排和容器缩放不得反复重算本轮锚点');

    runtime.listeners.get('wheel')({ deltaY: 20 });
    runtime.chatEl.scrollTop = 2500;
    runtime.listeners.get('scroll')();
    assert.equal(runtime.api.captureScrollPinState(), true, '用户主动滑到底部后恢复实时跟随');
});

test('切出聊天页会停止持续滚底，返回后只恢复离开前的位置', () => {
    const runtime = createScrollRuntime();
    runtime.flushAnimationFrames();
    runtime.chatEl.scrollHeight = 2200;
    runtime.chatEl.scrollTop = 1700;
    runtime.api.onUserSendMessage('turn-anchor');
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 802);

    runtime.api.onPageLeave();
    runtime.pageChat.classList.remove('active');
    runtime.chatEl.scrollHeight = 3000;
    runtime.triggerMutation();
    runtime.triggerResize();
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 802, '隐藏聊天页期间不得继续滚动');

    runtime.pageChat.classList.add('active');
    runtime.api.onPageEnter();
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 802, '返回聊天页只恢复一次，不得切回持续粘底');
    assert.equal(runtime.api.captureScrollPinState(), false);
});

test('同一会话重绘后按稳定消息锚点恢复阅读位置', () => {
    const runtime = createScrollRuntime();
    runtime.anchorEl.id = 'turn-anchor';
    runtime.anchorEl.dataset = { backendMessageId: 'user-backend-1' };
    runtime.anchorEl.offsetHeight = 80;
    runtime.chatEl.children = [runtime.anchorEl];
    runtime.chatEl.scrollHeight = 2200;
    runtime.chatEl.scrollTop = 1700;
    runtime.api.onUserSendMessage('turn-anchor');
    runtime.flushAnimationFrames();
    const state = runtime.api.captureViewState();

    const restoredAnchor = {
        id: 'turn-anchor-restored',
        dataset: { backendMessageId: 'user-backend-1' },
        offsetTop: 1120,
        offsetHeight: 80,
    };
    runtime.chatEl.children = [restoredAnchor];
    runtime.chatEl.scrollHeight = 3200;
    runtime.api.suspendForRender();
    runtime.api.restoreViewState(state);
    runtime.flushAnimationFrames();

    assert.equal(runtime.chatEl.scrollTop, 1102, '消息重绘后应保持原锚点在视口中的相对位置');
    assert.equal(runtime.api.captureScrollPinState(), false, '恢复后仍保持稳定阅读模式');
    runtime.triggerMutation();
    runtime.triggerResize();
    runtime.flushAnimationFrames();
    assert.equal(runtime.chatEl.scrollTop, 1102, '后续布局变化不得重新开启持续滚底');
});
...
test('登录成功后重新加载曾因未授权失败的项目侧栏', () => {
    ...
    assert.match(html, /\/static\/js\/auth\.js\?v=[^"']+/);
});
...
test('刷新运行中任务补齐最新详情后保持粘底但尊重用户上滑', () => {
    ...
    assert.match(chat, /preserveSameConversationView/);
    assert.match(chat, /captureViewState\(\)/);
    assert.match(chat, /suspendForRender\(\)/);
    assert.match(chat, /restoreViewState\(preservedChatViewState\)/);
    assert.match(router, /CyberStrikeChatScroll\.onPageLeave\(\)/);
    assert.match(router, /CyberStrikeChatScroll\.onPageEnter\(\)/);
});
...
test('实时迭代在有限高度内独立跟随且外层保留稳定阅读区', () => {
    ...
    assert.match(css, /\.progress-container\.is-streaming \.progress-timeline\.expanded,[\s\S]{0,360}max-height: min\(44vh, 460px\);[\s\S]{0,180}overflow-y: auto;/);
    assert.match(css, /\.message\.progress-message \.progress-timeline\.expanded \{[\s\S]{0,260}max-height: min\(44vh, 460px\);[\s\S]{0,160}overflow-y: auto;/);
...
test('同一会话的其他标签页自动补流且发送前阻止重复任务', () => {
    ...
    assert.match(sendSource, /const userMessageId = addMessage\('user'/);
    assert.match(sendSource, /onUserSendMessage\(userMessageId\)/);
    assert.ok(sendSource.indexOf('onUserSendMessage(userMessageId)') < sendSource.indexOf('const progressId = addProgressMessage()'));
...
test('页面在任务补流脚本之前加载智能滚动控制器', () => {
    const scrollIndex = html.indexOf('/static/js/chat-scroll.js?v=20260830-pageview1');
    const monitorIndex = html.indexOf('/static/js/monitor.js?v=20260830-codexargs1');
