...
test('刷新恢复会话时先完成权威审批配置同步再允许发送', async () => {
    ...
    const window = { addEventListener() {}, apiFetch: () => response };
    const context = vm.createContext({
        window,
        document: { addEventListener: (name, fn) => listeners.set(name, fn), getElementById: () => null },
        localStorage: { getItem: () => null }, console, setTimeout() {}, apiFetch: () => response,
    });
    vm.runInContext(fs.readFileSync('web/static/js/hitl.js', 'utf8'), context);
    listeners.get('DOMContentLoaded')();
    const ready = window.csaiHitlDefaultReviewerReady;
    assert.equal(ready, window.csaiHitlDefaultConfigReady);
    let settled = false;
    ready.then(() => { settled = true; });
    await Promise.resolve();
    assert.equal(settled, false, 'readiness must wait for the server response');
    finishFetch({ ok: true, json: async () => ({ defaultReviewer: 'audit_agent', defaultMode: 'all' }) });
    await ready;
    assert.equal(window.csaiHitlDefaultReviewer, 'audit_agent');
});
...
test('输入框可按会话通道获取模型并双向同步会话推理且审批模型只出现在审计 Agent 入口', () => {
    ...
    assert.match(chat, /provider !== 'codex_account' && !apiKey/);
    assert.match(chat, /provider: provider,[\s\S]{0,120}api_key: apiKey/);
...
test('对话临时展开层支持点击空白处与 Escape 收起', () => {
    assert.match(chat, /function dismissChatTransientLayersOnDocumentClick\(event\)/);
    assert.match(chat, /reasoningOpen[\s\S]{0,500}!chatLayerContainsTarget\(reasoning, target\)[\s\S]{0,300}closeChatReasoningPanel\(\)/);
    assert.match(chat, /modelMenu && !modelMenu\.hidden && !chatLayerContainsTarget\(modelWrap, target\)/);
    assert.match(chat, /agentPanel && agentPanel\.style\.display !== 'none'[\s\S]{0,220}!chatLayerContainsTarget\(agentWrap, target\)/);
    assert.match(chat, /function dismissChatTransientLayersOnEscape\(event\)[\s\S]{0,500}closeRoleSelectionPanel\(\)[\s\S]{0,200}closeChatProjectPanel\(\)/);
    assert.match(chat, /document\.addEventListener\('click', dismissChatTransientLayersOnDocumentClick\)/);
    assert.match(chat, /document\.addEventListener\('keydown', dismissChatTransientLayersOnEscape\)/);
});
...
test('多对话并发时释放隐藏主流且旧请求不能覆盖新对话状态', () => {
    ...
    assert.match(template, /monitor\.js\?v=[^"']+/);
    assert.match(template, /chat-scroll\.js\?v=[^"']+/);
    assert.match(template, /chat\.js\?v=[^"']+/);
    assert.match(template, /style\.css\?v=[^"']+/);
});
...
test('审批状态主动轮询并在服务不可用时立即关闭旧审批', () => {
    ...
    assert.match(template, /projects\.js\?v=[^"']+/);
});
