...
async function submitWorkflowHitlDecisionFromPage(runId, approved, conversationId) {
    ...
    try {
        ...
        appAlert((e && e.message) ? e.message : hitlT('submitFailed', 'Submit failed'));
    }
}
...
async function submitHitlDecision(interruptId, decision, conversationIdOpt) {
    ...
    if (editBox && editBox.value && editBox.value.trim()) {
        try {
            ...
            appAlert(hitlT('invalidJson', 'Invalid JSON arguments'));
            return;
        }
    }
...
async function submitHitlDecisionWithPayload(interruptId, decision, comment, editedArguments, conversationIdForFollow) {
    ...
    if (!resp.ok) {
        ...
        appAlert(hitlT('submitFailedPrefix', 'Submit failed:') + ' ' + errText);
        return false;
    }
    refreshHitlPending();
...
async function batchDeleteHitlLogs() {
    ...
    if (!ids.length) {
        appAlert(hitlT('selectLogsFirst', 'Select audit logs to delete first'));
        return;
    }
    const count = ids.length;
    if (!await appConfirm(hitlT('batchDeleteConfirm', 'Delete the selected {{count}} audit log(s)? This cannot be undone.', { count: count }))) {
        return;
    }
    try {
        ...
        appAlert(hitlT('batchDeleteSuccess', 'Successfully deleted {{count}} audit log(s)', { count: deletedCount }));
    } catch (e) {
        console.error('batchDeleteHitlLogs', e);
        appAlert(hitlT('batchDeleteFailed', 'Batch delete failed') + ': ' + (e && e.message ? e.message : String(e)));
    }
}
...
async function clearHitlLogs() {
    ...
    if (!await appConfirm(hitlT(confirmKey, 'Clear all {{count}} audit log(s)? This cannot be undone.', { count: count }))) {
        return;
    }
    try {
        ...
        hitlLogsPage = 1;
        await refreshHitlLogs();
        appAlert(hitlT('clearAllSuccess', 'Cleared {{count}} audit log(s)', { count: deletedCount }));
    } catch (e) {
        console.error('clearHitlLogs', e);
        appAlert(hitlT('clearAllFailed', 'Clear failed') + ': ' + (e && e.message ? e.message : String(e)));
    }
}
...
async function openHitlLogModal(idOpt) {
    ...
    if (!resp.ok) {
        appAlert(hitlT('loadFailed', 'Failed to load'));
        return;
    }
