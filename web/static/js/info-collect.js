...
async function submitFofaSearch() {
    ...
    if (!query) {
        appAlert(_t('infoCollect.enterFofaQuery'));
        return;
    }
    ...
    try {
        ...
        appAlert(_t('infoCollect.queryFailed') + ': ' + (e && e.message ? e.message : String(e)));
    } finally {
...
async function parseFofaNaturalLanguage() {
    ...
    if (!text) {
        appAlert(_t('infoCollect.enterNaturalLanguage'));
        return;
    }
...
function copyFofaTarget(target) {
    ...
    if (!text) {
        appAlert(_t('infoCollect.noTargetToCopy'));
        return;
    }
    navigator.clipboard.writeText(text).then(() => {
        ...
        appAlert(_t('infoCollect.manualCopyHint') + text);
    });
}
...
function scanFofaRow(encodedRowJson, clickEvent) {
    ...
    if (!target) {
        appAlert(_t('infoCollect.cannotInferTarget'));
        return;
    }
    ...
    setTimeout(async () => {
        ...
        if (autoSend) {
            if (typeof sendMessage === 'function') {
                ...
                appAlert(_t('infoCollect.noSendMessage'));
            }
        } else {
...
function toggleFofaColumnsPanel() {
    ...
    const btn = document.getElementById('fofa-columns-toggle');
...
document.addEventListener('click', (e) => {
    ...
    closeFofaColumnsPanel();
});
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') closeFofaColumnsPanel();
});
...
function exportFofaResults(format) {
    ...
    if (!p || !Array.isArray(p.results) || p.results.length === 0) {
        appAlert(_t('infoCollect.noExportResult'));
        return;
    }
    ...
    if (format === 'xlsx') {
        ...
        if (typeof XLSX === 'undefined') {
            appAlert(_t('infoCollect.xlsxNotLoaded'));
            return;
        }
...
async function batchScanSelectedFofaRows() {
    ...
    if (tasks.length === 0) {
        appAlert(_t('infoCollect.noScanTarget'));
        return;
    }
    ...
    try {
        ...
        appAlert(_t('infoCollect.batchScanFailed') + ': ' + (e && e.message ? e.message : String(e)));
    }
}
...
function showCellDetailModal(field, fullText) {
    ...
    document.getElementById('info-collect-cell-modal-copy')?.addEventListener('click', () => {
        navigator.clipboard.writeText(text).then(() => showInlineToast(_t('common.copied'))).catch(() => appAlert(_t('common.copyFailed')));
    });
