...
async function submitUserInterruptContinue() {
    ...
    try {
        ...
        appAlert((typeof window.t === 'function' ? window.t('tasks.cancelTaskFailed') : '操作失败') + ': ' + error.message);
    } finally {
        if (stopBtn) {
...
async function submitUserInterruptHardCancel() {
    ...
    try {
        ...
        appAlert((typeof window.t === 'function' ? window.t('tasks.cancelTaskFailed') : '取消任务失败') + ': ' + error.message);
    }
}
...
async function performHardCancelProgressTask(progressId, conversationId = '') {
    ...
    if (!targetConversationId) {
        ...
        appAlert(typeof window.t === 'function' ? window.t('tasks.taskInfoNotSynced') : '任务信息尚未同步，请稍后再试。');
        return;
    }
...
function isPartialEinoUsageSummary(data) {
    const reason = data && data.reason != null ? String(data.reason).trim().toLowerCase() : '';
...
function handleStreamEvent(event, progressElement, progressId, 
    ...
    switch (event.type) {
        ...
        case 'eino_usage_summary': {
            const d = event.data || {};
            const assistantId = typeof getAssistantId === 'function' ? getAssistantId() : null;
            if (assistantId && typeof window.setAssistantTurnTokenUsage === 'function') {
                window.setAssistantTurnTokenUsage(assistantId, d);
            }
            // Partial summaries are still retained as compact turn metadata, but
            // should not appear as a normal execution step immediately before an
            // error card. That made one failure look like three separate errors.
            if (isPartialEinoUsageSummary(d)) break;
...
async function deleteExecution(executionId) {
    ...
    if (!await appConfirm(deleteConfirmMsg)) {
        return;
    }
    ...
    try {
        ...
        appAlert(execDeletedMsg);
    } catch (error) {
        console.error('删除执行记录失败:', error);
        const deleteFailedMsg = typeof window.t === 'function' ? window.t('mcpMonitor.deleteExecFailed') : '删除执行记录失败';
        appAlert(deleteFailedMsg + ': ' + error.message);
    }
}
...
async function batchDeleteExecutions() {
    ...
    if (!await appConfirm(batchConfirmMsg)) {
        return;
    }
    ...
    try {
        ...
        appAlert(batchSuccessMsg);
    } catch (error) {
        console.error('批量删除执行记录失败:', error);
        const batchFailedMsg = typeof window.t === 'function' ? window.t('mcp.batchDeleteFailed') : '批量删除执行记录失败';
        appAlert(batchFailedMsg + ': ' + error.message);
    }
}
