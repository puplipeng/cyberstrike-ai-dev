...
function wsToggleRolePanel() {
    ...
    var btn = document.getElementById('ws-role-selector-btn');
    if (!panel) return;
...
function wsToggleAgentModePanel() {
    ...
    var btn = document.getElementById('ws-agent-mode-btn');
    if (!panel) return;
    var isOpen = panel.style.display === 'flex';
...
async function applyWebshellAiProjectSelection(projectId) {
    ...
    if (convId) {
        try {
            ...
            console.error(e);
            appAlert(wsProjectT('projects.updateProjectBindingFailed', '更新项目绑定失败') + ': ' + (e.message || e));
            wsUpdateProjectButtonLabel();
            return;
        }
    } else {
...
function bindWebshellActionMenusAutoCloseOnce() {
    ...
    var selector = 'details.webshell-conn-actions,details.webshell-row-actions,details.webshell-toolbar-actions';
    var openSelector = 'details.webshell-conn-actions[open],details.webshell-row-actions[open],details.webshell-toolbar-actions[open]';
    function closeMenus(except) {
        document.querySelectorAll(openSelector).forEach(function (d) {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (summary && summary.closest(selector) === clickedInMenu) {
            closeMenus(clickedInMenu);
            return;
        }
        if (e.target.closest('button,a,[role="menuitem"]')) clickedInMenu.open = false;
    }, true);
    document.addEventListener('keydown', function (e) {
...
function runBatchProbeWebshellConnections() {
    ...
    if (!Array.isArray(webshellConnections) || webshellConnections.length === 0) {
        appAlert(wsT('webshell.probeNoConnections') || '暂无可探活连接');
        return;
    }
...
function isWebshellPartialEinoUsageSummary(data) {
    var reason = data && data.reason != null ? String(data.reason).trim().toLowerCase() : '';
...
function renderWebshellProcessDetailsBlock(processDetails, defaultCollapsed) {
    ...
    processDetails = processDetails.filter(function (detail) {
        return !detail || detail.eventType !== 'eino_usage_summary' || !isWebshellPartialEinoUsageSummary(detail.data || {});
    });
    if (!processDetails.length) return null;
        ...
        .then(function (list) {
            ...
            list.forEach(function (item) {
                ...
                delBtn.addEventListener('click', async function (e) {
                    e.stopPropagation();
                    if (!await appConfirm(wsT('webshell.aiDeleteConversationConfirm') || '确定删除该对话？')) return;
...
function selectWebshell(id, stateReady) {
    ...
    workspace.innerHTML =
        ...
        '<button type="button" class="role-selector-btn ws-role-selector-btn" id="ws-role-selector-btn" onclick="wsToggleRolePanel()" aria-haspopup="listbox" aria-expanded="false" aria-controls="ws-role-selection-panel">' +
        ...
        '<button type="button" class="role-selector-btn agent-mode-btn" id="ws-agent-mode-btn" onclick="wsToggleAgentModePanel()" aria-haspopup="listbox" aria-expanded="false" aria-controls="ws-agent-mode-panel">' +
        ...
        dbProfilesEl.querySelectorAll('[data-action]').forEach(function (btn) {
            btn.addEventListener('click', async function () {
                ...
                if (action === 'delete') {
                    ...
                    if (!await appConfirm(wsT('webshell.dbDeleteProfileConfirm') || '确定删除该数据库连接配置吗？')) return;
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                try {
                    ...
                    // ─── Terminal events ───
                        ...
                        if (!isWebshellPartialEinoUsageSummary(_ed)) {
...
function renderFileList(listEl, currentPath, rawOutput, conn, nameFilter) {
    ...
    listEl.querySelectorAll('.webshell-file-del').forEach(function (btn) {
        btn.addEventListener('click', async function (e) {
            e.preventDefault();
            if (!await appConfirm(wsT('webshell.deleteConfirm'))) return;
...
async function webshellFileMkdir(conn, pathInput) {
    if (!conn || typeof apiFetch === 'undefined') return;
    var base = (pathInput && pathInput.value.trim()) || '.';
    var name = await appPrompt(wsT('webshell.newDir') || '新建目录', 'newdir');
...
async function webshellFileNewFile(conn, pathInput) {
    if (!conn || typeof apiFetch === 'undefined') return;
    var base = (pathInput && pathInput.value.trim()) || '.';
    var name = await appPrompt(wsT('webshell.newFile') || '新建文件', 'newfile.txt');
    if (name == null || !name.trim()) return;
    var path = base === '.' ? name.trim() : base + '/' + name.trim();
    var content = await appPrompt('初始内容（可选）', '');
...
async function webshellFileRename(conn, oldPath, oldName, listEl) {
    if (!conn || typeof apiFetch === 'undefined') return;
    var newName = await appPrompt((wsT('webshell.rename') || '重命名') + ': ' + oldName, oldName);
...
async function webshellBatchDelete(conn, pathInput) {
    if (!conn) return;
    var listEl = document.getElementById('webshell-file-list');
    ...
    if (paths.length === 0) { appAlert(wsT('webshell.batchDelete') + '：请先勾选文件'); return; }
    if (!await appConfirm(wsT('webshell.batchDelete') + '：确定删除 ' + paths.length + ' 个文件？')) return;
    var base = (pathInput && pathInput.value.trim()) || '.';
    var i = 0;
    function delNext() {
...
function webshellBatchDownload(conn, pathInput) {
    ...
    var paths = [];
    checked.forEach(function (cb) { paths.push(cb.getAttribute('data-path')); });
    if (paths.length === 0) { appAlert(wsT('webshell.batchDownload') + '：请先勾选文件'); return; }
...
function webshellFileDownload(conn, path) {
    ...
    apiFetch('/api/webshell/file', {
        ...
        .catch(function (err) { appAlert(wsT('webshell.execError') + ': ' + (err && err.message ? err.message : '')); });
}
...
// 删除连接（请求服务端删除后刷新列表）
async function deleteWebshell(id) {
    if (!await appConfirm(wsT('webshell.deleteConfirm'))) return;
...
function showAddWebshellModal() {
    ...
    if (document.getElementById('webshell-clear-password')) document.getElementById('webshell-clear-password').checked = false;
...
function testWebshellConnection() {
    ...
    if (!url) {
        appAlert(wsT('webshell.url') ? (wsT('webshell.url') + ' 必填') : '请填写 Shell 地址');
        return;
    }
    ...
    if (typeof apiFetch === 'undefined') {
        ...
        appAlert(wsT('webshell.testFailed') || '连通性测试失败');
        return;
    }
    ...
    apiFetch('/api/webshell/exec', {
        ...
        body: JSON.stringify({
            url: url,
            password: password || '',
            connection_id: !password && !(document.getElementById('webshell-clear-password') || {}).checked ? ((document.getElementById('webshell-edit-id') || {}).value || '') : '',
            type: type,
        ...
        .then(function (data) {
            ...
            if (reallyOk) {
                appAlert(wsT('webshell.testSuccess') || '连通性正常，Shell 可访问');
            } else {
                var msg;
        ...
        .catch(function (e) {
            if (btn) { btn.disabled = false; btn.textContent = wsT('webshell.testConnectivity'); }
            appAlert((wsT('webshell.testFailed') || '连通性测试失败') + ': ' + (e && e.message ? e.message : String(e)));
        });
}
...
function saveWebshellConnection() {
    ...
    if (editId && !password) delete body.password;
    if ((document.getElementById('webshell-clear-password') || {}).checked) body.password = '';
    if (typeof apiFetch === 'undefined') return;
    ...
    apiFetch(reqUrl, {
        ...
        .catch(function (e) {
            ...
            appAlert(e && e.message ? e.message : '保存失败');
        });
}
