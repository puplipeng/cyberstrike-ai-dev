...
function clearAuthStorage() {
    if (window.SSHManager) window.SSHManager.cleanup();
    authToken = null;
...
const PAGE_PERMISSION_MAP = {
    ...
    'vulnerability-intel': 'vulnerability:read',
    'github-leaks': 'vulnerability:read',
...
async function initializeApp() {
    ...
    if (hasStoredAuth && isTokenValid()) {
        // 本地校验通过立即隐藏登录框（不等远程 validate——实现访问即校验）
        hideLoginOverlay();
        try {
...
async function deleteRobotAccountBinding(id) {
    if (!id || !await appConfirm('确定解除该机器人账号绑定吗？')) return;
