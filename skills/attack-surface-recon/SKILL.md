...
主动流水线(逐步执行):
  ...
  terminal: naabu -l alive_subs.txt -top-ports 1000 -silent | tee ports.txt
  terminal: PowerShell/Windows 取前 20 个目标时用 `Get-Content alive_subs.txt | Select-Object -First 20 | Set-Content sample_subs.txt`，再执行 `nmap -sT -sV -sC -Pn -iL sample_subs.txt -oN nmap_out.txt`；若本机没有 nmap，先用 rustscan 或继续沿用 naabu
  ...
  terminal: ffuf -u https://{target}/FUZZ -w wordlists/common-web.txt -mc 200,301,302,403 -fs {catch-all-size}
